...
// ═══ [GR-D ส่งด่วนต่างจังหวัด] k182 — ครอบทุกสำนวน (487-500) ═══
const gdT = [];
{
  const ตจว = /มีเฉพาะ กทม|ส่งเป็นพัสดุ/;
  // ── ทุกสำนวนที่ลูกค้าต่างจังหวัดใช้จริง ต้องถูกกันหมด ──
  const เคสตจว = [
    ['อยู่ขอนแก่น ขอส่งด่วนได้ไหม', 'ขอนแก่น'],
    ['ส่งด่วนได้ไหม อยู่ภูเก็ต', 'ภูเก็ต'],
    ['อยู่เชียงใหม่ ส่งแกรปได้ไหม', 'เชียงใหม่'],
    ['อยู่อุดรธานี อยากได้ส่งด่วนวันนี้', 'อุดรธานี'],
    ['ส่งด่วนถึงหาดใหญ่ไหมคะ', 'หาดใหญ่'],
    ['สั่งส่งด่วนไปนครราชสีมาได้ป่าว', 'นครราชสีมา'],
    ['อยู่สุราษฎร์ ส่งด่วนกี่บาท', 'สุราษฎร์'],
    ['ขอส่งด่วนครับ อยู่ระยอง', 'ระยอง'],
  ];
  let n = 487, พลาด = [];
  for (const [q, จว] of เคสตจว) {
    const r = await run(q, 'ได้ค่ะ รบกวนปักหมุดด้วยนะคะ');
    const ok = ตจว.test(r.out) && r.out.indexOf(จว) !== -1;
    if (!ok) พลาด.push(q + ' → ' + r.out.slice(0, 70));
    gdT.push({ n, name: '⚠️ ต่างจังหวัดขอส่งด่วน: "' + q + '" → ต้องบอกว่าส่งด่วนมีแค่ กทม.', ok, why: r.out.slice(0, 130) });
    n++;
  }
  // ── ห้ามชวนปักหมุดกับลูกค้าต่างจังหวัด (ปักมาแล้วต้องมาบอกทีหลังว่าส่งไม่ได้) ──
  const r9 = await run('อยู่ขอนแก่น ขอส่งด่วนได้ไหม', 'ได้ค่ะ รบกวนปักหมุดด้วยนะคะ');
  gdT.push({ n: 495, name: '⚠️ ต่างจังหวัด → ห้ามขอให้ปักหมุด/แชร์โลเคชั่น', ok: !/ปักหมุด|โลเคชั่น|แชร์พิกัด/.test(r9.out), why: r9.out.slice(0, 130) });

  // ── กทม./ปริมณฑล ต้องยังใช้ส่งด่วนได้เหมือนเดิม (กันแก้แล้วเกินเลย) ──
  const กทม = [['อยู่ลาดพร้าว ส่งด่วนได้ไหม'], ['ส่งด่วนได้ไหมคะ อยู่บางนา'], ['อยู่นนทบุรี ขอส่งด่วน']];
  let n2 = 496, พลาด2 = [];
  for (const [q] of กทม) {
    const r = await run(q, 'ได้ค่ะ');
    const ok = !ตจว.test(r.out);
    if (!ok) พลาด2.push(q);
    gdT.push({ n: n2, name: '⚠️ กทม./ปริมณฑล: "' + q + '" → ต้องไม่โดนปฏิเสธส่งด่วน', ok, why: r.out.slice(0, 120) });
    n2++;
  }
  // ── สั่งของพร้อมบอกส่งด่วน = ต้องยังออกการ์ด ไม่ใช่ตกไปทางส่งด่วนแล้วออเดอร์หาย ──
  const r10 = await run('เอา MARBO 9K องุ่น 2 อัน ส่งด่วน', 'ขออนุญาตทวนคำสั่งซื้ออีกครั้งนะคะ 🧾\nMARBO 9K | องุ่น | 2');
  gdT.push({ n: 499, name: '⚠️ สั่งของ + บอก "ส่งด่วน" ในประโยคเดียว → ออเดอร์ต้องไม่หาย',
    ok: /การ์ด|ทวนคำสั่งซื้อ|MARBO/.test(r10.out), why: r10.out.slice(0, 130) });
  const r11 = await run('เอา MARBO 9K องุ่น 2 อัน ส่งแกรป', 'ขออนุญาตทวนคำสั่งซื้ออีกครั้งนะคะ 🧾\nMARBO 9K | องุ่น | 2');
  gdT.push({ n: 500, name: 'สั่งของ + "ส่งแกรป" → ต้องยังทำงานเหมือนเดิม (ไม่ถอยหลัง)',
    ok: /การ์ด|ทวนคำสั่งซื้อ|MARBO/.test(r11.out), why: r11.out.slice(0, 130) });
}
for (const t of gdT) {
...
// ═══ [อ่านรูปที่วง] k184 — เครื่องหมายลูกค้ามีอำนาจสูงสุด (517-526) ═══
const imT = [];
{
  const SRC = readFileSync(WORKER, 'utf8');
  const iรูป = SRC.indexOf('ลูกค้าส่งรูปนี้มา');
  const บล็อก = SRC.slice(iรูป, SRC.indexOf('image_url', iรูป));
  const iวง = บล็อก.indexOf('ขั้นที่ 2 — อ่านบริเวณที่ลูกค้าทำเครื่องหมาย');
  const iทั้งภาพ = บล็อก.indexOf('ขั้นที่ 3 — อ่านรูปสินค้าจริง');
  const iกฎ = บล็อก.indexOf('กฎสูงสุดของการอ่านรูป');

  imT.push({ n: 517, name: '⚠️ เคสจริง: มีกฎสูงสุดว่าเครื่องหมายลูกค้าคือคำสั่ง', ok: iกฎ !== -1, why: 'ไม่มีกฎสูงสุด' });
  imT.push({ n: 518, name: '⚠️ กฎสูงสุดต้องอยู่ก่อนทุกขั้น (ไม่ถูกดันลงไปท้าย)', ok: iกฎ !== -1 && iกฎ < iวง && iกฎ < iทั้งภาพ, why: `กฎ=${iกฎ} วง=${iวง} ทั้งภาพ=${iทั้งภาพ}` });
  imT.push({ n: 519, name: '⚠️ ขั้นอ่าน "บริเวณที่วง" ต้องมาก่อนขั้นอ่าน "ทั้งภาพ"', ok: iวง !== -1 && iทั้งภาพ !== -1 && iวง < iทั้งภาพ, why: `วง=${iวง} ทั้งภาพ=${iทั้งภาพ}` });
  imT.push({ n: 520, name: 'มีคำสั่งว่าอ่านเฉพาะบริเวณที่ทำเครื่องหมาย', ok: /เฉพาะสิ่งที่อยู่ในบริเวณที่ทำเครื่องหมาย/.test(บล็อก), why: '' });
  imT.push({ n: 521, name: 'มีคำสั่งว่าส่วนอื่นของรูปไม่ใช่ตัวเลือกให้เสนอ', ok: /ไม่ใช่ตัวเลือกให้เสนอ/.test(บล็อก), why: '' });
  imT.push({ n: 522, name: 'มีกฎว่าถ้าขัดกัน ให้ยึดสิ่งที่ถูกวง', ok: /ยึดสิ่งที่ถูกวงเสมอ/.test(บล็อก), why: '' });
  imT.push({ n: 523, name: '⚠️ ห้ามเอ่ยรุ่นที่ไม่ปรากฏในรูป (ต้นเหตุเคส ESKO)', ok: /ไม่ปรากฏอยู่ในรูปนี้/.test(บล็อก), why: '' });
  imT.push({ n: 524, name: '⚠️ ห้ามหยิบรุ่นจากบทสนทนาก่อนหน้ามาเติม', ok: /ห้ามหยิบรุ่นจากบทสนทนาก่อนหน้า/.test(บล็อก), why: '' });
  imT.push({ n: 525, name: 'อ่านไม่ออกต้องถามกลับ ห้ามเดารุ่นยอดฮิต', ok: /ห้ามเดารุ่นยอดฮิต/.test(บล็อก), why: '' });
  imT.push({ n: 526, name: 'ยังคงจำแนกสลิปเป็นขั้นแรกเหมือนเดิม (ไม่กระทบโฟลว์สลิป)', ok: /ขั้นที่ 1 — จำแนกรูป/.test(บล็อก) && /\[SLIP\]/.test(บล็อก), why: '' });
}
for (const t of imT) {
...
// ═══ [สมองคุย] k186 ระยะ 1 — ไม่ถามซ้ำ + ไม่ตอบยอดผิดขั้น (538-543) ═══
const cbT = [];
{
  const คุย = async (uid, ask, ai) => {
    sent = []; aiReply = ai;
  ...
  const ถามซ้ำ = /รับกลิ่นไหน กี่ชิ้นดีคะ/;

  // 1) บอกครบในข้อความเดียว → ห้ามถามซ้ำ
  ตั้งต้น();
  const r1 = await คุย('B1', 'ขอองุ่น 2 อันครับ MARBO 9K ส่งพัสดุธรรมดา', การ์ด('MARBO 9K', 'องุ่น', 2));
  cbT.push({ n: 538, name: '⚠️ บอกครบในข้อความเดียว → ห้ามถามซ้ำว่า "รับกลิ่นไหน กี่ชิ้น"',
    ok: !ถามซ้ำ.test(r1) && /การ์ด|MARBO/.test(r1), why: r1.slice(0, 110) });

  // 2) MARBO 9K → องุ่น → 2 (3 เทิร์น) → เทิร์นสุดท้ายต้องไปขั้นถัดไป
  ตั้งต้น();
  await คุย('B2', 'MARBO 9K', 'MARBO 9K ราคา 350 บาทค่ะ รับกลิ่นไหนดีคะ');
  await คุย('B2', 'องุ่น', 'รับกี่ชิ้นดีคะ 💕');
  const r2 = await คุย('B2', '2 อัน', การ์ด('MARBO 9K', 'องุ่น', 2));
  cbT.push({ n: 539, name: 'คุย 3 เทิร์นทีละช่อง → เทิร์นสุดท้ายต้องไปขั้นถัดไป (ออกการ์ด)',
    ok: /การ์ด|MARBO/.test(r2), why: r2.slice(0, 110) });

  // 3) ถามจำนวนแล้วลูกค้าตอบ "1" → ต้องตีความเป็นจำนวน
  ตั้งต้น();
  await คุย('B3', 'เอา MARBO 9K องุ่น', 'รับกี่ชิ้นดีคะ 💕');
  const r3 = await คุย('B3', '1', การ์ด('MARBO 9K', 'องุ่น', 1));
  cbT.push({ n: 540, name: 'ตอบ "1" หลังบอทถามจำนวน → ต้องตีความเป็นจำนวน ไม่ถามซ้ำ',
    ok: !ถามซ้ำ.test(r3) && /การ์ด|MARBO/.test(r3), why: r3.slice(0, 110) });

  // 4) มีออเดอร์เก่าค้าง → ถามราคาสินค้าใหม่ → ห้ามตอบยอดออเดอร์เก่า
  ตั้งต้น();
  await คุย('B4', 'เอา STAR 2,500 แตงโม 2 อัน ส่งพัสดุ', การ์ด('STAR 2,500', 'แตงโม', 2));
  const r4 = await คุย('B4', 'มาโบ 9k 10 ตัวเท่าไหร่', 'MARBO 9K 350 บาท/แท่งค่ะ');
  cbT.push({ n: 541, name: '⚠️ เคสจริง 🎦: มีออเดอร์เก่าค้าง + ถามราคาของใหม่ → ห้ามตอบยอดออเดอร์เก่า',
    ok: !/ยอดที่ต้องโอนคือ\s*340/.test(r4) && !/STAR 2,500 แตงโม x2/.test(r4), why: r4.slice(0, 120) });

  // 5) มีออเดอร์รอโอน → "รวมเท่าไหร่" → ต้องยังตอบยอดออเดอร์ (ไม่ถอยหลัง)
  ตั้งต้น();
  await คุย('B5', 'เอา MARBO 9K องุ่น 2 อัน ส่งพัสดุ', การ์ด('MARBO 9K', 'องุ่น', 2));
  const r5 = await คุย('B5', 'รวมเท่าไหร่', 'ยอดตามการ์ดค่ะ');
  cbT.push({ n: 542, name: '⚠️ มีออเดอร์รอโอน + "รวมเท่าไหร่" → ต้องยังตอบยอด 740 (ไม่ถอยหลัง)',
    ok: /740/.test(r5), why: r5.slice(0, 110) });

  // 6) บอกข้อมูลเดิมซ้ำ → ห้ามย้อนไปถามช่องที่ครบแล้ว
  ตั้งต้น();
  await คุย('B6', 'เอา MARBO 9K องุ่น 2 อัน ส่งพัสดุ', การ์ด('MARBO 9K', 'องุ่น', 2));
  const r6 = await คุย('B6', 'MARBO 9K องุ่น 2 อัน ส่งพัสดุธรรมดา', การ์ด('MARBO 9K', 'องุ่น', 2));
  cbT.push({ n: 543, name: '⚠️ ลูกค้าบอกข้อมูลเดิมซ้ำ → ห้ามย้อนกลับไปถามช่องที่ครบแล้ว',
    ok: !ถามซ้ำ.test(r6), why: r6.slice(0, 110) });
}
for (const t of cbT) {
  if (t.ok) { pass++; console.log(`${GRN}✅ ${t.n}${RESET} ${DIM}[สมองคุย]${RESET} ${t.name}`); }
  else { fails.push({ n: String(t.n), c: { ask: t.name }, why: [t.why], out: String(t.why || '') }); console.log(`${RED}❌ ${t.n}${RESET} ${DIM}[สมองคุย]${RESET} ${t.name}\n      ${RED}↓${RESET} ${t.why}`); }
}

// ═══ [คุยเหมือนคน] k187 — จำสิ่งที่ลูกค้าเพิ่งบอก ไม่ถามซ้ำ (544-550) ═══
const hmT = [];
{
  const คุย = async (uid, ask, ai) => {
    sent = []; aiReply = ai;
  ...
  // 1) เคสจริงสนามซ้อม: รุ่นอยู่เทิร์น 1 · กลิ่น+จำนวนอยู่เทิร์น 2
  ตั้งต้น();
  await คุย('H1', 'ขอราคา marbo 9k กลิ่นองุ่น 2 อัน ส่งems', 'กลิ่นองุ่นหมดค่ะ แนะนำองุ่นว่านหางจระเข้');
  const r1 = await คุย('H1', 'เออ งั้นเอาองุ่นว่านหางจระเข้ 2 อัน ส่งพัสดุธรรมดา โอนเท่าไหร่', การ์ด('MARBO 9K', 'องุ่นว่านหางจระเข้', 2));
  hmT.push({ n: 544, name: '⚠️ เคสจริง: รุ่นอยู่เทิร์นก่อน · กลิ่น+จำนวนเทิร์นนี้ → ห้ามถามซ้ำ',
    ok: !ถามซ้ำ.test(r1) && !!store.get('ord:v20:H1'), why: r1.slice(0, 110) });

  // 2) บอกรุ่นเทิร์น 1 → สั่งเทิร์น 2
  ตั้งต้น();
  await คุย('H2', 'MARBO 9K มีกลิ่นไหนบ้าง', 'MARBO 9K มีองุ่น โคล่า ค่ะ');
  const r2 = await คุย('H2', 'เอาองุ่น 2 อัน ส่งพัสดุธรรมดา', การ์ด('MARBO 9K', 'องุ่น', 2));
  hmT.push({ n: 545, name: 'จำรุ่นจากเทิร์นก่อนได้ → สั่งต่อได้เลย', ok: !ถามซ้ำ.test(r2) && !!store.get('ord:v20:H2'), why: r2.slice(0, 110) });

  // 3) ⚠️ ลูกค้าเปลี่ยนใจ — ข้อความล่าสุดต้องชนะของเก่า
  ตั้งต้น();
  await คุย('H3', 'MARBO 9K มีกลิ่นไหน', 'MARBO 9K มีองุ่น โคล่า ค่ะ');
  await คุย('H3', 'เอา INFY 20K มิ้นต์ 1 อัน ส่งพัสดุธรรมดา', การ์ด('INFY 20K', 'มิ้นต์', 1));
  const ord3 = String(store.get('ord:v20:H3') || '');
  hmT.push({ n: 546, name: '⚠️ ลูกค้าเปลี่ยนรุ่น → ข้อความล่าสุดต้องชนะ ห้ามใช้รุ่นเก่า',
    ok: ord3.indexOf('INFY 20K') !== -1 && ord3.indexOf('MARBO 9K') === -1, why: ord3.slice(0, 110) });

  // 4) ⚠️ ถามเฉยๆ ไม่ได้สั่ง → ห้ามออกการ์ดเอง
  ตั้งต้น();
  await คุย('H4', 'MARBO 9K มีกลิ่นไหน', 'MARBO 9K มีองุ่น โคล่า ค่ะ');
  await คุย('H4', 'องุ่นเป็นยังไง', 'องุ่นหวานอมเปรี้ยวค่ะ');
  hmT.push({ n: 547, name: '⚠️ คุยเรื่องกลิ่นเฉยๆ ยังไม่บอกจำนวน → ห้ามออกการ์ดเอง',
    ok: !store.get('ord:v20:H4'), why: String(store.get('ord:v20:H4') || '(ไม่มีออเดอร์ ✓)').slice(0, 90) });

  // 5) ⚠️ ย้อนไกลเกินไปต้องไม่หยิบมาใช้ (เกิน 6 เทิร์น)
  ตั้งต้น();
  await คุย('H5', 'MARBO 9K มีกลิ่นไหน', 'มีองุ่น โคล่า ค่ะ');
  for (let i = 0; i < 7; i++) await คุย('H5', 'ส่งกี่วันถึง', 'พัสดุ 2-3 วันค่ะ');
  const r5 = await คุย('H5', 'ส่งพัสดุธรรมดา', 'รับทราบค่ะ');
  hmT.push({ n: 548, name: 'ข้อมูลเก่าเกิน 6 เทิร์น → ต้องไม่เดาว่าลูกค้ายังสั่งอยู่', ok: !store.get('ord:v20:H5'), why: r5.slice(0, 90) });

  // 6) ไม่ถอยหลัง: บอกครบข้อความเดียว
  ตั้งต้น();
  const r6 = await คุย('H6', 'ขอองุ่น 2 อันครับ MARBO 9K ส่งพัสดุธรรมดา', การ์ด('MARBO 9K', 'องุ่น', 2));
  hmT.push({ n: 549, name: 'ไม่ถอยหลัง: บอกครบในข้อความเดียว → ยังออกการ์ดทันที', ok: !ถามซ้ำ.test(r6), why: r6.slice(0, 100) });

  // 7) ไม่ถอยหลัง: ถามวิธีส่งเฉยๆ ต้องยังตอบข้อมูลค่าส่ง
  ตั้งต้น();
  const r7 = await คุย('H7', 'ส่งพัสดุยังไง', 'พัสดุ 40 บาทค่ะ');
