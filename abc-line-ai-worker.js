...
const BUILD = "2026-07-28-j1-lessqr";
...
function buildQuickReply(reply, userText, sm, buf) {
  try {
    ...
    // ส่งต่อแอดมินแล้ว (จีทูถูกปิดในแชทนั้น) → ห้ามมีปุ่มให้กด เพราะกดไปก็ไม่มีใครตอบ
    if (/แอดมินเข้ามาดูแล|แอดมินหลังการขาย|รับเรื่องแล้ว|ไม่สามารถให้คำแนะนำ/.test(r)) return null;
    ...
    if (!askFlavor && /กี่ชิ้น|กี่แท่ง|กี่หัว|กี่อัน|กี่กล่อง|กี่คอต|กี่ตลับ|กี่ซอง|กี่กระปุก|กี่ชุด|กี่ตัว|กี่ลัง|จำนวนเท่าไหร่|เอากี่|รับกี่/.test(r))
    ...
    // ⛔ ไม่มีเมนูปุ่มทั่วไปแล้ว — โชว์เฉพาะตอนที่ปุ่มช่วยลูกค้าจริง (เลือกกลิ่น / จำนวน / วิธีส่ง)
    // เดิมเมนู 5 ปุ่มขึ้นแทบทุกข้อความ ทำให้แชทรก และหลอกให้กดตอนจีทูถูกปิดอยู่
  } catch (e) {}
  return null;
}
