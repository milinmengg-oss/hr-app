 ...
 *   SLIPOK_KEY / SLIPOK_BRANCH = ตรวจสลิปอัตโนมัติกับ SlipOK (ไม่บังคับ — ถ้าไม่ตั้ง แอดมินจะแค่รับสลิปเฉยๆ)
...
const BUILD = "2026-08-07-k227-knowunknown";
...
// 🔇 k114: มิ้วต์ที่เพิ่งตั้ง — จำในเครื่องทันที (แคช KV 60 วิ ทำให้แอดมินตอบแทรกช่วงแอดมินเพิ่งรับเคส)
const MUTED = new Map();
// k185 (GR-G): จำว่าตอบรับ "ทีมงานกำลังดูแล" ให้ใครไปเมื่อไหร่ — กันตอบรัวกวนคนที่กำลังคุย
const MUTEACK = new Map();
// 🔇🔇 k201 — "ปิดปากแล้วต้องเงียบ 100%"
//   ชุด userId ที่ถูกส่งต่อคนในคำขอนี้ — ใช้เป็นด่านสุดท้ายก่อนยิงข้อความออก LINE
//   กันคำตอบที่ประมวลผลค้างอยู่หลุดออกไปหลังจากปิด G2 ไปแล้ว
//   เก็บเป็น "โควตาข้อความที่ยังส่งได้" — ตอน handoff อนุญาต 1 ข้อความ (ข้อความแจ้งรับเรื่อง)
//   ที่เหลือทั้งหมดถูกบล็อก ⛔ ไม่ต้องไปไล่แก้ 17 จุดที่เรียก muteNow ทีละจุด
const MUTED_UIDS = new Map();
// ⏱️ k224 · B-1: อายุของตัวปิดปากในหน่วยความจำ — ต้องเท่ากับ TTL ของคีย์ mute: ใน KV (1 ชม.)
//   ถ้าไม่มีค่านี้ ลูกค้าที่ถูกส่งต่อคนครั้งเดียวจะไม่ได้รับข้อความอีกเลยตลอดอายุของ isolate
const MUTED_UID_TTL_MS = 60 * 60 * 1000;
// 🔌🔌 k224 · B-3 — กดปิดบอทแล้วต้องหยุดทันที
//   ปัญหาเดิม: ทุกข้อความอ่าน botoff: จาก KV ตรง ๆ ซึ่ง Cloudflare KV เป็น eventually consistent
//     ค่าที่เพิ่งเขียนอาจใช้เวลาถึง ~60 วินาทีกว่าจะอ่านเจอ → กดปิดแล้วบอทยังตอบลูกค้าต่ออีก 1 นาที
//     วันเปิดร้าน ถ้าบอทตอบผิด นาทีนั้นคือความเสียหายที่กู้คืนไม่ได้
//   แก้: จำผลไว้ในหน่วยความจำสั้น ๆ (3 วิ) เพื่อ**ลดจำนวนการอ่าน** ไม่ใช่เพื่อยืดอายุ
//     และที่สำคัญกว่า — เมื่อแอดมิน "กดปิด" ให้เขียนลงหน่วยความจำทันที ไม่ต้องรอ KV
//   ⛔ ห้ามยืด TTL ของ cache เกินไม่กี่วินาที ไม่งั้นจะกลายเป็นตัวหน่วงเสียเอง
const BOTOFF_CACHE = new Map();          // shopId → { off: boolean, t: number }
const BOTOFF_CACHE_MS = 3000;
function botoffSet(shopId, off) {        // เรียกทันทีที่แอดมินกดเปิด/ปิด
  try { BOTOFF_CACHE.set(String(shopId), { off: !!off, t: Date.now() }); } catch (e) {}
}
async function botoffGet(env, shopId) {
  try {
    const k = String(shopId);
    const c = BOTOFF_CACHE.get(k);
    if (c && (Date.now() - c.t) < BOTOFF_CACHE_MS) return c.off;
    const v = env && env.CONV ? await env.CONV.get("botoff:" + k) : null;
    const off = !!v;
    BOTOFF_CACHE.set(k, { off, t: Date.now() });
    if (BOTOFF_CACHE.size > 200) { const k0 = BOTOFF_CACHE.keys().next().value; BOTOFF_CACHE.delete(k0); }
    return off;
...
// 🛵 k107: ค่าส่งด่วนที่แอดมินเพิ่งกรอก — จำในเครื่องทันที (KV อ่านช้า/แคช 60 วิ ทำให้แอดมินบอก "กำลังเช็ค" ทั้งที่แจ้งราคาแล้ว)
const EXPFEE = new Map();
// 📦 k124: เลขพัสดุที่แอดมินเพิ่งกรอก — เก็บในหน่วยความจำด้วย กันดีเลย์ KV 60 วิ (บทเรียนจาก k94/k107)
const TRACK = new Map();
// k139: จำว่าเพิ่งยกเลิกไปเมื่อไหร่ กันตอบข้อความยกเลิกซ้ำเวลาลูกค้าพิมพ์ติดกัน 2 ข้อความ
...
//     ลูกค้าถามตรงๆ 3 รอบ: "ต้องโอนยอดไหนคะ" · "350 หรือ 209คะ" · "350หรือ 509 คะ"
//     → บอทตอบ "ยอดรวมในการ์ดคือยอดที่ถูกต้องค่ะ" ทั้ง 3 ครั้ง ไม่เคยบอกตัวเลขเลย
//     → แล้วข้อความทวนรายการโชว์ "MARBO 9K เยลลี่ x1 = 350" เฉยๆ = ลูกค้าเข้าใจว่าโอน 350
//     → ขาดไป 159 บาท ทุกออเดอร์ที่เข้าเส้นนี้
//   ต้นเหตุ: ระบบพูดถึง "ยอด" ได้หลายที่ แต่ไม่มีที่ไหนรับประกันว่าเป็นยอดที่ต้องโอนจริง
//     · คำถามเรื่องยอดไม่มีทางลัดรับ → ตกไปให้ AI ซึ่งโดน k15 ห้ามพูดเรื่องการชำระเงิน
//     · ข้อความทวนรายการ (k27) กรองบรรทัดด้วย /x\d|รวมยอด/ → ถ้าไม่มีบรรทัดยอดรวม เหลือแต่ราคาสินค้า
//   กฎใหม่: **ทุกที่ที่พูดถึงยอด ต้องใช้ ordTotal() ตัวเดียวกัน**
//     ⛔ ถ้ายังไม่รู้ยอด (รอค่าส่งด่วน) ต้องบอกตรงๆ ว่ายังไม่สรุป — ห้ามโชว์ราคาสินค้าลอยๆ
//        เพราะลูกค้าจะอ่านเป็นยอดโอนทันที (นี่คือความเสียหายจริงของเคสนี้)
// 🧾🧾 k224 · B-2 — สถานะ "ชำระแล้ว" ต้องไม่สืบทอดข้ามออเดอร์
//   ปัญหาเดิม: คีย์ ord: อยู่ได้ 3 วัน และถูกลบเฉพาะตอนแอดมินกดปุ่มในหลังบ้าน
//     ลูกค้าเก่ากลับมาซื้อของเดิมภายใน 3 วัน → ระบบเห็นสถานะ "✅ ชำระแล้ว" ของออเดอร์ที่แล้ว
//     → ตอบว่า "ชำระเงินเรียบร้อยแล้ว ไม่ต้องกดยืนยันซ้ำ" → **ไม่ส่งเลขบัญชี = ส่งของฟรี**
//   ⛔ ห้ามลด TTL ของ ord: — หลังบ้านต้องเห็นออเดอร์ย้อนหลัง 3 วัน และนั่นคือข้อมูลธุรกรรม
//   แก้ที่ "การตีความ" แทน: ออเดอร์ที่จ่ายแล้วจะถือว่า "ยังเป็นออเดอร์ปัจจุบัน" ได้ไม่เกิน 12 ชม.
//     เกินกว่านั้น = คนละรอบการซื้อ ต้องเริ่มขั้นตอนเก็บเงินใหม่ตามปกติ
//   ⚠️ ตัวข้อมูลยังอยู่ครบ ไม่ได้ลบอะไรเลย — เปลี่ยนแค่ว่า "ยังใช้กันการเก็บเงินซ้ำอยู่ไหม"
const ORD_PAID_ACTIVE_MS = 12 * 60 * 60 * 1000;
function ordPaidActive(oj, now) {
  try {
    if (!oj || !/✅/.test(String(oj.status || ""))) return false;
    const t = +(oj.t || 0);
    if (!t) return true;                                   // ไม่มีเวลา = ของเก่ามาก ให้เชื่อไว้ก่อน (ปลอดภัยกว่า)
    const age = (now || Date.now()) - t;
    if (age <= ORD_PAID_ACTIVE_MS) return true;
    console.log("K224_ORD_PAID_STALE age=" + Math.round(age / 3600000) + "h → ถือเป็นออเดอร์ใหม่");
    return false;
  } catch (e) { return false; }
}
function ordTotal(ordObj) {
  const b = String((ordObj && ordObj.block) || "");
  const n = (re) => { const m = b.match(re); return m ? +String(m[1]).replace(/,/g, "") : null; };
  const total = n(/(?:รวมยอดชำระ|ยอดรวม)\s*([\d,]+)/);
  const goods = n(/ยอดสินค้า\s*([\d,]+)/);
  const ship = n(/ค่าส่ง(?:ด่วน)?\s*([\d,]+)/);
  const waitFee = /ค่าส่งด่วน \(รอทีมงานเช็ค\)/.test(b);
  const items = b.split("\n").filter(l => /^-\s/.test(l));
  return { total, goods, ship, waitFee, items, express: /ค่าส่งด่วน/.test(b) };
}
// ข้อความสรุปยอดที่ปลอดภัย — ใช้ได้ทุกที่ที่ต้องบอกลูกค้าว่าต้องโอนเท่าไหร่
function totalMsg(ordObj) {
  const o = ordTotal(ordObj);
  const lines = o.items.length ? o.items.join("\n") + "\n" : "";
  if (o.total !== null) {
    return "ยอดที่ต้องโอนคือ **" + o.total + " บาท** ค่ะ 💰\n\n" + lines
      + (o.goods !== null ? "• ค่าสินค้า " + o.goods + " บาท\n" : "")
      + (o.ship !== null ? "• ค่าส่ง" + (o.express ? "ด่วน" : "") + " " + o.ship + " บาท\n" : "")
      + "• รวม **" + o.total + " บาท**";
  }
  if (o.waitFee) {
    return "ยอดรวมยังสรุปไม่ได้ค่ะ 🙏🏻 เพราะทีมงานกำลังเช็คค่าส่งด่วนให้อยู่นะคะ 🛵\n\n" + lines
      + (o.goods !== null ? "• ค่าสินค้า " + o.goods + " บาท\n" : "")
      + "• ค่าส่งด่วน — รอทีมงานแจ้งราคา\n\n⛔ รบกวนรอยอดรวมก่อนโอนนะคะ จะได้ไม่ต้องโอนซ้ำค่ะ 💕";
  }
  return "";
}
...
const RX_ANGRY = /ไม่ให้อภัย|ตอบมา(สิ|ซิ|หน่อย|ที|เลย)|ทำไมไม่เข้าใจ|ตอบไม่ตรง|ถามอยู่|ห่วย|แย่มาก|แจ้งตำรวจ|สคบ|โกง|หลอก|จะร้องเรียน|เอาเรื่อง|ไม่พอใจ|งี่เง่า|โง่/;
// 💸💸💸 k165 — รวมกฎ "ขอเงินคืน / ยกเลิก" ทั้งหมดเป็นกฎเดียว (แทน RX_MONEYBACK เดิมที่ไล่เติมคำทีละคำ)
//   เคสจริง 4/8 01.19 (ลูกค้า JW · **บั๊กเงินที่หนักสุดของวัน**):
//     ลูกค้าจ่ายจริง 190 บาท (สลิปผ่าน ✅) → "ไม่รับแล้ว" → "คืนยอดค่ะ"
//     → บอทตอบ "เดี๋ยวระบบสรุปยอดและแจ้งข้อมูลการชำระเงินให้ทันทีค่ะ" = **ชวนให้จ่ายเพิ่ม**
//     → และ **ไม่ส่งต่อคนเลย** ทั้งที่เป็นเรื่องเงินคืนของลูกค้าที่จ่ายจริงแล้ว
//   ต้นเหตุ: RX_MONEYBACK เดิมจับแบบ "เรียงตามลำดับคำ" — ต้องเป็น <เงิน> แล้วตามด้วย <คืน>
//     "ขอยอดคืน" จับได้ ✓ · "คืนยอด" จับไม่ได้ ✗ (ภาษาไทยสลับลำดับได้อิสระ)
//     k147b เคยเจอปัญหาเดียวกันแล้วแก้ด้วยการ "เติมคำ โอนคืน" → พอเจอคำใหม่ก็หลุดอีก = ไล่แก้ไม่จบ
//   กฎใหม่ (แก้ที่รูปแบบ ไม่ใช่ไล่เติมคำ):
//     • มีคำเรื่องเงิน + มีคำเรื่องคืน/ยกเลิก อยู่ในข้อความเดียวกัน = ขอเงินคืน **ไม่ว่าลำดับไหน**
//     • คำอ้อม (ไม่เอาแล้ว/ไม่รับแล้ว/ยกเลิก) = ขอเงินคืน **เมื่อลูกค้าจ่ายเงินแล้วเท่านั้น**
//   ⚠️ เกณฑ์คือ "จ่ายเงินแล้วหรือยัง" ไม่ใช่ "มีออเดอร์ไหม" — ยกเลิกตอนยังไม่โอน
//      เป็นเรื่องปกติ ระบบยกเลิกให้ได้เลย ไม่ต้องเรียกคน (กฎ k108 ที่มีอยู่แล้ว ห้ามทับ)
//   ⛔ พลาดเคสเงินคืนจริง เสียหายกว่าเคสด่วนปลอมมาก — เข้าข่ายสงสัยให้ส่งต่อคนไว้ก่อน
const RX_MONEY_WORD = /ตัง|ตังค์|สตางค์|เงิน|ค่าของ|ค่าสินค้า|ยอด|ออเดอร์|order|รายการ|ที่โอน|ที่จ่าย/i;
const RX_BACK_WORD = /คืน|ยกเลิก|refund|cancel|ไม่รับแล้ว|ไม่เอาแล้ว/i;
// คำอ้อมที่ต้อง "มีออเดอร์อยู่จริง" ก่อนถึงจะนับเป็นขอเงินคืน (k163)
const RX_SOFT_QUIT = /ไม่อยากได้แล้ว|ไม่อยากได้ละ|ไม่เอาของแล้ว|ไม่เอาแล้ว|ไม่รับแล้ว|ยกเลิก/;
// คำที่พูดถึงเงินตรงๆ → ส่งต่อคนเสมอ ไม่ว่ามีออเดอร์หรือไม่
const RX_HARD_REFUND = /(ตัง|ตังค์|สตางค์|เงิน|ค่าของ|ยอด)\s*คืน|คืน\s*(ตัง|ตังค์|สตางค์|เงิน|ค่าของ|ยอด)|คืนเงิน|เงินคืน|โอนคืน|โอนกลับ|refund/i;
function refundIntent(text, paid) {
  const t = String(text || "");
  if (RX_HARD_REFUND.test(t)) return true;                              // พูดถึงเงินตรงๆ = ส่งต่อคนเสมอ
  if (!paid) return false;                                              // ยังไม่จ่าย = ยกเลิกได้ตามปกติ (k108)
  if (RX_MONEY_WORD.test(t) && RX_BACK_WORD.test(t)) return true;       // เงิน + คืน/ยกเลิก (ลำดับไหนก็ได้)
  if (RX_SOFT_QUIT.test(t)) return true;                                // คำอ้อม + จ่ายเงินแล้ว
  return false;
}
// ลิงก์เช็คสถานะ — ใช้ลิงก์แทนการพิมพ์ชื่อขนส่ง (เจ้าของร้านสั่ง: ห้ามเอ่ยชื่อบริษัทขนส่ง)
const TRACK_URL = {
  flash: no => "https://www.flashexpress.com/fle/tracking?se=" + encodeURIComponent(no),
  kerry: no => "https://th.kerryexpress.com/th/track/?track=" + encodeURIComponent(no),
};
function trackMsg(rec) {
  const link = (TRACK_URL[rec.c] || TRACK_URL.flash)(rec.no);
  return "📦 พัสดุของคุณลูกค้าจัดส่งออกเรียบร้อยแล้วนะคะ 💕\n\n"
    + "เลขพัสดุ: " + rec.no + "\n"
...
async function resetSession(env, shopId, userId) {
  ...
  try {
    ...
    if (ov) paid = ordPaidActive(JSON.parse(ov) || {});
...
const MODELS = [
  ...
  // k121 (เจ้าของร้าน 3/8): สลับหลักเป็น DeepSeek V4 Flash — Qwen เดาเก่งเกิน (จับรุ่นผิด/มโนกลิ่น)
  //   V4 Flash: $0.098/$0.197 ต่อ 1M token · ctx 1M · ถูกกว่า DeepSeek V3 เดิม 2-4 เท่า
  //   แพงกว่า Qwen ~2-3 เท่า แต่เดาน้อยกว่า = ลดงานตามแก้ · สลับกลับ: /setmodel?m=qwen/qwen3.7-flash
  // k128 (จากตัวเลขจริง /quality 3/8): DeepSeek ตอบไม่ทันประมาณ 20% แล้วตกไปตัวสำรอง
  //   วันนี้ DeepSeek ตอบ 32 ครั้ง (แม่น 96.9%) · Qwen แทรกมา 6 ครั้ง (แม่น 83.3%) · Gemini 2 ครั้ง (100%)
  //   → เจ้าของร้านนึกว่าใช้ DeepSeek ล้วน แต่ 1 ใน 5 คำตอบเป็น Qwen ตัวที่เดาเก่งเกิน
  //   แก้: ไม่ยืดเวลารอ (ลูกค้าทุกคนจะช้าขึ้นเพื่อแก้ปัญหาแค่ 20%) แต่เปลี่ยน "ตัวสำรองที่ 1" เป็น Gemini
  //        Gemini 2.5 Flash แม่นกว่า Qwen · Qwen เก็บไว้ท้ายสุดเป็นตัวกันตาย (เร็วและถูกที่สุด)
  "deepseek/deepseek-v4-flash",          // หลัก: DeepSeek V4 Flash
  "google/gemini-2.5-flash",             // สำรอง 1: แม่นรองลงมา (เดิมคือ Qwen ซึ่งมั่วบ่อย)
  "qwen/qwen3.7-flash",                  // สำรอง 2: กันตายท้ายสุด เร็วที่สุด
];
...
// 🎁 k151 (เจ้าของร้านยืนยัน 3 ส.ค. 22.55): **โปร "ครบ 1,000 บาท = ส่งฟรี" ไม่มีจริง**
//   แต่ข้อความนี้โฆษณาไว้ 3 จุด ทั้งที่ computeOrder ไม่เคยมีกฎนี้เลย (คิดค่าส่ง 40 ตามจริงเสมอ)
//   = บอกลูกค้าว่ามีโปร แล้วเก็บเงินจริงไม่ตรงกับที่บอก → ลูกค้าทักท้วงหน้างาน เสียเครดิตร้าน
//   เคสจริง 3/8 22.52: ลูกค้าถาม "มีโปรไร" → บอทตอบ "ยอดรวมครบ 1,000 บาท = ส่งฟรี"
//   แก้: ตัดออกทุกจุด เหลือเฉพาะโปรส่งฟรีตามจำนวนชิ้น ซึ่งตรงกับที่โค้ดคิดเงินจริง
const PROMO_MSG = "🎁 โปรโมชั่นของร้านตอนนี้ค่ะ 💕\n\n📦 ซื้อครบตามจำนวน = ส่งฟรี\n• พอตใช้แล้วทิ้ง — ครบ 4 แท่ง (คละยี่ห้อได้)\n• บิ๊กพอต / ชุด KIT — ครบ 4 ชิ้น\n• หัวพอตเล็ก — ครบ 10 หัว\n• ไส้บุหรี่ IQOS — ครบ 2 ชิ้น\n\n🎁 บิ๊กพอตครบ 5 หัว = แถมเครื่องเปล่า 1 เครื่อง (มูลค่า 250)\n(ออเดอร์ที่ได้ของแถม จะคิดค่าส่ง 40 บาทตามปกตินะคะ)\n\n⚠️ เครื่อง IQOS · น้ำยาขวด · นิโคตินพอช ไม่ร่วมโปรส่งฟรีนะคะ\n⚠️ โปรส่งฟรีใช้กับพัสดุปกติเท่านั้น ส่งด่วนคิดค่าส่งตามระยะทางค่ะ\n\nสนใจรุ่นไหนแจ้งได้เลยค่ะ เดี๋ยวแอดมินสรุปยอดให้ ✨";
const CLAIM_MSG = "📋 ระยะเวลารับเคลมสินค้าค่ะ\n\n• ซื้อ 1-19 แท่ง → เคลมได้ภายใน 7 วัน\n• ซื้อ 20 แท่งขึ้นไป → ภายใน 14 วัน\n• ซื้อ 50 แท่งขึ้นไป → ภายใน 21 วัน\n• ซื้อ 100 แท่งขึ้นไป → ภายใน 30 วัน\n(นับจากวันที่ได้รับสินค้าค่ะ)\n\n📸 หลักฐานที่ต้องมีทุกครั้ง\n1) รูป/คลิปกล่องพัสดุ + ใบปะหน้าที่อยู่ ให้เห็นชัด\n2) คลิปตอนแกะกล่อง เห็นว่าได้รับอะไร กี่ชิ้น\n3) คลิปสินค้าที่มีปัญหา พร้อมอธิบายอาการ\n\n⚠️ ไม่มีคลิปตอนแกะกล่อง ทางร้านไม่สามารถเคลมให้ได้นะคะ 🙏🏻\nถ้าสินค้ามีปัญหา แจ้งได้เลยค่ะ เดี๋ยวทีมงานหลังการขายดูแลให้ทันทีค่ะ 💕";
...
function bestSellerGate(reply, ask) {
  ...
  return {
    ...
    reply: "ขออภัยค่ะ 🙏🏻 แอดมินไม่มีข้อมูลจัดอันดับยอดขายของร้านค่ะ\n\nแต่บอกงบที่ตั้งไว้ หรือแนวกลิ่นที่ชอบ (เช่น เย็น · ผลไม้ · องุ่น) มาได้เลยนะคะ เดี๋ยวแอดมินแนะนำรุ่นที่มีของจริงให้ค่ะ 💕"
  };
}
...
function fakePromoGate(reply) {
  ...
  return { reply: empty ? PROMO_TRUE_MSG : kept + "\n\n" + PROMO_TRUE_MSG, blocked: true, said: bad[0] };
}
...
const AMBIG_ASK_MSG = "ขออภัยค่ะ 🙏🏻 แอดมินยังไม่แน่ใจว่าคุณลูกค้าหมายถึงรุ่นไหนนะคะ\nรบกวนพิมพ์ชื่อรุ่นเต็ม ๆ อีกครั้งได้ไหมคะ เดี๋ยวเช็คให้ทันทีเลยค่ะ 💕\n\n(หรือขอดูเมนูสินค้าทั้งหมดก็ได้นะคะ พิมพ์ว่า \"เมนู\" ได้เลยค่ะ ✨)";
...
// 🎯 k216 · ตัวช่วยของกติกา "ชื่อยาวกว่าชนะ" (ใช้ใน _MODEL_IN)
//   ปรับรูปให้เทียบกันได้: ตัดวรรณยุกต์ · แปลง "20เค"→"20K" · ตัดวรรค/วงเล็บ/จุลภาค · ตัวใหญ่
function _normCat(s) {
  return _K2(foldTH(String(s || "")))
    .toUpperCase()
    .replace(/[\s\-_.,()+\/]/g, "");
}
// 🧩 k218 — "คำบอกประเภทที่ติดอยู่หน้าชื่อแบรนด์ = ลูกค้าบอกประเภทมาแล้ว"
//   หลักฐานจากแชทจริง 300 บท: ลูกค้าพิมพ์ "เครื่องvazerเข้ายังครับ"
//     ระบบตอบ VAZER RELOAD 15K (หัวน้ำยา 330) ทั้งที่ลูกค้าถามหา เครื่อง VAZER RELOAD (220)
//   ต้นเหตุ: ตารางชื่อเล่นเก็บ "ชื่อแบรนด์" อย่างเดียว ไม่ได้เก็บว่าลูกค้าขอประเภทไหน
//     คำว่า "เครื่อง" ที่ติดอยู่ข้างหน้าถูกทิ้งไปเฉย ๆ
//   ⛔ ต้องดู "ติดกัน" เท่านั้น ไม่ใช่ "มีคำนี้อยู่ในข้อความ"
//     "มาโบ1เครื่องเเถมหัวน้ำยาใช่ไหมคะ" — คำว่าหัวน้ำยาพูดถึงของแถม ไม่ใช่ตัวสินค้า
//   ⛔ ถ้าแบรนด์นั้นมีหลายตัวในประเภทที่ขอ = กำกวม ห้ามเดา ปล่อยไว้เหมือนเดิม
const CATQ = [
  [/(?:เครื่อง|ตัวเครื่อง)\s*$/, "device"],
  [/(?:ไส้บุหรี่|ไส้)\s*$/,       "iqos"],
  [/(?:หัวพอต|หัวน้ำยา|หัว)\s*$/, "head"],
];
function _brandCore(k) {
  return String(k).toUpperCase()
    .replace(/^(เครื่อง|หัวพอต|หัวน้ำยา|ไส้บุหรี่)\s*/, "")
    .replace(/\s*\((KIT|โคลน)\)$/i, "")
    .replace(/\b\d+(,\d+)?K?\b/gi, "")
    .replace(/[\s\-_.,()+\/]/g, "");
}
// ถ้าลูกค้าเขียนคำบอกประเภทติดหน้าชื่อแบรนด์ → เลือกตัวของแบรนด์เดียวกันที่อยู่ประเภทนั้น
function _catSibling(text, key, matched) {
  try {
    if (!key || !matched) return "";
    const raw = String(text || "");
    const i = raw.toUpperCase().indexOf(String(matched).toUpperCase());
    if (i <= 0) return "";
    const before = raw.slice(Math.max(0, i - 12), i);
    let want = "";
    for (const [re, w] of CATQ) if (re.test(before)) { want = w; break; }
    if (!want) return "";
    const have = topicOfModel(key);
    if (!have || topicSame(want, have)) return "";          // ประเภทตรงอยู่แล้ว ไม่ต้องแตะ
    const core = _brandCore(key);
    if (core.length < 4) return "";
    const isClone = CLONE_MARK_RE.test(raw);
    const sib = (MODEL_NAMES_LONGFIRST || []).filter(nm => {
      if (nm === key || _brandCore(nm) !== core) return false;
      if (!isClone && /\(โคลน\)/.test(nm)) return false;     // ไม่ได้บอกว่าโคลน = ไม่เอาตัวโคลน
      return topicSame(want, topicOfModel(nm));
    });
    if (sib.length === 1) { console.log("K218_CATQ " + want + " " + key + "→" + sib[0]); return sib[0]; }
    return "";                                              // 0 หรือมากกว่า 1 = ไม่ชัด ห้ามเดา
  } catch (e) { return ""; }
}
// 🔤🔤 k228 · PRODUCT RESOLUTION — อ่านชื่อแบรนด์ที่ลูกค้าเขียนเป็นภาษาไทย
//   ปัญหาที่แก้ (เคสจริง 7 ส.ค. 13.22-13.23):
//     ลูกค้าพิมพ์ "สตา" / "สตาร์" หมายถึง STAR 2,500 (สินค้าจริง 150 บาท 12 กลิ่น)
//     ระบบจับไม่ได้เลย → ตกไปให้ AI เดา → ตอบ SONIC บ้าง ABC LEGO บ้าง
//   Root Cause: การสร้างตัวเลือก (Candidate Generation) มีทางเดียวคือ
//     **ตารางชื่อเล่นที่มนุษย์เขียนเอง (TH_MODEL)** — STAR ไม่มีในตาราง จึงไม่มีตัวเลือกให้จัดอันดับเลย
//     ไม่ใช่ "จัดอันดับผิด" แต่คือ "ไม่มีอะไรให้จัดอันดับ"
//   ⛔ ไม่แก้ด้วยการเพิ่มคำว่า "สตา" — พรุ่งนี้มีสินค้าใหม่ก็หลุดอีก
//   ความสามารถที่เพิ่ม: ถอดเสียงไทยเป็นโครงพยัญชนะ แล้วเทียบกับ "ชื่อจริงในแคตตาล็อก" โดยตรง
//     สินค้าใหม่ที่ร้านเพิ่มเข้ามา ระบบจะอ่านชื่อไทยของมันได้เองทันที ไม่ต้องมาเขียนกฎเพิ่ม
const TH_CONS = { "ก": "K", "ข": "K", "ค": "K", "ฆ": "K", "ง": "NG", "จ": "C", "ฉ": "C", "ช": "C", "ซ": "S", "ฌ": "C", "ญ": "Y",
  "ฎ": "D", "ฏ": "T", "ฐ": "T", "ฑ": "T", "ฒ": "T", "ณ": "N", "ด": "D", "ต": "T", "ถ": "T", "ท": "T", "ธ": "T", "น": "N",
  "บ": "B", "ป": "P", "ผ": "P", "ฝ": "F", "พ": "P", "ฟ": "F", "ภ": "P", "ม": "M", "ย": "Y", "ร": "R", "ล": "L", "ว": "W",
  "ศ": "S", "ษ": "S", "ส": "S", "ห": "H", "ฬ": "L", "ฮ": "H" };
// โครงพยัญชนะจากข้อความไทย — ตัดสระ/วรรณยุกต์ทิ้ง เหลือแต่โครงเสียง
function thSkeleton(s) {
  let o = "";
  for (const ch of String(s || "")) { const c = TH_CONS[ch]; if (c) o += c; }
  return o.replace(/C/g, "K").replace(/V/g, "W").replace(/Y/g, "").replace(/(.)\1+/g, "$1");
}
// โครงพยัญชนะจากชื่อภาษาอังกฤษ — ใช้กติกาเดียวกันเพื่อเทียบกันได้
function latSkeleton(s) {
  return String(s || "").toUpperCase().replace(/[^A-Z]/g, "")
    .replace(/CK/g, "K").replace(/C/g, "K").replace(/J/g, "K").replace(/Z/g, "S").replace(/X/g, "KS").replace(/V/g, "W")
    .replace(/[AEIOUY]/g, "").replace(/(.)\1+/g, "$1");
}
let _SKEL_CACHE = null;
// หา "สินค้าที่ชื่อออกเสียงตรงกับที่ลูกค้าพิมพ์" — คืน "" ถ้าชี้ไม่ได้ตัวเดียว (Never Guess)
function thBrandMatch(text) {
  try {
    const raw = String(text || "");
    // เอาเฉพาะช่วงที่เป็นอักษรไทย แล้วตัดคำถาม/คำสุภาพที่ไม่ใช่ชื่อแบรนด์ออก
    const cleaned = raw
      .replace(/(มี|เหลือ|ขอ|เอา|ราคา|กลิ่น|รส|สี|อะไร|ไหน|บ้าง|มั้ย|ไหม|ป่าว|คะ|ค่ะ|ครับ|คับ|นะ|หน่อย|เท่า|ไหร่|กี่|บาท|ชิ้น|อัน|แท่ง|หัว|ตัว|เครื่อง|รุ่น|แบบ|ยี่ห้อ|แบรนด์|ดู|สั่ง|ซื้อ|จะ|ที่|ของ|ใช้|แล้ว|ทิ้ง|พอต|น้ำยา)/g, " ");
    const parts = cleaned.split(/[^ก-๙]+/).filter(x => x && x.length >= 2);
    if (!parts.length) return "";
    if (!_SKEL_CACHE) {
      _SKEL_CACHE = [];
      for (const nm of (MODEL_NAMES_LONGFIRST || [])) {
        const k = latSkeleton(nm);
        if (k.length >= 2) _SKEL_CACHE.push([k, nm]);
      }
    }
    for (const part of parts) {
      const k = thSkeleton(part);
      if (k.length < 2) continue;
      const hit = [];
      for (const [sk, nm] of _SKEL_CACHE) {
        if (sk.indexOf(k) !== 0) continue;
        // ⛔ ต้องครอบคลุมโครงเสียงของสินค้าอย่างน้อย 60% ไม่งั้นคำไทยทั่วไปจะไปตรงกับแบรนด์มั่ว
        //    เคสจริงที่เทสจับได้: "ขายไรบ้าง" → โครง KR → ไปตรงกับ CARNIVAL (KRNWLK) แค่ 33%
        if (k.length / sk.length < 0.6) continue;
        if (hit.indexOf(nm) === -1) hit.push(nm);
      }
      if (!hit.length) continue;
      // ⛔ ชี้ได้หลายตัว = ยังไม่ชัด ห้ามเดา (ตัวที่ชัดกว่ามีตารางชื่อเล่นจัดการอยู่แล้ว)
      const brands = [...new Set(hit.map(n => _brandCore(n)))];
      if (brands.length !== 1) { console.log("K228_TH_BRAND_AMBIG " + part + " n=" + brands.length); continue; }
      // แบรนด์เดียวแต่มีหลายชื่อในตาราง → เอาตัวที่ "มีข้อมูลกลิ่นจริง" ก่อน
      //   ⚠️ เคสจริง: มีทั้ง "STAR 2,500" (12 กลิ่น) และ "STAR 2500" (0 กลิ่น) อยู่ในตารางราคา
      //      ถ้าเลือกตัวหลัง ลูกค้าถามกลิ่นแล้วจะได้ลิสต์ว่าง = พังเงียบ
      const _fl = n => { try { return ((FLAVORS[n] && FLAVORS[n].f) || []).length; } catch (e) { return 0; } };
      hit.sort((a, b) => (_fl(b) - _fl(a)) || (a.length - b.length));
      console.log("K228_TH_BRAND " + part + " → " + hit[0]);
      return hit[0];
    }
    return "";
  } catch (e) { return ""; }
...
let _CAT_CACHE = null;
// หา "ชื่อจริงในแคตตาล็อก" ที่ยาวที่สุดซึ่งปรากฏในข้อความ และต้องยาวกว่าที่แพทเทิร์นจับได้
//   คืน "" ถ้าไม่มีตัวไหนยาวกว่า = ปล่อยให้ตารางชื่อเล่นตัดสินเหมือนเดิม
function _catalogLongest(text, minLen) {
  try {
    const n = _normCat(text);
    if (n.length < 5) return "";
    if (!_CAT_CACHE) {
      _CAT_CACHE = [];
      for (const nm of (MODEL_NAMES_LONGFIRST || [])) {
        const k = _normCat(nm);
        if (k.length >= 5) _CAT_CACHE.push([k, nm]);
      }
      _CAT_CACHE.sort((a, b) => b[0].length - a[0].length);   // ยาวก่อนสั้น
    }
    for (const [k, nm] of _CAT_CACHE) {
      if (k.length <= (minLen || 0)) break;                   // สั้นกว่าที่แพทเทิร์นจับได้ = ไม่ต้องดูต่อ
      if (n.indexOf(k) !== -1) return nm;
    }
  } catch (e) {}
  return "";
}
const _MODEL_IN = (s) => {
  ...
  // 🎯🎯 k216 — "ชื่อที่ยาวกว่าและเจาะจงกว่า ชนะ"
  //   ปัญหาที่แก้ (วัดจริง 14 รุ่นจาก 65 จับตัวเองไม่ถูก):
  //     ลูกค้าพิมพ์ "เครื่อง IQOS ILUMA I PRIME" (5,200 บาท) → ระบบตอบ "ไส้บุหรี่ IQOS JP" (2,150)
  //     ลูกค้าพิมพ์ "เครื่อง RELX INFINITY 2+" (990)        → ระบบตอบ "หัวพอต RELX INFINITY" (140)
  //     = ได้ของผิดประเภท ไม่ใช่แค่ราคาเพี้ยน
  //   ต้นเหตุ: TH_MODEL เป็นตารางชื่อเล่น/คำย่อ เรียงลำดับ **ใครตรงก่อนชนะ**
  //     รุ่นย่อย (INDO / MALAY / ILUMA / (KIT) / ESKOLIQ) ไม่มีแพทเทิร์นของตัวเอง
  //     แพทเทิร์นกว้างของรุ่นหลัก (/iqos/ · /relx infinity/) จึงคว้าไปก่อนเสมอ
  //   ⛔ แก้ที่กติกา ไม่ใช่ไล่เพิ่มแพทเทิร์นทีละรุ่น (ทำแบบนั้นพอมีรุ่นใหม่ก็หลุดอีก)
  //   กติกา: ถ้าในข้อความมี "ชื่อจริงในแคตตาล็อก" ที่ยาวกว่าสิ่งที่แพทเทิร์นจับได้ → ชื่อจริงชนะ
  //   ⚠️ ต้องเทียบ "ความยาว" ไม่ใช่แค่ "เจอ" ไม่งั้นกฎ k171 พัง:
  //      "หัว MARBO 9K" → แพทเทิร์นจับ "หัวMARBO" (8) · แคตตาล็อกจับ "MARBO9K" (7)
  //      7 < 8 → แพทเทิร์นชนะ = ยังเป็น M SWITCH ถูกต้องเหมือนเดิม
  let _hitKey = "", _hitLen = 0, _hitTxt = "";
  for (const [re, key] of TH_MODEL) {
    let m = null;
    for (const c of [raw, nosp, rk, nk, fr, fn, fk, fkn]) {
      const r = new RegExp(re.source, re.flags.replace("g", ""));
      const mm = r.exec(c);
      if (mm) { m = mm; break; }
    }
    if (m) { _hitKey = key; _hitLen = _normCat(m[0]).length; _hitTxt = m[0]; break; }
  }
  const _specific = _catalogLongest(raw, _hitLen);
  if (_specific) return cloneSwap(_specific, raw);
  if (_hitKey) {
    // k218: ลูกค้าเขียนคำบอกประเภทติดหน้าชื่อแบรนด์ ("เครื่องvazer") → เลือกตัวในประเภทนั้น
    const _sib = _catSibling(raw, _hitKey, _hitTxt);
    return cloneSwap(_sib || _hitKey, raw);
  }
  // 🔤 k228: ตัวถอดเสียงไทย (thBrandMatch) สร้างไว้แล้วแต่ **ยังไม่ต่อสาย**
  //   เหตุผล: ทดสอบแล้วทำให้ Gold120 Wrong Product Critical พุ่งจาก 0 → 8
  //     และ Product Eval ตกจาก 200 → 197 คือสร้างบั๊ก "ส่งของผิดรุ่น" ขึ้นมาใหม่
  //   ⛔ ห้ามต่อสายจนกว่าจะแก้ให้ WPC = 0 ได้ — ดูรายงาน k228
  const tn = normTH(raw);
...
// 🧾🧾 k157 (เคสจริง 4/8 00.03-00.07 — ลูกค้า m · ยืนยันด้วยแคป LINE):
//   ลูกค้าบอกครบ 4 รอบ แชร์หมุดแล้ว คิดค่าส่งด่วน 66 บาทแล้ว แต่ **การ์ดไม่เคยออกเลย**
//     บอท 0.03: "MARBO 9K มีกลิ่นองุ่นว่านหางจระเข้ค่ะ รับกี่ชิ้นดีคะ"
//     ลูกค้า 0.05: "1ชิ้น ส่งแกรปครับ"      ← ครบแล้ว (รุ่น+กลิ่นอยู่ในเทิร์นก่อน · จำนวน=1)
//     บอท 0.06: "รบกวนแจ้งกลิ่นที่ต้องการของ MARBO 9K + จำนวน มาได้เลยค่ะ"  ← ถามซ้ำ
//     ลูกค้า 0.06: "มาโบว่านท่าง 1ชิ้น" → บอทถามซ้ำ **ข้อความเดิมเป๊ะ** อีกรอบ
//     ลูกค้า 0.07: "มาโบว่านหาง 1 ชิ้น ส่งแกรป" → ยังไม่ออกการ์ดอีก
//   ต้นเหตุ: k154 ล็อกกลิ่นได้เฉพาะเมื่อ "รุ่น+กลิ่น อยู่ในข้อความเดียวกัน"
//     แต่คนไทยพิมพ์กระจายหลายข้อความ — รุ่นกับกลิ่นอยู่เทิร์นก่อน เหลือแค่จำนวนในเทิร์นนี้
//     มี carryModel (k55) พาชื่อรุ่นข้ามเทิร์นอยู่แล้ว แต่ **ไม่มีตัวพากลิ่นข้ามเทิร์น**
//   แก้: พากลิ่นข้ามเทิร์นคู่กับรุ่น → ครบ 3 อย่างเมื่อไหร่ k154 ออกการ์ดให้เอง
//   ⚠️ ห้ามทับกลิ่นที่ลูกค้าเพิ่งเปลี่ยนเอง — ถ้าลูกค้าเอ่ยชื่อกลิ่นมาในข้อความนี้ ให้ถอยทันที
// 🧾🧾 k160 (เจอตอนไล่โฟลว์ออเดอร์ 4 ส.ค. — ยืนยันด้วยการรันโค้ดจริง):
//   ลูกค้าพิมพ์ "เอามาโบ องุ่น 1 แท่ง ส่งแกรป" = สั่งของครบทุกอย่าง + บอกวิธีส่ง
//   → ทางลัดส่งด่วนจับคำว่า "แกรป" แล้ว return ทันที = **ออเดอร์หายทั้งดุ้น** (ord ไม่ถูกบันทึกเลย)
//   → พอแอดมินกรอกค่าส่งใน /expfee ระบบหาออเดอร์ไม่เจอ (ต้องมี status "รอโอน 💰")
//     ลูกค้าเลยได้แค่ "ค่าส่งด่วนคือ X บาท" ไม่มีการ์ด ต้องพิมพ์สั่งใหม่ทั้งหมด
//   = อาการเดียวกับเคสจริง 4/8 00.05-00.07 (แชร์หมุดแล้ว คิดค่าส่งแล้ว แต่วนถามรุ่น/กลิ่น/จำนวนใหม่)
//   ⚠️ โค้ดที่รับออเดอร์ส่งด่วนถูกต้องมีอยู่แล้ว (k102) แต่ถูกทางลัดตัดหน้า ไม่เคยได้ทำงาน
//   กฎใหม่: ถ้าข้อความนั้น "เป็นการสั่งของ" ด้วย → ห้ามใช้ทางลัด ให้ไหลไปทางรับออเดอร์ปกติ
//   ⚠️ ต้องตัดตัวเลขที่เป็นส่วนหนึ่งของชื่อรุ่นก่อนนับจำนวน (บทเรียน k154: "มาโบ9k" ไม่ใช่ 9 ชิ้น)
function looksLikeOrderText(t) {
  try {
    const s = String(t || "");
    const sq = s.replace(/\d+\s*(k|เค)/gi, " ").replace(/\d[\d,]{2,}/g, " ");
    const hasQty = /\d{1,3}\s*(ชิ้น|อัน|แท่ง|หัว|ตัว|กล่อง|ลัง|คอต|ตลับ)/.test(sq)
                || /(เอา|ขอ|สั่ง|รับ)[^\n]{0,20}?\d{1,3}/.test(sq);
    if (!hasQty) return false;
    if (_MODEL_IN(s)) return true;
    const tn = normTH(s);
    for (const k in FLAVORS) {
      for (const f of (FLAVORS[k].f || [])) {
        const nb = normTH(String(f).replace(/\s*\d+(\.\d+)?%\s*$/, "").trim());
        if (nb.length >= 4 && tn.indexOf(nb) !== -1) return true;
      }
    }
    return false;
  } catch (e) { return false; }
}
function carryFlavor(text, hist) {
  if (!Array.isArray(hist) || !hist.length) return "";
  const last = hist[hist.length - 1];
  if (!last || last.role !== "assistant") return "";
  const c = String(last.content || "");
  // เอาเฉพาะตอนบอท "เพิ่งยืนยันว่ากลิ่นนี้มีของ" — ห้ามหยิบจากประโยคที่บอกว่าหมด (กฎเดียวกับ k69/k150)
  if (/หมดชั่วคราว|หมดทุกกลิ่น|ของหมด|หมดสต็อก/.test(c)) return "";
  ...
  const bareOf = f => String(f).replace(/\s*\d+(\.\d+)?%\s*$/, "").trim();
  const fs = [];
  for (const f of v.f) { const b = bareOf(f); if (b.length >= 4 && c.indexOf(b) !== -1 && fs.indexOf(b) === -1) fs.push(b); }
  const longest = fs.filter(a => !fs.some(b => b !== a && b.length > a.length && b.indexOf(a) === 0));
  if (longest.length !== 1) return "";            // บอทลิสต์หลายกลิ่น = ลูกค้ายังไม่ได้เลือก
  const tn = normTH(text);                        // ลูกค้าเอ่ยกลิ่นเองในข้อความนี้ไหม → ถ้าเอ่ย ห้ามทับ
  for (const f of v.f) {
    const nb = normTH(bareOf(f));
    if (nb.length >= 4 && tn.indexOf(nb.slice(0, Math.min(nb.length, 6))) !== -1) return "";
  }
  // พา "รุ่น + กลิ่น" มาคู่กันเสมอ — carryModel (k55) ต้องการให้ข้อความมีคำถามเชิงถามของ
  //   แต่ "1ชิ้น ส่งแกรปครับ" ไม่เข้าเงื่อนไขนั้น เลยได้กลิ่นมาแต่ไม่มีรุ่น = ล็อกไม่ได้อยู่ดี
  return " " + ms[0] + " " + longest[0];
}
function carryModel(text, hist) {
  try {
    ...
    // 🎯 k150 (เคสจริง 3/8 22.50 — ลูกค้าหลุดมือคาแชท):
    //   ลูกค้า: "สตา" → บอท: "STAR 2,500 ยังมีของพร้อมส่ง มี 12 กลิ่น 150 บาท"
    //   ลูกค้า: "เหลือกลิ่นไหน" → บอท: "หัวพอต INFY PLUS หมดชั่วคราวทุกกลิ่น" (คนละรุ่น คนละเรื่อง)
    //   ลูกค้า: "เอาองุ่นว่าน 1" (สั่งของแล้ว!) → บอทตอบ INFY PLUS หมด อีกรอบ = ออเดอร์หลุด
    //   ต้นเหตุ: ลูป k69 ไล่หา "รุ่นที่ลูกค้าเอ่ย" ย้อนหลัง 8 เทิร์นแบบไม่สนใจว่าเก่าแค่ไหน
    //     "สตา" เป็นคำย่อที่ _MODEL_IN จับไม่ได้ → ข้ามไป → ไปเจอ "ชาจี อินฟี่" ที่เก่ากว่า 3 เทิร์น
    //     → แบก INFY มาตอบ ทั้งที่บอทเพิ่งตอบเรื่อง STAR ไปเมื่อเทิร์นที่แล้ว
    //   กฎใหม่: ถ้าบอท "เพิ่งตอบยืนยันว่ารุ่นนี้มีของ" ในเทิร์นล่าสุด และลูกค้าถามต่อแบบไม่ระบุรุ่น
    //     → ให้ยึดรุ่นนั้น (ลูกค้ากำลังถามต่อเรื่องรุ่นที่คุยค้างอยู่ ไม่ใช่รุ่นที่เอ่ยไปเมื่อ 3 เทิร์นก่อน)
    //   ⚠️ ยังคงกฎ k69 ไว้: ห้ามหยิบรุ่นจากข้อความบอทที่ "ขอโทษว่าของหมด" (นั่นคือรุ่นที่ลูกค้าไม่ได้จะเอา)
    {
      const _last = Array.isArray(hist) && hist.length ? hist[hist.length - 1] : null;
      if (_last && _last.role === "assistant") {
        const _c = String(_last.content || "");
        const _neg = /หมดชั่วคราว|หมดทุกกลิ่น|หมดสต็อก|ไม่มีของ/.test(_c);
        const _pos = /ยังมีของ|มีของพร้อมส่ง|มีพร้อมส่ง|กลิ่นที่มี|รับกลิ่นไหน|มีค่ะ|มีนะคะ/.test(_c);
        if (!_neg && _pos) {
          const _ms = [];
...
// 🔀🔀 k216 — Topic Switch: ลูกค้าเปลี่ยน "หมวดสินค้า" ทั้งที่ไม่ได้เอ่ยชื่อรุ่น
//   เคสจริง (ภาพหน้าจอ 6 ส.ค. 22.16-22.18 ร้าน v20):
//     22.17 ลูกค้า "บูส"                      → ระบบล็อก RELX BOOST POD (หมวด บิ๊กพอต)
//     22.18 ลูกค้า "ใช้แล้วทิ้งหละเหลืออะไรบ้าง"  → บอทลิสต์กลิ่นของ RELX BOOST POD ต่อ
//   ต้นเหตุ: slotCarry ตัดสินแค่ 2 อย่าง "เทิร์นนี้มีชื่อรุ่นไหม" กับ "มีคำอ้างอิงไหม"
//     ข้อความที่บอก **หมวด** แต่ไม่บอก **รุ่น** จึงถูกมองว่า "ไม่มีข้อมูลสินค้าใหม่" → ลากของเดิมมา
//   ⚠️ ระบบรู้จักหมวดอยู่แล้ว แต่รู้อยู่ที่เดียวคือใน styleHint (บรรทัด ~2315)
//     ซึ่งทำงานเฉพาะตอนลูกค้าพูดถึง "แนวกลิ่น" (เย็น/หวาน/กาแฟ) เท่านั้น
//     เส้นทางที่ลาก context ไม่เคยเห็นข้อมูลหมวดเลยแม้แต่ครั้งเดียว
//   กฎใหม่ (กฎกลาง ไม่ผูกรุ่นใดรุ่นหนึ่ง ไม่มี if รายเคส):
//     ถ้าเทิร์นนี้ระบุหมวดสินค้าชัดเจน และเป็นคนละหมวดกับของที่ค้างอยู่ → **ห้ามลากของเดิมมา**
//     คืน null เพื่อให้ระบบตั้งต้นใหม่ตามหัวข้อล่าสุด
//   ⛔ ไม่แตะ catOf() เพราะตัวนั้นเป็นตัวจัดกลุ่ม "โปรส่งฟรี" คนละเรื่องกับหัวข้อสนทนา
//      (catOf รวม น้ำยาขวด + นิโคตินพอช + เครื่อง IQOS เป็น "other" ก้อนเดียว ใช้แยกหัวข้อไม่ได้)
const TOPIC_WORDS = [
  ["disp",     /ใช้แล้วทิ้ง|ใช้แล้วทิ้ง|สูบทิ้ง|สูบละทิ้ง|พอตทิ้ง|แบบทิ้ง|ตัวทิ้ง|แท่งทิ้ง/],
  ["smallpod", /หัวเล็ก|หัวพอตเล็ก|หัวน้ำยาเล็ก|พอตหัวเล็ก/],
  ["bigpod",   /บิ๊กพอต|big\s?pod|หัวใหญ่|หัวน้ำยาใหญ่|ชุด\s?kit|เลโก้/i],
  // ⚠️ "หัวพอต / หัวน้ำยา / หัวเติม" ลอย ๆ = หัวเปลี่ยน แต่บอกไม่ได้ว่าเล็กหรือใหญ่
  //    จึงเป็น "หมวดใหญ่" ที่ครอบทั้ง bigpod และ smallpod — ห้ามฟันธงเป็นอันใดอันหนึ่ง
  ["head",     /หัวพอต|หัวน้ำยา|หัวเติม|หัวเปลี่ยน|พอตหัว/],
  ["liquid",   /น้ำยาขวด|ขวดน้ำยา|ซอลนิค|saltnic|freebase|ฟรีเบส|น้ำยา\s?\d{2}\s?ml/i],
  ["pouch",    /นิโคตินพอช|pouch|ซอง\s?นิโคติน/i],
  ["iqos",     /ไส้บุหรี่|iqos|ไอคอส/i],
  ["device",   /เครื่อง(?!ละ)/],
];
// ⚠️ ต้องตัด "จำนวน + หน่วยนับ" ทิ้งก่อนอ่านหมวด ไม่งั้น "เอา6เครื่องก่อนครับ" (Gold120 A09)
//    จะถูกอ่านว่าลูกค้าเปลี่ยนหัวข้อไปคุยเรื่องเครื่อง แล้วทิ้งสินค้าที่คุยค้างไว้
//    บทเรียนเดียวกับ fmQty: **เลขกับหน่วยนับไม่ใช่การบอกหมวดสินค้า**
const TOPIC_STRIP_RE = /\d{1,3}\s*(?:ชิ้น|อัน|แท่ง|หัว|ตัว|กล่อง|ลัง|เครื่อง|ชุด|ห่อ|ซอง|แพ็?ค)/g;
// หมวดของ "สินค้าที่ค้างอยู่ในความจำ" — แยกตามหัวข้อสนทนา ไม่ใช่ตามโปรส่งฟรี
function topicOfModel(key) {
  const k = String(key || "");
  if (!k) return "";
  if (/ไส้บุหรี่/.test(k)) return "iqos";
  if (/POUCH/i.test(k)) return "pouch";
  if (/SALTNIC|FREEBASE/i.test(k)) return "liquid";
  if (/^เครื่อง/.test(k)) return "device";
  if (/\(KIT\)/.test(k)) return "bigpod";
  const bare = k.replace(/^(หัวพอต|หัวน้ำยา)\s*/, "").trim();
  if (typeof BIGPOD !== "undefined" && (BIGPOD.indexOf(k) !== -1 || BIGPOD.indexOf(bare) !== -1)) return "bigpod";
  if (typeof SMALLPOD !== "undefined" && (SMALLPOD.indexOf(k) !== -1 || SMALLPOD.indexOf(bare) !== -1)) return "smallpod";
  if (/^หัวพอต/.test(k)) return "smallpod";
  return "disp";
}
// หมวดที่ "ข้อความของลูกค้า" ชี้ถึง — คืน "" ถ้าไม่ได้ระบุหมวด (ห้ามเดา)
//   ⚠️ ถ้าชี้ได้มากกว่า 1 หมวด = กำกวม ให้ถือว่าไม่ได้ระบุ (Never Guess)
function topicInText(text) {
  const t = String(text || "").replace(TOPIC_STRIP_RE, " ");
  if (!t.trim()) return "";
  const hit = [];
  for (const [name, re] of TOPIC_WORDS) if (re.test(t) && hit.indexOf(name) === -1) hit.push(name);
  // "หัวพอตเล็ก" เข้าทั้ง smallpod และ head — ไม่ใช่กำกวม แต่คือตัวเจาะจงกับตัวกว้าง "ของเรื่องเดียวกัน"
  //   เอาตัวเจาะจงชนะตัวกว้าง — หลักเดียวกับ "วลียาวกว่าชนะ"
  //   ⛔ ยุบได้เฉพาะคู่ head+bigpod / head+smallpod เท่านั้น
  //      คู่อื่นเช่น disp+head คือลูกค้าพูดถึง 2 หมวดจริง = เปรียบเทียบ ห้ามฟันธง
  if (hit.length === 2 && hit.indexOf("head") !== -1) {
    const other = hit.find(x => x !== "head");
    if (other === "bigpod" || other === "smallpod") return other;
    return "";
  }
  return hit.length === 1 ? hit[0] : "";
}
// หมวดในข้อความ กับ หมวดของสินค้าที่ค้างอยู่ ถือว่า "เรื่องเดียวกัน" ไหม
//   head = หมวดใหญ่ ครอบทั้ง bigpod และ smallpod → ห้ามนับว่าเปลี่ยนเรื่อง
function topicSame(want, have) {
  if (!want || !have) return true;
  if (want === have) return true;
  if (want === "head" && (have === "bigpod" || have === "smallpod")) return true;
  return false;
}
// 🚫🚫🚫 k222 · AMBIGUITY DETECTOR — ด่านเดียวที่ตอบว่า "ข้อความนี้ตีความได้กี่แบบ"
//   กฎสูงสุดของระบบ: Never Guess > Always Ask
//   คืนค่า "" = ชัดเจน ตอบได้  ·  คืนข้อความ = กำกวม ต้องถามกลับก่อน
//   ⛔ ตัวนี้ "อ่านอย่างเดียว" ไม่แก้ Slot ไม่แก้ออเดอร์ ไม่เปลี่ยนเส้นทางเงิน
//   ⛔ ไม่ได้เขียนกฎใหม่ — รวมกฎที่มีอยู่แล้วให้ตอบเป็นคำตอบเดียว
//      (slotResolveRef ambiguous · slotCarry null · topicInText ว่าง · slotStale)
function ambiguityOf(text, s, now) {
  try {
    const t = String(text || "");
    if (!t.trim()) return "";
    const items = (slotOk(s) ? s.items : []).filter(x => x && x.model);
    const hasRef = /(ตัว|อัน|รุ่น|กลิ่น|สี)\s*(นี้|นั้น|นี่|นั่น|เดิม)|เมื่อกี้|เมื่อกี๊|อันเนี้ย|ตัวเนี้ย/.test(t);
    let inMsg = ""; try { inMsg = _MODEL_IN(t) || ""; } catch (e) {}
    if (inMsg) return "";                                        // บอกชื่อรุ่นมาเอง = ชัดเจน

    // 1) อ้างถึงของเดิม แต่ยังไม่เคยคุยสินค้าไหนเลย
    if (hasRef && !items.length) return "อ้างถึงสินค้าเดิม แต่ยังไม่เคยคุยรุ่นไหน";

    // 2) อ้างถึงของเดิม แต่ความจำหมดอายุแล้ว (เงียบไปเกิน 30 นาที)
    if (hasRef && slotStale(s, now)) return "อ้างถึงสินค้าเดิม แต่คุยกันไว้นานเกิน 30 นาที";

    // 3) ตัวแปลคำอ้างอิงบอกเองว่ากำกวม
    try {
      const r = slotResolveRef(t, s);
      if (r && r.ambiguous && r.ambiguous.length) return "ชี้ได้หลายตัว: " + r.ambiguous.slice(0, 4).join(" / ");
      if (r && r.empty) return "อ้างถึงตำแหน่งที่ไม่มีอยู่จริงในรายการ";
    } catch (e) {}

    // 4) ไม่มีชื่อรุ่น ไม่มีคำอ้างอิง แต่ถามถึงคุณสมบัติของสินค้า (สี/กลิ่น/ราคา/มีไหม)
    //    → ไม่มีสิ่งอ้างอิงว่าหมายถึงสินค้าอะไร
    const asksAttr = /^(?=.*(สี|กลิ่น|รส|ราคา|เท่าไห?ร่|กี่บาท|มีไหม|มีมั้ย|มีป่าว|มีปะ|เหลือ))/.test(t) && t.replace(/[\s!?.ๆ]/g, "").length <= 24;
    if (asksAttr && !items.length && !hasRef) return "ถามถึงสินค้า แต่ยังไม่รู้ว่าหมายถึงรุ่นไหน";

    // 4b) 🚫 k227 — "ลูกค้าเรียกชื่ออะไรสักอย่างที่ระบบไม่รู้จัก"
    //   เคสจริง 13.23: "สตา" (= STAR 2,500) ไม่มีในตารางชื่อเล่น ระบบจับไม่ได้
    //   ⛔ กฎนี้ไม่ได้ผูกกับคำว่า "สตา" — ครอบทุกคำที่ระบบไม่รู้จัก รวมถึงสินค้าใหม่ในอนาคต
    //   เกณฑ์: ข้อความสั้น · ไม่มีรุ่น/กลิ่น/หมวด/คำสั่งใด ๆ ที่ระบบรู้จัก · ไม่ใช่คำทักทาย
    //   ⚠️ ผลของการยิงกฎนี้คือ "เติมคำสั่งห้ามเดา" เท่านั้น ไม่ได้บล็อกคำตอบทันที
    //      ด่านขาออกจะบล็อกก็ต่อเมื่อ AI ดันเอ่ยชื่อรุ่นออกมาจริง ๆ → ยิงเกินไม่เสียหาย
    {
      const bare = t.replace(/[\s!?.,ๆ๏]/g, "").replace(FM_POLITE_RE, "");
      const isSocial = /^(สวัสดี|หวัดดี|ดีครับ|ดีค่ะ|ขอบ(คุณ|ใจ)|โอเค|ok|okay|ได้|จ้า|ครับ|ค่ะ|คะ|คับ|อือ|555+|ยินดี|ไม่เป็นไร|รอ|โทษ|ขอโทษ)/i.test(bare);
      // ⛔ ถ้าระบบยัง "พาสินค้าที่คุยค้างไว้" มาได้ = รู้อยู่แล้วว่าคุยเรื่องอะไร ไม่ใช่ความกำกวม
      //    เช่น คุย MARBO 9K อยู่ แล้วถาม "เหลือกลิ่นไหนบ้าง" — ตอบได้ ไม่ต้องถามซ้ำ
      let _canCarry = false;
      try { const c = slotCarry(s, t, now); _canCarry = !!(c && c.model); } catch (e) {}
      if (!_canCarry && !isSocial && bare.length >= 2 && bare.length <= 22 && /[ก-๙a-zA-Z]/.test(bare)) {
        let known = false;
        try { if (topicInText(t)) known = true; } catch (e) {}
        if (!known) { try { if ((flavorHint(t, {}, 1) || "").trim()) known = true; } catch (e) {} }
        if (!known) { try { if ((brandHint(t, {}, 1) || "").trim()) known = true; } catch (e) {} }
        if (!known) { try { if (afterSaleIntent(t) || askTotalIntent(t) || browseKind(t)) known = true; } catch (e) {} }
        // คำสั่ง/คำถามทั่วไปที่มีเส้นทางของตัวเองอยู่แล้ว
        if (!known && /เมนู|โปร|ส่งฟรี|ค่าส่ง|จัดส่ง|กี่วัน|บัญชี|โอน|สลิป|ที่อยู่|แอดมิน|ยกเลิก|เคลม|พัสดุ|สถานะ|เท่าไห?ร่|กี่บาท|ราคา/.test(t)) known = true;
        if (!known) return "ลูกค้าเอ่ยถึงบางอย่างที่ระบบไม่รู้จัก";
      }
    }

    // 5) พาของเดิมไม่ได้เพราะกำกวม (กฎ k211 เดิม)
    if (!hasRef && items.length >= 2) {
      let carried = null; try { carried = slotCarry(s, t, now); } catch (e) {}
      if (!carried && !topicInText(t)) return "เพิ่งคุยหลายรุ่นในช่วงเดียวกัน ยังชี้ไม่ได้ว่าหมายถึงตัวไหน";
    }
    return "";
  } catch (e) { return ""; }
}
// 🔎🔎 k225 — "ขอดูตัวอื่น / มีอะไรบ้าง" = ลูกค้าขอ **รายการ** ไม่ได้ถามถึงตัวที่คุยอยู่
//   เคสจริง 7 ส.ค. 01.48-01.49 (ร้าน v20):
//     ลูกค้า "สูบทิ้งมีอะไรบ้าง"  → บอทตอบ MARBO 9K
//     ลูกค้า "มีแค่มาโบหรอ"      → บอทตอบ MARBO 9K อีก
//     ลูกค้า "มีแบรนด์อื่นมั้ย"   → บอทตอบ MARBO 9K อีกเป็นรอบที่ 3 → ลูกค้าหายไป
//   ต้นเหตุ 2 ชั้น
//     ชั้น 1: slotCarry ลากรุ่นเดิมมาต่อ — ตัวกัน k216 กันเฉพาะตอน "เปลี่ยนหมวด"
//             แต่เคสนี้ลูกค้าอยู่หมวดเดิม แค่ขอดูตัวอื่น จึงไม่มีกฎไหนกันเลย
//     ชั้น 2: ไม่มีใครส่ง "รายชื่อรุ่นในหมวดนั้น" ให้ AI → AI มีข้อมูลอยู่ตัวเดียวก็ตอบตัวเดิมวนไป
//   ⛔ ต้องแยกจาก "ถามถึงตัวที่คุยอยู่" ให้ขาด — "เหลือกลิ่นไหนบ้าง" ไม่ใช่การขอดูตัวอื่น
const BROWSE_OTHER_RE = /(แบรนด์|ยี่ห้อ|รุ่น|ตัว|อัน|แบบ|ยี่ห้อ)\s*(อื่น|ไหนอีก|อะไรอีก)|อย่างอื่น|นอกจาก(นี้|นั้น)|มีแค่[^\n]{0,14}(หรอ|เหรอ|หรือ|ป่าว|มั้ย|ไหม)|(แนะนำ|เสนอ)(ตัว|รุ่น|อัน)?อื่น/;
const BROWSE_LIST_RE = /(มีอะไรบ้าง|มีอะไรมั่ง|มีไรบ้าง|มีรุ่นไหนบ้าง|มีตัวไหนบ้าง|มียี่ห้อไหนบ้าง|มีแบรนด์ไหนบ้าง|ขายอะไรบ้าง|มีอะไรขาย)/;
// ถามหา "รายการ" ไหม — คืน "other" (ขอตัวอื่น) · "list" (ขอรายการในหมวด) · "" (ไม่ใช่)
function browseKind(text) {
  const t = String(text || "");
  if (!t) return "";
  if (BROWSE_OTHER_RE.test(t)) return "other";
  if (BROWSE_LIST_RE.test(t)) return "list";
  return "";
}
function slotCarry(s, text, now) {
  const cur = slotCurrent(s);
  if (!cur || !cur.model) return null;
  const t = String(text || "");
  // ⏱️ k219 ด่านที่ 0: เงียบไปเกิน 30 นาที = ถือเป็นบทสนทนาใหม่
  //   ⛔ แม้ลูกค้าจะพิมพ์ "ตัวเดิม" ก็ยังไม่พาของเก่ามาเอง — คืน null ให้ระบบถามยืนยันก่อน
  //      (ข้อกำหนด: ถ้าไม่แน่ใจว่าอ้างถึงของเดิมหรือไม่ ห้ามเดา)
  if (slotStale(s, now)) {
    console.log("K219_STALE_CTX backref=" + (BACKREF_RE.test(t) ? 1 : 0));
    return null;
  }
  // 🔎 k225 ด่านที่ 0.5: ลูกค้าขอ "ตัวอื่น" ชัดเจน → ห้ามลากตัวเดิมมาตอบซ้ำ
  //   ⛔ เฉพาะ "ขอตัวอื่น" เท่านั้น — "มีอะไรบ้าง" ยังพาได้ เพราะอาจถามถึงกลิ่นของตัวที่คุยอยู่
  if (browseKind(t) === "other") {
    console.log("K225_BROWSE_OTHER drop=" + cur.model);
    return null;
  }
  // 🔀 k216 ด่านแรก: เปลี่ยนหมวดสินค้า = เปลี่ยนหัวข้อ → ห้ามลากของเดิมมา
  //   วางไว้ก่อนทุกด่าน เพราะถึงเทิร์นนี้จะเอ่ยชื่อรุ่นด้วย ถ้าคนละหมวดก็ยังต้องใช้ของในเทิร์นนี้อยู่ดี
  try {
    const want = topicInText(t), have = topicOfModel(cur.model);
    if (want && have && !topicSame(want, have)) {
      console.log("K216_TOPIC_SWITCH from=" + have + " to=" + want);
      return null;
    }
  } catch (e) {}
...
function slotResolveRef(text, s) {
  try {
    ...
    for (const r of SLOT_REF) {
      ...
      // 🚫🚫 k222 · Never Guess — คำชี้ที่ "ไม่ได้ระบุตำแหน่ง" ใช้ได้เฉพาะเมื่อชี้ได้ตัวเดียว
      //   "อันนี้" / "ตัวเดิม" ไม่ได้บอกว่าอันไหน — ถ้าเพิ่งคุยกัน 2 รุ่นในช่วงเวลาเดียวกัน
      //   การหยิบ "ตัวล่าสุด" หรือ "ตัวก่อนหน้า" มาให้ = **เดา** ไม่ใช่การเข้าใจ
      //   ⚠️ เดิม slotCarry มีกฎนี้แล้ว (k211) แต่ slotResolveRef ไม่มี → สองที่ตอบไม่ตรงกัน
      //      "เอา 2" → slotCarry บอกกำกวม (ถูก) แต่ "เอาอันนี้" → slotResolveRef ชี้เลย (ผิด)
      //   ⛔ คำชี้ที่ระบุตำแหน่งชัด (อันแรก · อันที่ 2 · อันซ้าย · อันสุดท้าย) ไม่เข้าข่ายนี้
      //      เพราะลูกค้าบอกตำแหน่งมาเองแล้ว = ไม่กำกวม
      if (r.kind === "current" || r.kind === "prev") {
        const recent = [];
        let newest = 0;
        for (const x of items) if (x && (x.t || 0) > newest) newest = x.t || 0;
        for (const x of items) {
          if (!x || !x.model) continue;
          if (newest && (newest - (x.t || 0)) > SLOT_CARRY_GAP_MS) continue;   // คุยคนละช่วง = ไม่นับ
          if (recent.indexOf(x.model) === -1) recent.push(x.model);
        }
        // 🚫 "ตัวเดิม / กลับไปตัวเดิม / รุ่นเดิม"
        //    มี 2 รุ่น → "เดิม" = อีกตัวหนึ่ง = ชัดเจน ไม่ต้องถาม
        //    มี 3 รุ่นขึ้นไปในช่วงเดียวกัน → "เดิม" ไม่ได้บอกว่าย้อนไปตัวไหน = ต้องถาม
        if (r.kind === "prev" && recent.length >= 3) {
          console.log("K222_REF_AMBIG prev n=" + recent.length);
          return { item: null, idx: -1, ref: r.kind, ambiguous: recent.slice(0, 6) };
        }
        // ⚠️ "อันนี้ / ตัวนี้" = ตัวที่เพิ่งพูดถึงล่าสุด — เป็นวิธีชี้ปกติของภาษาไทย ไม่ใช่การเดา
        //    เคสที่กำกวมจริงคือ "บอทลิสต์ 2 รุ่นในข้อความเดียวแล้วลูกค้าตอบว่าอันนี้"
        //    ⛔ Slot ปัจจุบันแยกไม่ออกว่า 2 รุ่นมาจากข้อความเดียวกันหรือคนละข้อความ
        //       จึงยังไม่กันเคสนี้ในรอบนี้ — บันทึกไว้ในรายงานข้อ 7 ว่าเป็นจุดที่ยังเดาอยู่
      }
...
// 📍 k58: แอดมินต้องรู้ว่าลูกค้าอยู่ในเขตส่งด่วนไหม
// เคสจริง 1/8: ลูกค้าพิมพ์ "ผมอยู่หลักสี่ ผมสั่งซื้อวันนี้จะได้วันไหน"
//   แอดมินไม่รู้ว่า "หลักสี่" = กทม. เลยตอบเรื่องค่าส่ง/โปรแทนที่จะตอบว่าได้วันไหน = ลูกค้าไม่ได้คำตอบ
...
// 🗺 k144 (เคสจริง 3/8): ลูกค้าพิมพ์ "โอเครวมยอดให้เลย" → ระบบอ่านว่าอยู่ "จังหวัดเลย" = นอกเขตส่งด่วน
//   → ปฏิเสธส่งด่วนกับลูกค้า กทม. ทันที (เสียยอดตรงๆ) และล็อกผิดไปทั้งบทสนทนา
//   ต้นเหตุ: ชื่อจังหวัดบางตัวเป็นคำไทยที่ใช้ทั่วไป (เลย/ตาก/น่าน/ตรัง/แพร่) แต่ regex จับแบบคำเปล่า
//   ทดสอบยืนยัน: "เอาเลยค่ะ" "สั่งเลยครับ" "ไม่มีเลย" "ตากลม" "แพร่หลาย" ติดหมดทุกอัน
//   กฎใหม่: จังหวัดกลุ่มนี้ต้องมีคำบอกสถานที่นำหน้าเท่านั้น (จังหวัด/จ./อยู่/มาจาก/ส่งไป/ปลายทาง/แถว/อำเภอ)
const UPCOUNTRY_RE = /เชียงใหม่|เชียงราย|ลำปาง|ลำพูน|แม่ฮ่องสอน|พะเยา|อุตรดิตถ์|สุโขทัย|พิษณุโลก|พิจิตร|เพชรบูรณ์|กำแพงเพชร|นครสวรรค์|อุทัยธานี|ชัยนาท|สิงห์บุรี|อ่างทอง|ลพบุรี|สระบุรี|อยุธยา|อยุทธยา|สุพรรณบุรี|กาญจนบุรี|ราชบุรี|เพชรบุรี|ประจวบ|หัวหิน|ชะอำ|สมุทรสงคราม|นครนายก|ปราจีนบุรี|สระแก้ว|ฉะเชิงเทรา|ชลบุรี|พัทยา|ศรีราชา|ระยอง|จันทบุรี|ตราด|นครราชสีมา|โคราช|บุรีรัมย์|สุรินทร์|ศรีสะเกษ|อุบล|ยโสธร|อำนาจเจริญ|มุกดาหาร|ร้อยเอ็ด|มหาสารคาม|ขอนแก่น|กาฬสินธุ์|สกลนคร|นครพนม|หนองคาย|บึงกาฬ|หนองบัวลำภู|อุดรธานี|ชัยภูมิ|นครศรีธรรมราช|กระบี่|พังงา|ภูเก็ต|สุราษฎร์|เกาะสมุย|เกาะพะงัน|ระนอง|ชุมพร|สงขลา|หาดใหญ่|สตูล|พัทลุง|ปัตตานี|ยะลา|นราธิวาส|เบตง/;
const UPCOUNTRY_AMBIG_RE = /(?:จังหวัด|จ\.\s*|อยู่(?:ที่)?|มาจาก|ส่งไป(?:ที่)?|ปลายทาง|แถว|อ\.\s*|อำเภอ)\s*(เลย|ตาก|น่าน|ตรัง|แพร่)/;
function matchUpcountry(text) {
  const s = String(text || "");
  const a = s.match(UPCOUNTRY_RE); if (a) return a[0];
  const b = s.match(UPCOUNTRY_AMBIG_RE); if (b) return b[1];
  return "";
}
function locHint(text) {
  ...
  const m = matchUpcountry(s) ? [matchUpcountry(s)] : null;
...
// 🔎🔎 k225 — ส่ง "รายชื่อรุ่นในหมวด" ให้ AI เวลาลูกค้าขอดูตัวอื่น/ถามว่ามีอะไรบ้าง
//   ⛔ ส่งเฉพาะรุ่นที่ "มีของจริง" — ห้ามลิสต์ของที่หมด (บทเรียน k192)
//   ⛔ ไม่แต่งราคาเอง ทุกตัวดึงจากตาราง PRICE ตรง ๆ
//   ⛔ ถ้าไม่รู้ว่าหมวดไหน = ไม่ส่งอะไรเลย ให้ระบบถามกลับ (Never Guess)
const CAT_LABEL = { disp: "พอตใช้แล้วทิ้ง", bigpod: "หัวน้ำยาใหญ่ (บิ๊กพอต)", smallpod: "หัวพอตเล็ก", device: "เครื่อง", liquid: "น้ำยาขวด", pouch: "นิโคตินพอช", iqos: "ไส้บุหรี่ IQOS" };
function catalogHint(text, sm, buf, prevCat) {
  try {
    const kind = browseKind(text);
    if (!kind) return "";
    // หมวดมาจากไหน — ลูกค้าพิมพ์เอง > รุ่นที่เอ่ยในข้อความ > หมวดของรุ่นที่คุยค้างไว้
    let cat = "";
    try { cat = topicInText(text) || ""; } catch (e) {}
    if (cat === "head") cat = "";                       // "หัวพอต" ลอย ๆ ครอบ 2 หมวด = ยังไม่ชัด
    if (!cat) { try { const m = _MODEL_IN(text); if (m) cat = topicOfModel(m); } catch (e) {} }
    if (!cat) cat = String(prevCat || "");
    if (!cat || !CAT_LABEL[cat]) return "";             // ไม่รู้หมวด = ไม่เดา
    const B = (typeof buf === "number") ? buf : 1;
    const inStock = (m) => {
      const fl = (FLAVORS[m] && FLAVORS[m].f) || [];
      if (!fl.length) { let q = null; try { q = findStockForItem(sm, m, ""); } catch (e) {} return q === null || q > B; }
      return fl.some(f => { let q = null; try { q = findStockForItem(sm, m, f); } catch (e) {} return q === null || q > B; });
    };
    const rows = [];
    for (const m of Object.keys(FLAVORS)) {
      if (topicOfModel(m) !== cat) continue;
      if (/\(โคลน\)/.test(m)) continue;                  // โคลนไม่ต้องเสนอเอง ลูกค้าถามค่อยบอก
      const p = PRICE[m];
      if (typeof p !== "number") continue;
      if (!inStock(m)) continue;
      const n = ((FLAVORS[m] && FLAVORS[m].f) || []).length;
      rows.push({ m, p, n });
    }
    if (!rows.length) return "";
    rows.sort((a, b) => b.p - a.p);
    const show = rows.slice(0, 12);
    let out = "\n\n[รายการ" + CAT_LABEL[cat] + "ที่มีของพร้อมส่ง — ข้อมูลจากระบบ ห้ามบอกลูกค้าว่ามาจากไหน]";
    for (const r of show) out += "\n• " + r.m + " " + r.p + " บาท" + (r.n ? " (" + r.n + " กลิ่น/สี)" : "");
    if (rows.length > show.length) out += "\n(และอีก " + (rows.length - show.length) + " รุ่น)";
    out += "\n⛔⛔ ลูกค้ากำลังขอ **ดูตัวเลือก** ห้ามตอบรุ่นเดียวซ้ำเดิมเด็ดขาด — ต้องเสนอหลายรุ่นจากลิสต์นี้";
    out += "\n⛔ ห้ามเอ่ยรุ่นที่ไม่มีในลิสต์นี้ และห้ามแต่งราคาเอง";
    out += "\n⚡ ตอบสั้น เสนอ 3-5 รุ่นพอ แล้วถามว่าสนใจตัวไหน";
    console.log("K225_CATALOG_HINT cat=" + cat + " n=" + rows.length);
    return out;
  } catch (e) { return ""; }
}
...
//   แอดมินลิสต์ MARBO 9K (บลูไอซ์), KS Quik 6K (ชานมอู่หลง) = มั่วทั้งหมด ไม่ใช่กลิ่นมิ้นต์ฟรีซสักตัว
...
// 🎨 k70: "ค้นตามแนวกลิ่น" — ปิดรูสุดท้ายที่ทำให้แอดมินมโนชื่อกลิ่น
...
function unknownAskHint(text, sm, buf) {
  ...
  return "\n\n[🎯 ระบบค้นชื่อสินค้าจากข้อความนี้ไม่เจอเลย — คำที่ลูกค้าใช้อาจสะกดเพี้ยน/เป็นชื่อเล่นที่ไม่รู้จัก/หรือร้านไม่มีขาย]"
    ...
    + "\n✅ ให้ถามกลับสั้นๆ เช่น \"รบกวนพิมพ์ชื่อรุ่นอีกครั้ง หรือส่งรูปสินค้ามาได้เลยค่ะ 🙏🏻 เดี๋ยวแอดมินเช็คให้ทันทีค่ะ\""
...
// 💸💸 k224 · CB-3 — "เลขที่ตามหลังคำบอกบริบท ไม่ใช่จำนวนสินค้า"
//   เคสจริง: "มาโบ9k แตงโม ค่าส่ง 40 ใช่มั้ยคะ" → ระบบเข้าใจว่าสั่ง 40 แท่ง = 14,000 บาท แทน 350
//   ต้นเหตุ: ตัวตัดเลขเดิมตัดเฉพาะเลขที่ "มีหน่วยตามหลัง" (บาท/mg/%) เท่านั้น
//     เลขที่ "มีคำบอกบริบทนำหน้า" อย่าง `ค่าส่ง 40` ไม่เคยถูกตัด แล้วกฎสุดท้าย
//     `(\d{1,3})\s*$` คว้าเลขท้ายประโยคมาเป็นจำนวน
//   ⛔ แก้ที่กติกา ไม่ใช่ไล่เติมคำ — ทุกคำในตารางนี้แปลว่า "เลขข้างหลังไม่ใช่จำนวนสินค้า"
const QTY_CTX_KILL = [
  // ค่าส่ง / ค่าธรรมเนียม
  /(ค่าส่ง|ค่าจัดส่ง|ค่าขนส่ง|ค่าธรรมเนียม|ค่าปรับ|ค่ามัดจำ|ปลายทาง|เก็บปลายทาง|ส่งด่วน|แกร[็ๆ]?[บป])\s*(?:=|คือ|ที่)?\s*\d{1,4}/g,
  // ราคา / ยอดเงิน
  /(ราคา|อันละ|ตัวละ|ชิ้นละ|แท่งละ|หัวละ|เครื่องละ|ยอด|รวม|ทั้งหมด|สุทธิ|ลด|ส่วนลด|โอน|จ่าย|ชำระ)\s*(?:=|คือ|ที่)?\s*\d{1,5}/g,
  // เลขออเดอร์ / เลขพัสดุ / เลขบัญชี
  /(ออเดอร์|order|เลขที่|เลขพัสดุ|เลขติดตาม|tracking|บัญชี|เลขบัญชี|พร้อมเพย์|ref|อ้างอิง)\s*[:：#]?\s*[A-Za-z]*\d{1,20}/gi,
  // ที่อยู่ — บ้านเลขที่ · หมู่ · ซอย · ถนน · รหัสไปรษณีย์
  /(บ้านเลขที่|เลขที่บ้าน|หมู่ที่|หมู่|ซอย|ซ\.|ถนน|ถ\.|ตรอก|ห้อง|ชั้น|อาคาร|ตึก|รหัสไปรษณีย์|ไปรษณีย์)\s*[\d\/\-]{1,12}/g,
  // เบอร์โทร
  /0\d{1,2}[- ]?\d{3}[- ]?\d{3,4}/g,
  /(เบอร์|โทร|tel|มือถือ|โทรศัพท์)\s*[:：]?\s*[\d\- ]{6,15}/gi,
];
// 🔁 k224 · CB-4 — "ประโยคทวนยืนยัน ไม่ใช่การสั่งเพิ่ม"
//   เคสจริง (Gold120 A18): "คือได้2เครื่องพร้อมหัวเลยใช่มั้ยคับ" → ระบบอ่านเป็นสั่ง 2 เครื่อง
//   หลักที่ใช้: **Context ก่อน Quantity** — ถ้ารูปประโยคคือการทวนถาม ห้ามอ่านเป็นคำสั่งซื้อ
//   ⛔ ต้องเป็นรูป "ทวนยืนยัน" ทั้งประโยคเท่านั้น (ลงท้าย) ไม่ใช่เจอคำนี้ที่ไหนก็ได้
const QTY_CONFIRM_RE = /(ใช่ไหม|ใช่มั้ย|ใช่มัย|ใช่ม่ะ|ใช่ปะ|ใช่รึ|ใช่หรือ|ชัวร์|แน่ใจ|หรอ|เหรอ|รึเปล่า|หรือเปล่า|ป่าว)\s*[?？]*\s*(ครับ|คร้าบ|คับ|ค้าบ|ค่ะ|คะ|ค๊า|จ้า|จ้ะ|นะ|น๊า|ฮะ|งับ|ครัช|ครัฟ|คร่ะ)*\s*[?？]*\s*$/;
function fmQty(seg) {
  const raw0 = String(seg || "");
  // 🔁 CB-4: ประโยคทวนยืนยัน = ไม่ใช่การสั่ง → ไม่ตัดสินจำนวน (ให้ระบบถามกลับ)
  if (QTY_CONFIRM_RE.test(raw0)) return null;
  let pre = raw0;
  for (const re of QTY_CTX_KILL) pre = pre.replace(re, " ");   // 💸 CB-3: ตัดเลขที่มีคำบอกบริบทนำหน้า
  const t = pre.replace(/\d+\s*[Kk]\b/g, " ").replace(/\d+\s*เค(?!รื)/g, " ")
...
function flavorHint(text, sm, buf){
  ...
  // เคสจริง 30/7: ลูกค้าพิมพ์ "พีชสตอ" แอดมินอ่านเป็น "กลิ่นพีช" แล้วบอกหมด ทั้งที่พีชสตรอว์เบอร์รี่มีของ
  try {
...
// เดิม flavorHint ยิงเฉพาะตอนลูกค้าพิมพ์ชื่อรุ่นเป๊ะๆ พอถามกว้างๆ แอดมินไม่ได้ข้อมูลสต็อกเลย → มั่วเอง
// (เคสจริง: ถาม "แบรนด์ abc มีตัวไหนพร้อมส่ง" แอดมินตอบ "ABC LEGO องุ่น 5% เหลือจำนวนจำกัด"
...
function brandHint(text, sm, buf) {
  ...
  // แนบ "กลิ่นจริงที่มีของ" ของรุ่นแนะนำ กันแอดมินกุชื่อกลิ่นเอง (เคยตอบ "บลูเรส" ซึ่งไม่มีจริง)
...
// 🌏 k145 (เคสจริง 3/8): ลูกค้าไทยพิมพ์ "Marbo 9 k" → detectLang คืน "en" → บันทึกลง KV 7 วัน
//   → langRule สั่ง AI ว่า "ห้ามตอบภาษาไทย" → ลูกค้าไทยโดนตอบอังกฤษทั้งย่อหน้า ต้องถามใหม่เป็นไทย
//   ต้นเหตุ: ชื่อรุ่นทั้งร้านเป็นอังกฤษ (MARBO 9K / RELX / KS QUIK) และ prompt ก็สอนให้พิมพ์แบบนี้
//   กฎใหม่: ตัดชื่อสินค้า/แบรนด์ออกก่อน ถ้าไม่เหลือคำอังกฤษจริง = ไม่ใช่หลักฐานว่าเป็นชาวต่างชาติ
const PRODUCT_TOKENS = /\b(marbo|relx|infy|esko|elfbar|elf|bar|ks|quik|pro|vazer|reload|lego|tank|abc|boost|pod|clear|swap|switch|kit|smash|go|sparta|diva|lana|iris|carnival|joiway|twins|vosoon|dual|plus|zero|infinity|ultra|large|creator|essential|iqos|terea|star|sonic|zyn|pouch|freebase|saltnic|k|x|ml)\b/gi;
function detectLang(t) {
  const s = String(t || "");
  if (/[\u0E00-\u0E7F]/.test(s)) return "th";                     // ไทย → ตอบไทย
  //   ⚠️ ต้องตัด "รหัสความจุ" (9K / 20k / 15K) ก่อน เพราะ \b ใน PRODUCT_TOKENS แยก "9K" ไม่ออก
  //      (9 กับ K เป็น word char ทั้งคู่ → ไม่มี boundary คั่น) → เหลือ "K" ลอย → ถูกนับเป็นภาษาอังกฤษ
  const _stripped = s.replace(/\d+\s*[kK]\b/g, " ").replace(PRODUCT_TOKENS, " ").replace(/[\d\s\p{P}\p{S}]/gu, "");
  if (/[A-Za-z]{2,}/.test(_stripped)) return "en";                  // เหลือคำอังกฤษจริงหลังตัดชื่อสินค้า
  // จีน/ญี่ปุ่น/เกาหลี/รัสเซีย/อาหรับ ฯลฯ → ตอบอังกฤษ (เบสหลักของร้าน)
  // k145: ต้องเช็คจาก _stripped ไม่ใช่ s ดิบ — ไม่งั้นตัวอักษรอังกฤษในชื่อรุ่นจะโดนดักซ้ำตรงนี้
  if (/[^\u0E00-\u0E7F\s\d\p{P}\p{S}]/u.test(_stripped)) return "en";
...
const ALIAS = [
  // k73 เคสจริง 1/8: ลูกค้าถาม "ชุดพร้อมสูบเหลือไร" → แอดมินตอบวนแต่ ESKO ที่หมด
...
function findPrice(modelText) {
  ...
  // 💰 k146b (คู่กับ k146): AI มักพิมพ์ "MARBO 9K โคลน" โดยไม่ใส่วงเล็บ → indexOf หาคีย์ "(โคลน)" ไม่เจอ
  //   → ตกไปเจอ "MARBO 9K" แล้ว return ทันที = คิดราคาแท้ 350 กับของโคลน 290 (ลูกค้าจ่ายเกิน 60/แท่ง)
  //   และ p.key ไม่ใช่ "(โคลน)" → cloneQty ไม่นับ → เสียเรทขายส่ง + เสียโปรส่งฟรี 20 แท่งไปด้วย
  //   กฎ: ถ้าข้อความบอกว่าเป็นโคลน ให้ล็อกคีย์ที่มี "(โคลน)" ก่อนเสมอ
  if (/โคลน|เทียบแท้|clone/i.test(String(modelText || ""))) {
    const bare = t.replace(/โคลน|เทียบแท้|CLONE|\(|\)/gi, " ").replace(/\s+/g, " ").trim();
    const cl = PRICE_KEYS.filter(k => /\(โคลน\)/.test(k) && bare.indexOf(k.replace(/\s*\(โคลน\)/, "").toUpperCase()) !== -1);
    if (cl.length === 1) return { key: cl[0], price: PRICE[cl[0]] };
  }
...
const NO_GUESS_RULE = "\n\n# ⛔ ห้ามเดา\n" +
"- ห้ามแต่งชื่อกลิ่น/สี/โปรเอง ใช้ได้เฉพาะที่ระบบส่งมาให้ ถ้าไม่มีข้อมูล ตอบว่า 'เดี๋ยวทีมงานเช็คให้อีกครั้งนะคะ 🙏🏻'\n" +
...
"1️⃣ บอกสิ่งที่ต้องการ\nพิมพ์ รุ่น + กลิ่น + จำนวน มาได้เลยค่ะ\nเช่น \"MARBO 9K องุ่น 2 ชิ้น\"\n💡 ไม่รู้ชื่อรุ่นก็ได้ค่ะ ส่งรูปสินค้าหรือรูปเมนูมา แอดมินอ่านให้เองเลย 📷\n\n" +
"2️⃣ ตรวจรายการ\nแอดมินจะสรุปเป็นการ์ดให้ พร้อมราคาและค่าส่ง\nถ้าถูกต้องแล้ว กดปุ่ม \"✅ ยืนยันรายการ\" ในการ์ดได้เลยค่ะ\n\n" +
"3️⃣ โอนเงิน\nพอกดยืนยัน ระบบจะส่งเลขบัญชีให้ทันที\nโอนแล้ว ส่งสลิปมาในแชทนี้ได้เลยนะคะ 🧾\n\n" +
"4️⃣ แจ้งที่อยู่\nพอสลิปผ่าน แอดมินจะขอที่อยู่จัดส่ง กรอกตามฟอร์มที่ส่งให้ได้เลยค่ะ\n\n" +
"5️⃣ รอรับสินค้า 🎉\n📦 พัสดุปกติ ค่าส่ง 40 บาท ได้รับใน 2-3 วัน\n🛵 ส่งด่วน (กทม.+ปริมณฑล) แชร์โลเคชั่นมา เดี๋ยวคิดค่าส่งให้ ได้รับใน 1-3 ชม.\n\n" +
"อยากคุยกับทีมงานคนจริง พิมพ์ \"ขอทีมงาน\" ได้ตลอดเลยนะคะ 💕\nเริ่มได้เลยค่ะ สนใจรุ่นไหนดีคะ 😊";
// ===== บุคลิก + คู่มือตอบ (กลั่นจากแชทจริงของร้าน) + ความรู้สินค้า =====
const SYSTEM_PROMPT = `คุณคือ "แอดมิน" แอดมินร้าน ABC ผู้หญิง บุคลิกสุภาพ ทางการ เรียบร้อย น่าเชื่อถือ ตอบลูกค้าทางแชท LINE ให้เหมือนแอดมินจริงของร้าน

# ชื่อของคุณ
- คุณชื่อ "แอดมิน" — แนะนำตัวตอนทักทายครั้งแรก และใช้แทนตัวเองว่า "แอดมิน" ได้ (เช่น "เดี๋ยวแอดมินเช็คให้นะคะ 💕") ไม่ต้องพร่ำเพรื่อทุกข้อความ
- 🤖 ถ้าลูกค้าถามตรงๆ ว่าเป็นคนหรือบอท/AI → **ยอมรับตามตรง ห้ามโกหกว่าเป็นคน** แล้วชูจุดขาย เช่น
  "แอดมินเป็นผู้ช่วย AI ของร้านค่ะ 🤖 ตอบได้ตลอด 24 ชม.เลยนะคะ ถ้าอยากคุยกับทีมงานคนจริง บอกได้เลยค่ะ เดี๋ยวเรียกให้ทันที 💕"
...
ลูกค้า: "ส่งกี่วัน"
"ABC ยินดีต้อนรับค่ะ ✨ แอดมินยินดีให้บริการค่ะ 💚
...
✅ รูปแบบที่ถูก: "เรื่องนี้แอดมินไม่มีข้อมูลค่ะ 🙏🏻 เดี๋ยวทีมงานเช็คให้นะคะ" แล้วชวนกลับเข้าเรื่องสินค้า
...
- ⛔⛔ ความยาวคำตอบ (กฎเหล็ก — แอดมินจริงพิมพ์สั้นมาก มัธยฐาน 39 ตัวอักษรจากแชทจริง 2.3 ล้านข้อความ):
  · คำถามที่ตอบได้ด้วย มี/ไม่มี/ได้/ไม่ได้/ใช่ → ตอบ "ประโยคเดียว" พอ เช่น "มีค่ะ 💕" · "หมดค่ะ 🙏🏻" · "ได้ค่ะ"
  · คำถามทั่วไป → ไม่เกิน 2 บรรทัด และไม่เกิน 120 ตัวอักษร
  · ห้ามเกริ่นนำก่อนเข้าเรื่อง (ห้ามขึ้นต้น "ขออนุญาตแจ้ง.." "ต้องขอเรียนว่า.." "สำหรับ..") ตอบสิ่งที่ถามเป็นคำแรกเลย
  · ถามมา 1 เรื่อง ตอบ 1 เรื่อง ห้ามแถมข้อมูลที่ไม่ได้ถาม และห้ามยิงหลายคำถามกลับในข้อความเดียว
  · ยกเว้น 2 กรณีนี้เท่านั้นที่ยาวได้: บล็อกทวนคำสั่งซื้อ และการลิสต์ชื่อกลิ่นที่มีของ (ต้องครบ ห้ามตัด)
...
3) รับออเดอร์ตามลำดับ: (1) เก็บ รุ่น+กลิ่น/สี+จำนวน → (2) สรุปยอด+แจ้งเลขบัญชี ให้ลูกค้าโอน+ส่งสลิป → (3) พอสลิปผ่านแล้วค่อยขอที่อยู่ → (4) สรุปออเดอร์ให้ทีมงาน
...
- ส่งด่วน (เฉพาะ กทม.+ปริมณฑล) = คิดตามระยะทาง ให้ขอลูกค้า "แชร์โลเคชั่น (ปักหมุด) หรือส่งลิงก์ Google Maps" มา แล้ว **ทีมงานจะเช็คราคาจริงให้** (⛔ แอดมินห้ามกุตัวเลขค่าส่งด่วนเองเด็ดขาด ทุกกรณี — ราคาต้องมาจากแอดมินเท่านั้น)
- ⛔ นอกจาก 40 (ขนส่งเอกชน) กับ 0 (ฟรีตามโปร) ห้ามแอดมินพูดตัวเลขค่าส่งอื่นเด็ดขาด
...
- ⛔⛔⛔ ห้ามให้ส่วนลดเองทุกกรณีเด็ดขาด — ห้ามลดราคาสินค้า ห้ามลดค่าส่ง ห้ามให้ "ราคาพิเศษ/เฉพาะออเดอร์นี้" ห้ามแถมของนอกโปรที่กำหนดไว้ ห้ามต่อรอง
  ...
  ถ้าลูกค้ายังไม่พอใจ ให้ตอบว่า "รอสักครู่นะคะ 🙏🏻 ทีมงานหลังการขายจะเข้ามาดูแลให้บริการค่ะ 💕" แล้วหยุด — ห้ามเสนอตัวเลขใหม่เอง
...
⛔ เรทขายส่งนี้ใช้กับ MARBO 9K โคลน/เทียบแท้ เท่านั้น รุ่นอื่นไม่มี — ถ้าลูกค้าถามเรทส่งรุ่นอื่น ให้บอกว่า "เดี๋ยวทีมงานเช็คเรทส่งให้อีกครั้งนะคะ 🙏🏻"
⛔⛔ ราคา MARBO 9K มี 2 แบบ ห้ามสลับกัน: **แท้ = 350 บาท/แท่ง** | **โคลน (เทียบแท้) = 290 บาท/แท่ง**
  เรทขายส่งข้างบนเป็นราคาของ "โคลน" เท่านั้น — ของแท้ไม่มีเรทขายส่ง ถ้าลูกค้าถามซื้อแท้จำนวนมาก ให้บอกว่า "เดี๋ยวทีมงานเช็คราคาให้นะคะ 🙏🏻"
...
# กติกาสำคัญ (ห้ามพลาด)
- ใช้ราคาจาก "รายการสินค้า" ด้านล่างเท่านั้น ห้ามเดา/แต่งราคา ถ้าลูกค้าถามรุ่นที่ไม่มีในรายการ ให้บอกว่าจะเช็คให้และทีมงานยืนยันอีกครั้งค่ะ
...
- เลขบัญชีสำหรับโอน: ถ้ามี "ข้อมูลชำระเงินของร้าน" อยู่ท้ายพรอมต์ ให้แจ้งข้อมูลนั้นเมื่อลูกค้าพร้อมโอน/ถามเลขบัญชี ถ้าไม่มี ให้บอกว่า "ทีมงานจะสรุปยอดและแจ้งเลขบัญชีให้อีกครั้งนะคะ" — ห้ามแต่งเลขบัญชีเอง
...
- ⛔⛔ กฎเหล็กเรื่องสต็อก (สำคัญที่สุด): คุณจะรู้สต็อกก็ต่อเมื่อมีหัวข้อ "# สต็อกจริงตอนนี้" แนบอยู่ท้ายพรอมต์เท่านั้น
  - ถ้า**ไม่มี**หัวข้อนั้น = คุณไม่รู้สต็อก **ห้าม**พิมพ์คำว่า "ผลการเช็คสต็อก" ห้ามบอกว่า มีของ/หมด/เหลือกี่ชิ้น เด็ดขาดทุกกรณี ให้ตอบประโยคเดียวว่า "เดี๋ยวทีมงานเช็คสต็อกและยืนยันให้อีกครั้งนะคะ 🙏🏻" แล้วดำเนินการรับออเดอร์ต่อแบบรอยืนยัน
...
⛔ คุณ (แอดมิน) ห้ามขอที่อยู่เองเด็ดขาด ระบบจะขอที่อยู่ให้เองหลังสลิปผ่านแล้วเท่านั้น

## ขั้น 3 — เมื่อในประวัติแชทมี "[ลูกค้าส่งสลิปโอนเงิน — ตรวจสอบแล้วชำระเงินถูกต้อง]" แล้ว และลูกค้าเพิ่งส่งที่อยู่จัดส่งมา → สรุปออเดอร์ให้ทีมงาน (บล็อกนี้เป๊ะ)
📦 สรุปออเดอร์
...
- ⛔ ห้ามพูดกับลูกค้าว่า "เช็คจากแอป/เช็คจากแกร็บ/เปิดแอปดู" เด็ดขาด (เผยวิธีทำงานภายในร้าน) → ให้พูดแค่ "ทีมงานเช็คค่าส่งด่วนให้"
- ค่าส่งด่วนคิดตามระยะทาง — ถ้าลูกค้าเลือกส่งด่วน ให้ขอโลเคชั่น(ปักหมุด)มาก่อน พอลูกค้าปักหมุด ทีมงานจะเช็คราคาจริงแล้วแจ้งเอง (⛔ แอดมินห้ามกุตัวเลขค่าส่งด่วนเองทุกกรณี, ห้ามระบุชื่อบริษัทขนส่ง)
...
"รอสักครู่นะคะ ทีมงานหลังการขายจะเข้ามาดูแลให้บริการค่ะ 🙏🏻" แล้วหยุด ไม่ต้องพยายามแก้เอง

# คลังคำตอบมาตรฐาน (กลั่นจากแชทแอดมินจริงของบริษัท — ตอบแนวนี้)
- ส่งด่วน: มีบริการในบางพื้นที่ (กทม.+ปริมณฑล) ตอบว่า "มีบริการส่งด่วนค่ะ 🛵 รบกวนคุณลูกค้าแชร์โลเคชั่น (ปักหมุด) หรือส่งลิงก์ Google Maps มาให้หน่อยนะคะ เดี๋ยวทีมงานเช็คราคาให้ทันทีค่ะ" — ⛔ ห้ามกุค่าส่งด่วนเอง ⛔ ห้ามพูดว่า "กำลังประสานทีมขนส่ง / รอ 5-10 นาที" หรือกุเวลารอใดๆ เด็ดขาด
...
- เคลมสินค้า: ระยะรับเคลมนับจากวันได้รับของ → 1-19 แท่ง = 7 วัน / 20+ = 14 วัน / 50+ = 21 วัน / 100+ = 30 วัน (ห้ามบอกตัวเลขอื่นเด็ดขาด) + ต้องมีวิดีโอตอนแกะกล่อง ถ้าลูกค้าแจ้งของเสีย (หัวตัน สูบไม่ขึ้น น้ำยาซึม เครื่องไม่ติด) ให้ถามก่อนว่า "รุ่นไหน อาการเป็นแบบไหนคะ" 1 ครั้ง แล้วส่งต่อทีมงานหลังการขาย
...
- 📱 ไอจี/เฟซบุ๊ก/เพจ/ช่องทางอื่น (k123 — เจ้าของร้านยืนยัน 3/8): **ร้านไม่มีไอจี ไม่มีเพจเฟซบุ๊ก** ขายผ่าน LINE ช่องทางเดียวเท่านั้น
  ⛔ ห้ามบอกว่า "เดี๋ยวส่งลิงก์ให้" · ห้ามแต่งชื่อไอจี/เพจขึ้นมาเอง · ห้ามบอกให้ไปค้นหาเอง
  ✅ ตอบทำนองนี้ แล้วดึงกลับมาที่การขายทันที: "ทางร้านขายผ่าน LINE ช่องทางเดียวค่ะ 💕 สั่งซื้อและสอบถามได้ที่นี่เลยนะคะ"
  ✅ ถ้าลูกค้าอยากดูรีวิว/ความน่าเชื่อถือ → ส่งลิงก์ Openchat ด้านบนแทนได้ค่ะ
...
- ถามเลขพัสดุ/สถานะ: "หลังสั่งซื้อสำเร็จ ออเดอร์จะจัดส่งออกภายใน 1-2 วันค่ะ แนะนำติดตามเลขพัสดุหลังสั่งซื้อ 2-3 วันนะคะ" — ถ้าลูกค้าบอกเลขผิด/เกินกำหนดแล้วยังไม่ได้ → ส่งต่อทีมงานหลังการขาย
- ของยังไม่ถึง/ล่าช้า: แสดงความเข้าใจก่อนเสมอ ("แอดเข้าใจคุณลูกค้าอย่างมากค่ะ ขออภัยในความล่าช้านะคะ") แล้วส่งต่อทีมงานหลังการขาย ห้ามเดาสถานะพัสดุเอง
- เปลี่ยนที่อยู่ / เปลี่ยนเป็นส่งด่วน / แก้ออเดอร์หลังสั่ง: รับเรื่องแล้วส่งต่อทีมงานหลังการขาย (แอดมินต้องเช็คสถานะออเดอร์ก่อน)
...
- หลังลูกค้าโอน+ส่งสลิปและทีมงานยืนยันแล้ว มาตรฐานร้านคือย้ำลูกค้าว่า: "เมื่อได้รับพัสดุ รบกวนถ่ายวิดีโอตอนแกะกล่องด้วยนะคะ เพื่อใช้เป็นหลักฐานในการเคลมกรณีพัสดุมีปัญหาค่ะ ✨"

# ตัวอย่างบทสนทนา (เลียนแบบสไตล์นี้ — มาจากแอดมินจริงของร้าน)
ลูกค้า: มีส่งด่วนมั้ย
แอดมิน: มีบริการส่งด่วนค่ะ 🛵 รบกวนคุณลูกค้าแชร์โลเคชั่น (ปักหมุด) มาให้หน่อยนะคะ เดี๋ยวทีมงานเช็คค่าส่งด่วนและแจ้งรอบส่งให้ค่ะ 💕

ลูกค้า: หัวตันอะครับ เคลมได้มั้ย
แอดมิน: ขออภัยในความไม่สะดวกนะคะ 🙏🏻 ขอสอบถามหน่อยค่ะ หัวรุ่นไหนคะ อาการสูบไม่ขึ้นเลยใช่ไหมคะ (เงื่อนไขเคลม: 1-19 แท่ง ภายใน 7 วัน / 20+ 14 วัน / 50+ 21 วัน / 100+ 30 วัน นับจากได้รับของ และต้องมีวิดีโอตอนแกะกล่องค่ะ) รอสักครู่นะคะ ทีมงานหลังการขายจะเข้ามาดูแลให้บริการค่ะ 🙏🏻
...
สะดวกแบบไหนดีคะ
(⛔ ห้ามเสนอลดค่าส่งด่วน ห้ามให้ราคาพิเศษ ถ้าลูกค้ายังยืนยันขอส่งด่วนฟรี ให้ส่งต่อทีมงานหลังการขาย)
...
- SONIC 8K = 300 บาท (มี 10 กลิ่น/สี)
...
- ถ้าลูกค้าถามเครื่องรุ่นที่ไม่อยู่ในลิสต์ → "เดี๋ยวทีมงานเช็คให้อีกครั้งนะคะ 🙏🏻"
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // 📚 ดูฐานกลิ่นทั้งหมดที่แอดมินรู้: /catalog
    if (url0.pathname === "/catalog") {
      ...
      return new Response("แอดมินรู้จัก " + Object.keys(FLAVORS).length + " รุ่น / " + sku + " กลิ่น-สี\n\n" + lines.join("\n"), { headers: { "Content-Type": "text/plain; charset=utf-8" } });
    }
    // 🔄 สั่งซิงก์สต็อกจากไฟล์ฐานเดี๋ยวนี้: /syncstock  (ทำวันละครั้งอัตโนมัติอยู่แล้ว)
    // 🔎 เทสปุ่ม Quick Reply ว่าจะขึ้นปุ่มอะไร: /qrtest?t=ข้อความที่แอดมินตอบ
    ...
    if (url0.pathname === "/simulate") {
      ...
      for (const t of qs) {
        ...
        out.push({ ลูกค้าถาม: t, วินาที: secs, แอดมินตอบ: reply, ออกการ์ด: card, ปุ่ม: qr ? qr.items.map(x => x.action.label) : [], ปัญหาที่ตรวจพบ: bad });
      }
    ...
    if (url0.pathname === "/eval") {
      ...
      // 🎯 k191 — Order Flow Test: รายชื่อบท / รัน 1 บท / สรุป KPI จากผลที่หน้าเว็บส่งกลับมา
      if (url0.searchParams.get("flowlist")) {
        return J({ build: BUILD, promptVer: promptVer(), จำนวนบท: FLOW_CASES.length,
          เป้าหมาย: { พัสดุ: "G2 ปิดออเดอร์เองได้ 100%", Grab: "G2 ทำครบจนเหลือแค่แอดมินเช็คค่าส่ง · หลังค่าส่งเข้าระบบต้องปิดต่อเองได้" },
          ขั้นตอน: FLOW_STAGES,
          บท: FLOW_CASES.map((c) => ({ id: c.id, เส้นทาง: c.เส้นทาง, ชื่อ: c.ชื่อ, ตา: c.ตา, ค่าส่งด่วนที่ป้อนไว้: c.ค่าส่งด่วน || null })) });
      }
      if (url0.searchParams.get("flow")) {
        const fid = parseInt(url0.searchParams.get("flow"), 10);
        const fc = FLOW_CASES.find((x) => x.id === fid);
        if (!fc) return J({ error: "ไม่พบบทที่ " + fid + " — ดูรายชื่อที่ ?flowlist=1" }, 404);
        try { return J({ build: BUILD, ...(await evalRunFlow(env, fc)) }); }
        catch (e) { return J({ build: BUILD, เคส: fc.id, ชื่อ: fc.ชื่อ, ผล: "ERROR", เหตุผลที่ตก: ["รันไม่สำเร็จ: " + String(e).slice(0, 200)] }); }
      }
      if (url0.searchParams.get("flowsum")) {
        try {
          const raw = url0.searchParams.get("flowsum");
          const rows = JSON.parse(decodeURIComponent(raw));
          return J({ build: BUILD, ...flowSummary(Array.isArray(rows) ? rows : []) });
        } catch (e) { return J({ error: "สรุปไม่ได้: " + String(e).slice(0, 150) }, 400); }
      }
    ...
    //   เคสจริง 2/8: ทีมงานเช็คแอปแล้วพิมพ์ "ค่าส่ง 185 บาทครับ" ในแชทเอง
    ...
    // 📦 k124 — ช่องกรอกเลขพัสดุในหลังบ้าน (ทำงานเหมือน /expfee)
    //   ที่มา: ขุดแชทร้านหลัก 34,471 บทสนทนา → ลูกค้าถาม "เลขพัสดุ / ของถึงไหน / เช็คสถานะ" **8,620 ครั้ง**
    //          และระบบไม่เคยมีคำตอบจริงเลย = ช่องว่างใหญ่สุดที่เหลือ
    //   วิธีใช้: แอดมินก๊อปเลขจาก XSelly → วางในหลังบ้าน → ระบบส่งให้ลูกค้าทันที + จำไว้
    //          ลูกค้าถาม "ของถึงไหน" ทีหลัง ระบบตอบเลขเดิมได้เอง ไม่ต้องรบกวนคน
    //   ⛔ เจ้าของร้านยืนยัน 3/8: ห้ามเอ่ยชื่อบริษัทขนส่งเป็นตัวหนังสือ → ส่งเป็น "ลิงก์เช็คสถานะ" แทน
    if (url0.pathname === "/track") {
      if (!OKEY()) return DENY();
      const shop = (url0.searchParams.get("shop") || "v20").toLowerCase();
      const uid = url0.searchParams.get("uid") || "";
      const no = (url0.searchParams.get("no") || "").trim().toUpperCase().replace(/\s+/g, "");
      const c = (url0.searchParams.get("c") || "").trim().toLowerCase();
      const J = (o, st) => new Response(JSON.stringify(o, null, 1), { status: st || 200, headers: { "content-type": "application/json; charset=utf-8", "access-control-allow-origin": "*" } });
      if (!uid) return J({ ผล: "ต้องมี uid" }, 400);
      ...
      try {
        ...
        // บันทึกลงความจำแชท เพื่อให้แอดมินเห็นว่าระบบแจ้งไปแล้ว (บทเรียนจาก k105)
        try {
          const _hk = "conv3:" + shop + ":" + uid;
          let _h = []; try { _h = JSON.parse((await env.CONV.get(_hk)) || "[]"); } catch (e) {}
          _h.push({ role: "assistant", content: msg, t: Date.now() });
          await env.CONV.put(_hk, JSON.stringify(_h.slice(-40)), { expirationTtl: 2592000 });
        } catch (e) {}
        // อัปเดตสถานะออเดอร์ให้แอดมินคนอื่นเห็นว่าส่งออกแล้ว
        try {
          const ok = "ord:" + shop + ":" + uid;
          const ov = await env.CONV.get(ok);
          if (ov) { const oj = JSON.parse(ov); oj.track = no; oj.status = "📦 ส่งออกแล้ว · เลขพัสดุ " + no; await env.CONV.put(ok, JSON.stringify(oj), { expirationTtl: 2592000 }); }
        } catch (e) {}
        console.log("TRACK_SET shop=" + shop + " no=" + no + " c=" + c);
    ...
    if (url0.pathname === "/expfee") {
      ...
      try {
        ...
        let msgs = [{ type: "text", text: "ค่าส่งด่วนของคุณลูกค้าคือ " + fee + " บาทค่ะ 🛵" }];
        let total = null;
        ...
        // 🧠 k105 (เจ้าของร้านชี้ 2/8): แอดมินกรอกค่าส่งในแผงถูกต้องแล้ว แต่แอดมิน "ไม่รู้" ว่าระบบแจ้งราคาไปแล้ว
        //   เพราะข้อความที่ระบบ push ไม่เคยถูกบันทึกลงความจำแชท → พอลูกค้าทวง "69 บาทไง" แอดมินเดาว่าเป็นราคาสินค้า
        //   แก้ที่ต้นเหตุ: บันทึกข้อความระบบลงความจำด้วย แอดมินจะเห็นบทสนทนาครบเหมือนคนอ่านแชท
        try {
          const _hk = "conv3:" + shop + ":" + uid;
          let _h = [];
          try { _h = JSON.parse((await env.CONV.get(_hk)) || "[]"); } catch (e) {}
          _h.push({ role: "assistant", content: "ค่าส่งด่วนของคุณลูกค้าคือ " + fee + " บาทค่ะ 🛵 (ค่าส่งด่วนรอบนี้ = " + fee + " บาท ยืนยันแล้ว)", t: Date.now() });
    ...
    // 📊 k91: คุณภาพคำตอบรายวัน — แอดมินตอบไปกี่ข้อความ ระบบกันความมั่วไปกี่ครั้ง
    ...
    if (url0.pathname.startsWith("/ctl/")) {
      ...
      try {
        ...
        if (act === "done") {
          const uid = url0.searchParams.get("uid");
          if (uid) { await env.CONV.delete("mute:" + shop + ":" + uid); UNMUTED.set(wakeKey(shop, uid), Date.now()); MUTED.delete(wakeKey(shop, uid)); MUTED_UIDS.delete(uid); }   // k201
          return J({ ok: 1 });
        }
...
// ═══════ k191 — Order Flow Test: วัดว่า "ปิดออเดอร์ได้จริงไหม" ไม่ใช่แค่ตอบถูก ═══════
//   เจ้าของร้านเปลี่ยน KPI หลักเป็น "ปิดออเดอร์ได้เอง" → ต้องมีชุดวัดแยกจาก Replay เดิม
//   เป้าหมายต่างกันตามเส้นทาง (เจ้าของร้านเลือกทาง A: Grab ให้คนเช็คค่าส่งเหมือนเดิม)
//     · พัสดุ  → G2 ต้องปิดเองได้ 100% (ค่าส่ง 40 ตายตัว ไม่ต้องพึ่งคน)
//     · Grab   → G2 ทำทุกขั้นจนเหลือแค่ให้แอดมินเช็คค่าส่ง แล้วหลังค่าส่งเข้าระบบต้องปิดต่อเองได้
//   ⛔ ข้อจำกัดที่ต้องรู้: สนามซ้อมส่งรูปสลิปไม่ได้ และไม่เรียก SlipOK จริง
//      → วัดได้ไกลสุดแค่ "ออกการ์ดยอด + เลขบัญชี + เก็บออเดอร์รอโอน" (เรียกว่า L1)
//      → "ปิดออเดอร์จริง (สลิปผ่าน)" = L2 ต้องวัดจากร้านจริงหลังเปิดเท่านั้น
const FLOW_STAGES = ["S1 เลือกสินค้า", "S2 เลือกกลิ่น", "S3 จำนวน", "S4 วิธีจัดส่ง", "S5 ที่อยู่/พิกัด", "S6 สรุปออเดอร์", "S7 รอชำระเงิน"];
const RX_STAGE = [
  /เมนูสินค้า|สนใจ(รับ|สินค้า)?ตัวไหน|รับอะไรดี|มีพร้อมส่ง|กลิ่นที่มี/,
  /รับกลิ่นไหน|สนใจกลิ่นไหน|กลิ่นไหนดี|กลิ่นอะไรดี|รับกลิ่นไหนแทน/,
  /รับกี่|กี่แท่ง|กี่หัว|กี่ชิ้น|กี่อัน|กี่ตัว/,
  /ส่งแบบไหน|ค่าส่ง 40|Flash|Grab|พัสดุปกติ|ส่งด่วน/i,
  /ขอที่อยู่|ชื่อผู้รับ|โลเคชั่น|ปักหมุด|รับพิกัด|google map|ขอหมุด|เบอร์โทร/i,
  /ยืนยันรายการ|ทวนคำสั่งซื้อ|สรุปคำสั่งซื้อ|รวมยอดชำระ|ยอดสินค้า/,
  /เลขบัญชี|ช่องทางการชำระ|สรุปรายการสั่งซื้อ \+ เลขบัญชี|โอนเข้าบัญชี/,
];
// ⛔ ถามข้อมูลผู้รับก่อนที่จะรู้ว่าลูกค้าจะเอาอะไร = พาออกนอก Flow (Wrong Turn)
const RX_ASK_RECIPIENT = /ขอที่อยู่|ชื่อผู้รับ|เบอร์โทร|ขอชื่อ|พิมพ์รายละเอียดจัดส่ง|ขอโลเคชั่น|ขอหมุด/;
const RX_KNOW_PRODUCT  = /ยืนยันรายการ|ทวนคำสั่งซื้อ|รวมยอดชำระ|ยอดสินค้า/;
// ลูกค้าทักท้วงว่าระบบเข้าใจผิด → เทิร์นถัดไปของบอทคือบททดสอบ Recovery
const RX_CUST_CORRECT  = /ไม่ใช่|ผิดแล้ว|บอกไปแล้ว|เพิ่งบอก|ที่บอกว่า|ไม่ได้สั่ง|ไม่ได้บอก|คนละ|ผิดนะ|อ่านใหม่/;
const RX_HANDOFF       = /ทีมงาน(จะ)?(เข้ามา|ดูแล|ตรวจสอบ)|ประสานงานแอดมิน|รอสักครู่นะคะ ทีม|รับเรื่องไว้แล้ว/;
// วัด KPI ของ 1 บทสนทนา — โค้ดล้วน ไม่ใช้ AI ตัดสิน (อธิบายที่มาของตัวเลขได้ทุกตัว)
function flowKPI(conv, c) {
  const rows = Array.isArray(conv) ? conv : [];
  const bot = (m) => m && m.who !== "ลูกค้า";
  const txt = (m) => String((m && m.text) || "");
  let stage = 0;                                   // ขั้นสูงสุดที่ไปถึง (1-7)
  let wrongTurn = false, wtWhy = "";
  let knowProduct = false;
  const asked = {};                                 // นับว่าถามเรื่องเดิมกี่รอบ
  let repeatAsk = 0, handoff = false;
  let recoveryNeeded = 0, recoveryOK = 0;
  let firstCloseIdx = -1;
  for (let i = 0; i < rows.length; i++) {
    const m = rows[i], t = txt(m);
    if (!bot(m)) continue;
    for (let s = RX_STAGE.length - 1; s >= 0; s--) { if (RX_STAGE[s].test(t)) { if (s + 1 > stage) stage = s + 1; break; } }
    if (RX_KNOW_PRODUCT.test(t)) knowProduct = true;
    // Wrong Turn: ขอข้อมูลผู้รับทั้งที่ยังไม่รู้ว่าจะเอาอะไร (ยังไม่เคยมีบล็อกทวนรายการ)
    if (!wrongTurn && RX_ASK_RECIPIENT.test(t) && !knowProduct) {
      // ยกเว้น Grab: ขอพิกัดเพื่อเช็คค่าส่งเป็นขั้นตอนปกติหลังรู้ของแล้วเท่านั้น
      wrongTurn = true; wtWhy = t.replace(/\n/g, " ").slice(0, 70);
    }
    for (const k of ["กลิ่น", "จำนวน", "ที่อยู่", "วิธีส่ง"]) {
      const rx = k === "กลิ่น" ? RX_STAGE[1] : k === "จำนวน" ? RX_STAGE[2] : k === "วิธีส่ง" ? RX_STAGE[3] : RX_ASK_RECIPIENT;
      if (rx.test(t)) { asked[k] = (asked[k] || 0) + 1; if (asked[k] >= 2) repeatAsk++; }
    }
    if (RX_HANDOFF.test(t)) handoff = true;
    if (firstCloseIdx === -1 && RX_STAGE[6].test(t) && stage >= 6) firstCloseIdx = i;
    // Recovery: เทิร์นก่อนหน้าลูกค้าทักท้วง → รอบนี้บอทต้องไม่ถามซ้ำและไม่โยนให้คน
    const prev = rows[i - 1];
    if (prev && !bot(prev) && RX_CUST_CORRECT.test(txt(prev))) {
      recoveryNeeded++;
      const ถามซ้ำ = RX_STAGE[1].test(t) || RX_STAGE[2].test(t);
      if (!RX_HANDOFF.test(t) && !ถามซ้ำ) recoveryOK++;
    }
  }
  // ── FPC (First Pass Completion): ข้อความแรกของลูกค้าให้ข้อมูลครบแค่ไหน แล้วบอทเข้าใจเลยไหม ──
  //   ผ่าน = สิ่งที่ลูกค้าบอกมาแล้วในข้อความแรก บอทต้องไม่ถามย้อนอีก
  //   ถ้าข้อความแรกไม่ได้ให้อะไรครบเลย → ไม่นับ (N/A) เพราะไม่มีอะไรให้ "เข้าใจตั้งแต่แรก"
  let FPC = null, fpcWhy = "";
  {
    const first = rows.find((m) => !bot(m));
    const ft = first ? txt(first) : "";
    let มีรุ่น = false, มีกลิ่น = false;
    try { มีรุ่น = !!_MODEL_IN(ft); } catch (e) {}
    const มีจำนวน = /\d{1,3}\s*(ชิ้น|อัน|แท่ง|หัว|ตัว|กล่อง)|(เอา|ขอ|สั่ง|รับ)[^\n]{0,20}?\d{1,3}/.test(ft);
    if (มีรุ่น) {
      const tnF = normTH(ft);
      const fl = [];
      for (const k in FLAVORS) for (const f of ((FLAVORS[k] && FLAVORS[k].f) || [])) fl.push(String(f).replace(/\s*\d+(\.\d+)?%\s*$/, "").trim());
      มีกลิ่น = fl.some((b) => b.length >= 3 && tnF.indexOf(normTH(b)) !== -1);
    }
    if (มีรุ่น && (มีกลิ่น || มีจำนวน)) {
      const botFirst = rows.slice(rows.indexOf(first) + 1).filter(bot).slice(0, 2).map(txt).join("\n");
      const ถามย้อน = (มีกลิ่น && RX_STAGE[1].test(botFirst)) || (มีจำนวน && RX_STAGE[2].test(botFirst));
      FPC = !ถามย้อน;
      if (!FPC) fpcWhy = botFirst.replace(/\n/g, " ").slice(0, 70);
    }
  }
  // ── Intent Switch: ลูกค้าเปลี่ยนวิธีจัดส่งกลางทาง → บอทต้องเปลี่ยน Flow ต่อเนื่อง ──
  //   ผ่าน = ไม่ถามรุ่น/กลิ่น/จำนวนซ้ำ และไม่โยนให้คน หลังลูกค้าเปลี่ยนใจ
  let switchNeeded = 0, switchOK = 0;
  {
    const RX_TO_GRAB = /ส่งด่วน|แกร?[็ๆ]?[บป]|ไรเดอร์|เอาวันนี้|ด่วนเลย/i;
    const RX_TO_PARCEL = /ส่งพัสดุ|พัสดุปกติ|ธรรมดา|flash|เคอรี่|kerry/i;
    let โหมด = "";
    for (let i = 0; i < rows.length; i++) {
      const m = rows[i];
      if (bot(m)) continue;
      const t = txt(m);
      const ใหม่ = RX_TO_GRAB.test(t) ? "Grab" : RX_TO_PARCEL.test(t) ? "พัสดุ" : "";
      if (!ใหม่) continue;
      if (โหมด && ใหม่ !== โหมด) {
        switchNeeded++;
        const หลัง = rows.slice(i + 1).filter(bot).slice(0, 2).map(txt).join("\n");
        const ถามซ้ำอีก = RX_STAGE[1].test(หลัง) || RX_STAGE[2].test(หลัง);
        if (หลัง && !ถามซ้ำอีก && !RX_HANDOFF.test(หลัง)) switchOK++;
      }
      โหมด = ใหม่;
    }
  }
  const turns = rows.filter((m) => !bot(m)).length;
  const ปิดได้ = stage >= 7;
  return {
    เข้าใจตั้งแต่ข้อความแรก: FPC, เหตุที่ไม่เข้าใจ: fpcWhy,
    เปลี่ยนวิธีส่งกี่ครั้ง: switchNeeded, เปลี่ยนแล้วไปต่อได้: switchOK,
    เส้นทาง: (c && c.เส้นทาง) || "พัสดุ",
    ปิดได้L1: ปิดได้, ขั้นที่ไปถึง: FLOW_STAGES[Math.max(0, stage - 1)] || "ยังไม่เริ่ม", ขั้นเลข: stage,
    จำนวนเทิร์น: turns, เทิร์นที่ปิดได้: firstCloseIdx === -1 ? null : rows.slice(0, firstCloseIdx).filter((m) => !bot(m)).length,
    ถามซ้ำ: repeatAsk, ส่งต่อคน: handoff,
    พาออกนอกFlow: wrongTurn, เหตุพาออกนอกFlow: wtWhy,
    ต้องกู้คืน: recoveryNeeded, กู้คืนสำเร็จ: recoveryOK,
  };
}
// ── 30 บทวัด Order Flow (พัสดุ 15 · Grab 15) ──
//   Grab บทที่ 9-15 = จำลองว่าแอดมินกรอกค่าส่งแล้ว (ป้อน exp: ล่วงหน้า) เพื่อวัดว่า G2 เดินต่อเองได้ไหม
const FLOW_CASES = [
  { id:101, เส้นทาง:"พัสดุ", ชื่อ:"บอกครบในข้อความเดียว", ตา:3,
    เป้าหมาย:"สั่ง MARBO 9K กลิ่นองุ่นว่านหางจระเข้ 2 อัน ส่งพัสดุธรรมดา บอกครบในข้อความแรกเลย แล้วให้ชื่อ-เบอร์-ที่อยู่เมื่อถูกขอ",
    ต้องได้:"ออกการ์ดยอดและเลขบัญชีได้ภายใน 2-3 เทิร์น ไม่ถามซ้ำสิ่งที่บอกไปแล้ว", ห้าม:"ถามรุ่น/กลิ่น/จำนวนซ้ำ · โยนให้คน" },
  { id:102, เส้นทาง:"พัสดุ", ชื่อ:"บอกกระจาย 4 ข้อความ", ตา:6,
    เป้าหมาย:"ทยอยบอกทีละอย่าง: รุ่นก่อน แล้วกลิ่น แล้วจำนวน แล้วเลือกพัสดุ จากนั้นให้ชื่อ-เบอร์-ที่อยู่",
    ต้องได้:"จำทุกอย่างที่บอกไปแล้ว ปิดได้ถึงเลขบัญชี", ห้าม:"ถามซ้ำเรื่องที่เพิ่งบอก" },
  { id:103, เส้นทาง:"พัสดุ", ชื่อ:"ตอบจำนวนเป็นตัวเลขล้วน", ตา:5,
    เป้าหมาย:"บอกรุ่นกับกลิ่น พอถูกถามจำนวนตอบแค่ '2' เฉยๆ แล้วเลือกพัสดุ ให้ที่อยู่",
    ต้องได้:"เข้าใจว่า 2 คือจำนวน ผูกกับคำถามก่อนหน้า", ห้าม:"ตีความ 2 เป็นอย่างอื่น · ถามจำนวนซ้ำ" },
  { id:104, เส้นทาง:"พัสดุ", ชื่อ:"เปลี่ยนกลิ่นกลางทาง", ตา:6,
    เป้าหมาย:"สั่งกลิ่นหนึ่งไปแล้ว เปลี่ยนใจเป็นอีกกลิ่นก่อนโอน แล้วสั่งต่อจนจบ",
    ต้องได้:"ออกการ์ดใหม่ตามกลิ่นใหม่ ปิดได้", ห้าม:"ยังใช้กลิ่นเก่า · สร้างออเดอร์ซ้อน" },
  { id:105, เส้นทาง:"พัสดุ", ชื่อ:"เปลี่ยนจำนวนกลางทาง", ตา:6,
    เป้าหมาย:"สั่ง 1 อันก่อน แล้วขอเพิ่มเป็น 3 อัน แล้วให้ที่อยู่",
    ต้องได้:"ยอดคิดใหม่ตามจำนวนใหม่", ห้าม:"ยอดยังเป็นของเก่า" },
  { id:106, เส้นทาง:"พัสดุ", ชื่อ:"กลิ่นที่ขอหมด ต้องเสนอแทน", ตา:6,
    เป้าหมาย:"ขอกลิ่นที่ร้านหมด พอรู้ว่าหมดก็ให้เลือกกลิ่นอื่นที่มีของแล้วสั่งต่อจนจบ",
    ต้องได้:"บอกหมดตรงๆ เสนอกลิ่นที่มีจริง แล้วปิดได้", ห้าม:"บอกว่ามีทั้งที่หมด · เงียบ" },
  { id:107, เส้นทาง:"พัสดุ", ชื่อ:"พิมพ์ชื่อรุ่นที่ร้านไม่มี", ตา:4,
    เป้าหมาย:"พิมพ์ชื่อรุ่นมั่วที่ร้านไม่มี แล้วขอให้แอดมินแนะนำรุ่นที่มีแทน",
    ต้องได้:"ไม่เดาว่ามีหรือหมด ขอชื่อรุ่นให้ชัดหรือเสนอรุ่นที่มีจริง", ห้าม:"เดาสถานะสต็อกของรุ่นที่ไม่รู้จัก" },
  { id:108, เส้นทาง:"พัสดุ", ชื่อ:"ถามราคาก่อนแล้วค่อยสั่ง", ตา:5,
    เป้าหมาย:"ถามราคาต่อชิ้นก่อน พอพอใจแล้วค่อยสั่ง 2 อัน ส่งพัสดุ ให้ที่อยู่",
    ต้องได้:"ตอบราคาถูกต้อง แล้วเดินหน้าปิดการขาย", ห้าม:"ตอบราคาผิด · ตอบราคาแล้วไม่ชวนต่อ" },
  { id:109, เส้นทาง:"พัสดุ", ชื่อ:"ที่อยู่ไม่ครบ ขาดรหัสไปรษณีย์", ตา:6,
    เป้าหมาย:"สั่งของครบแล้วให้ที่อยู่แบบขาดรหัสไปรษณีย์ พอถูกทักก็ให้เพิ่ม",
    ต้องได้:"ทักเฉพาะช่องที่ขาด ไม่ขอที่อยู่ใหม่ทั้งชุด", ห้าม:"ขอที่อยู่ใหม่ทั้งหมด · ปล่อยผ่านทั้งที่ไม่ครบ" },
  { id:110, เส้นทาง:"พัสดุ", ชื่อ:"ถามยอดซ้ำ 2 รอบ", ตา:6,
    เป้าหมาย:"สั่งของแล้วถามยอดที่ต้องโอน 2 ครั้งด้วยคำถามต่างกัน",
    ต้องได้:"ตอบยอดเป็นตัวเลขเดิมทั้ง 2 ครั้ง", ห้าม:"ตอบยอดไม่ตรงกัน · เลี่ยงไม่บอกตัวเลข" },
  { id:111, เส้นทาง:"พัสดุ", ชื่อ:"เพิ่มสินค้าหลังสรุปยอด", ตา:6,
    เป้าหมาย:"สั่ง 1 อันจนได้การ์ดยอดแล้ว ขอเพิ่มอีกกลิ่น 1 อัน แล้วให้ที่อยู่",
    ต้องได้:"ออกการ์ดใหม่ที่มี 2 รายการ ยอดถูกต้อง", ห้าม:"ยอดไม่รวมของที่เพิ่ม" },
  { id:112, เส้นทาง:"พัสดุ", ชื่อ:"ลบสินค้าหลังสรุปยอด", ตา:6,
    เป้าหมาย:"สั่ง 2 กลิ่นจนได้ยอดแล้ว ขอตัดออก 1 กลิ่น แล้วให้ที่อยู่",
    ต้องได้:"ยอดลดลงตามของที่ตัดออก", ห้าม:"ยอดยังรวมของที่ตัดไปแล้ว" },
  { id:113, เส้นทาง:"พัสดุ", ชื่อ:"พิมพ์ยืนยันซ้ำ 3 ครั้ง", ตา:6,
    เป้าหมาย:"สั่งของครบแล้วพิมพ์ 'ยืนยัน' ซ้ำ 3 ครั้งติดกัน",
    ต้องได้:"ไม่สร้างออเดอร์ซ้ำ ไม่ตอบข้อความเดิมเป๊ะซ้ำๆ", ห้าม:"สร้างออเดอร์ซ้อน · ส่งเลขบัญชีซ้ำรัว" },
  { id:114, เส้นทาง:"พัสดุ", ชื่อ:"ถามเก็บปลายทางก่อนสั่ง", ตา:5,
    เป้าหมาย:"ถามว่ามีเก็บเงินปลายทางไหม พอรู้ว่าไม่มีก็ยอมโอนแล้วสั่งต่อจนจบ",
    ต้องได้:"บอกว่าไม่มีปลายทางแล้วชวนโอนต่อทันที ปิดได้", ห้าม:"ตอบว่ามีปลายทาง · ตอบแล้วปล่อยบทจบ" },
  { id:115, เส้นทาง:"พัสดุ", ชื่อ:"สั่ง 4 ชิ้นเข้าโปรส่งฟรี", ตา:5,
    เป้าหมาย:"สั่งพอตใช้แล้วทิ้ง 4 แท่งคละกลิ่นที่มีของ ส่งพัสดุ ให้ที่อยู่",
    ต้องได้:"ยอดต้องไม่บวกค่าส่ง 40 เพราะเข้าโปรส่งฟรีแล้ว", ห้าม:"บวกค่าส่งทั้งที่เข้าโปร" },

  { id:201, เส้นทาง:"Grab", ชื่อ:"บอกครบข้อความเดียว + ขอส่งด่วน", ตา:4,
    เป้าหมาย:"สั่งของครบพร้อมบอกว่าเอาส่งด่วนแกร็บ แล้วให้ชื่อ-เบอร์ และแชร์พิกัดเมื่อถูกขอ",
    ต้องได้:"เก็บของครบแล้วขอพิกัดเพื่อเช็คค่าส่ง", ห้าม:"ขอที่อยู่แบบพัสดุ · เดาค่าส่งเอง" },
  { id:202, เส้นทาง:"Grab", ชื่อ:"บอกกระจายหลายข้อความ", ตา:6,
    เป้าหมาย:"ทยอยบอกรุ่น กลิ่น จำนวน แล้วค่อยบอกว่าเอาส่งด่วน จากนั้นให้ชื่อ-เบอร์",
    ต้องได้:"จำทุกอย่างแล้วขอพิกัด", ห้าม:"ถามซ้ำ" },
  { id:203, เส้นทาง:"Grab", ชื่อ:"ถามค่าส่งด่วนก่อนส่งพิกัด", ตา:5,
    เป้าหมาย:"สั่งของแล้วถามว่าค่าส่งด่วนเท่าไหร่ ทั้งที่ยังไม่ได้ส่งพิกัด",
    ต้องได้:"บอกว่าต้องขอพิกัดก่อนถึงเช็คได้ ไม่เดาตัวเลข", ห้าม:"ตอบค่าส่งด่วนเป็นตัวเลขทั้งที่ยังไม่มีพิกัด" },
  { id:204, เส้นทาง:"Grab", ชื่อ:"กลิ่นหมดระหว่างทาง", ตา:6,
    เป้าหมาย:"ขอส่งด่วน สั่งกลิ่นที่หมด พอรู้ก็เปลี่ยนเป็นกลิ่นที่มี แล้วให้ชื่อ-เบอร์",
    ต้องได้:"บอกหมดถูกต้อง เปลี่ยนกลิ่นแล้วเดินต่อขอพิกัด", ห้าม:"บอกว่ามีทั้งที่หมด" },
  { id:205, เส้นทาง:"Grab", ชื่อ:"ให้ชื่อแต่ไม่ให้เบอร์", ตา:6,
    เป้าหมาย:"สั่งของขอส่งด่วน ให้แค่ชื่อ ไม่ให้เบอร์ พอถูกทักก็ให้เบอร์",
    ต้องได้:"ทักขอเฉพาะเบอร์ที่ขาด", ห้าม:"ขอชื่อซ้ำ" },
  { id:206, เส้นทาง:"Grab", ชื่อ:"ถามรอบส่งด่วน", ตา:5,
    เป้าหมาย:"สั่งของขอส่งด่วน แล้วถามว่ารอบส่งกี่โมง จะได้ของกี่โมง",
    ต้องได้:"ตอบตารางรอบส่งได้ แล้วเดินหน้าขอพิกัดต่อ", ห้าม:"รับปากเวลาที่ทำไม่ได้" },
  { id:207, เส้นทาง:"Grab", ชื่อ:"อยู่ต่างจังหวัดแต่ขอส่งด่วน", ตา:5,
    เป้าหมาย:"บอกว่าอยู่เชียงใหม่แต่อยากได้ส่งด่วนวันนี้",
    ต้องได้:"บอกตรงๆ ว่าส่งด่วนเฉพาะ กทม./ปริมณฑล แล้วเสนอพัสดุแทน", ห้าม:"รับปากส่งด่วนต่างจังหวัด" },
  { id:208, เส้นทาง:"Grab", ชื่อ:"ขอส่งด่วนก่อนเลือกสินค้า", ตา:5,
    เป้าหมาย:"เปิดมาบอกว่าจะเอาส่งด่วนเลย ยังไม่บอกว่าเอาอะไร แล้วค่อยเลือกของทีหลัง",
    ต้องได้:"ถามว่าจะรับอะไรก่อน ไม่ขอที่อยู่/พิกัดตั้งแต่ยังไม่รู้ของ", ห้าม:"ขอพิกัดหรือที่อยู่ทั้งที่ยังไม่รู้ว่าลูกค้าจะเอาอะไร" },
  { id:209, เส้นทาง:"Grab", ชื่อ:"ค่าส่งเข้าระบบแล้ว → ต้องสรุปต่อเอง", ตา:4, ค่าส่งด่วน:120,
    เป้าหมาย:"สั่งของส่งด่วนโดยที่ทีมงานเช็คค่าส่งให้แล้ว ถามว่าตกลงต้องโอนเท่าไหร่",
    ต้องได้:"บอกยอดรวมที่รวมค่าส่งด่วนแล้ว และออกเลขบัญชีให้", ห้าม:"บอกว่ายังรอทีมงานเช็คค่าส่ง · ยอดไม่รวมค่าส่งด่วน" },
  { id:210, เส้นทาง:"Grab", ชื่อ:"รู้ค่าส่งแล้วถามยอดซ้ำ", ตา:5, ค่าส่งด่วน:150,
    เป้าหมาย:"ค่าส่งด่วนเช็คแล้ว ถามยอดที่ต้องโอน 2 รอบด้วยคำถามต่างกัน",
    ต้องได้:"ตอบยอดเดียวกันทั้ง 2 รอบ รวมค่าส่งด่วนถูกต้อง", ห้าม:"ยอดไม่ตรงกัน" },
  { id:211, เส้นทาง:"Grab", ชื่อ:"เพิ่มของหลังรู้ค่าส่ง", ตา:5, ค่าส่งด่วน:100,
    เป้าหมาย:"ค่าส่งด่วนเช็คแล้ว ขอเพิ่มสินค้าอีก 1 ชิ้น แล้วถามยอดใหม่",
    ต้องได้:"ยอดใหม่ = ของเดิม + ของเพิ่ม + ค่าส่งด่วนเดิม", ห้าม:"ลืมบวกของที่เพิ่ม · เปลี่ยนค่าส่งเอง" },
  { id:212, เส้นทาง:"Grab", ชื่อ:"เปลี่ยนจาก Grab เป็นพัสดุ", ตา:5, ค่าส่งด่วน:180,
    เป้าหมาย:"รู้ค่าส่งด่วนแล้วรู้สึกแพง เลยขอเปลี่ยนเป็นส่งพัสดุธรรมดาแทน แล้วให้ที่อยู่",
    ต้องได้:"เปลี่ยนเป็นค่าส่ง 40 คิดยอดใหม่ แล้วปิดได้", ห้าม:"ยังคิดค่าส่งด่วน · ต้องให้คนมาแก้" },
  // ⚠️ นับเป็น Grab เพราะ "ปลายทางที่จบ" คือส่งด่วน — ถ้านับเป็นพัสดุจะทำให้ KPI พัสดุเพี้ยน
  { id:213, เส้นทาง:"Grab", ชื่อ:"เปลี่ยนจากพัสดุเป็น Grab", ตา:5,
    เป้าหมาย:"สั่งแบบพัสดุจนได้ยอดแล้ว เปลี่ยนใจอยากได้วันนี้เลยขอเปลี่ยนเป็นส่งด่วน",
    ต้องได้:"เปลี่ยนเส้นทางให้แล้วขอพิกัด ไม่ยึดยอดพัสดุเดิม", ห้าม:"ยังใช้ค่าส่ง 40 กับส่งด่วน" },
  { id:214, เส้นทาง:"Grab", ชื่อ:"ยืนยันซ้ำหลังรู้ค่าส่ง", ตา:5, ค่าส่งด่วน:90,
    เป้าหมาย:"ค่าส่งด่วนเช็คแล้ว พิมพ์ยืนยันซ้ำ 2 ครั้ง",
    ต้องได้:"ไม่สร้างออเดอร์ซ้ำ ยอดคงเดิม", ห้าม:"ออเดอร์ซ้อน" },
  { id:215, เส้นทาง:"Grab", ชื่อ:"ถามสถานะหลังได้เลขบัญชี", ตา:5, ค่าส่งด่วน:110,
    เป้าหมาย:"ค่าส่งเช็คแล้ว ได้เลขบัญชีแล้ว ถามว่าตอนนี้ออเดอร์ถึงไหนแล้ว",
    ต้องได้:"บอกสถานะว่ารอโอน และบอกยอดที่ต้องโอนได้", ห้าม:"บอกว่าจ่ายแล้วทั้งที่ยังไม่โอน" },
];
...
// k191: เพิ่มพารามิเตอร์ "เตรียม" (ไม่บังคับ) — ใช้ป้อนสถานะตั้งต้นก่อนเริ่มคุย
//   จำเป็นสำหรับเทส Grab ครึ่งหลัง: จำลองว่าแอดมินกรอกค่าส่งด่วนไปแล้ว
//   ⛔ ผู้เรียกเดิมไม่ส่งพารามิเตอร์นี้ → พฤติกรรมเดิมไม่เปลี่ยนเลย
async function evalConverse(env, ถามข้อถัดไป, จำนวนตา, เตรียม) {
  ...
  const conv = [];
  try {
    if (typeof เตรียม === "function") { try { await เตรียม(eenv, uid); } catch (e) {} }
...
// ── k191: รัน 1 บทของ Order Flow Test แล้ววัด KPI ──
//   ใช้ตัวจำลองลูกค้า + กรรมการชุดเดิม (ไม่แตะ) แล้วเพิ่มตัวนับ KPI ฝั่งโค้ด
async function evalRunFlow(env, c) {
  const t0 = Date.now();
  const เตรียม = (c.ค่าส่งด่วน)
    ? async (eenv, uid) => {   // จำลองว่าแอดมินเช็คค่าส่งด่วนจากแอปแล้วกรอกเข้าระบบ (ทาง A)
        try { await eenv.CONV.put("exp:evalsim:" + uid, JSON.stringify({ fee: c.ค่าส่งด่วน, km: 8, lat: 13.75, lng: 100.5, pending: false, t: Date.now() })); } catch (e) {}
      }
    : null;
  const conv = await evalConverse(env, (cv) => evalCustomer(env, c, cv), Math.max(2, Math.min(7, c.ตา || 5)), เตรียม);
  const g = await evalGrade(env, c, conv, t0);
  const r = g.ผลลัพธ์;
  r.KPI = flowKPI(conv, c);
  if (c.ค่าส่งด่วน) r.KPI.ค่าส่งด่วนที่ป้อนไว้ = c.ค่าส่งด่วน;
  return r;
}
// รวม KPI หลายบทเป็นภาพเดียว — ใช้ทั้งใน endpoint และหน้าแดชบอร์ด
function flowSummary(rows) {
  const ok = (rows || []).filter((x) => x && x.KPI);
  const g = (f) => ok.filter(f);
  const pct = (a, b) => (b ? Math.round((a / b) * 1000) / 10 : 0);
  const กลุ่ม = (ชื่อ, ลิสต์) => {
    const ปิด = ลิสต์.filter((x) => x.KPI.ปิดได้L1).length;
    const เทิร์น = ลิสต์.filter((x) => x.KPI.เทิร์นที่ปิดได้ != null).map((x) => x.KPI.เทิร์นที่ปิดได้);
    return { เส้นทาง: ชื่อ, บท: ลิสต์.length, ปิดได้L1: ปิด, "ปิดได้%": pct(ปิด, ลิสต์.length),
      เทิร์นเฉลี่ยถึงปิด: เทิร์น.length ? Math.round((เทิร์น.reduce((a, b) => a + b, 0) / เทิร์น.length) * 10) / 10 : null };
  };
  const drop = {};
  for (const s of FLOW_STAGES) drop[s] = 0;
  for (const x of ok) drop[x.KPI.ขั้นที่ไปถึง] = (drop[x.KPI.ขั้นที่ไปถึง] || 0) + 1;
  const dropPct = {};
  for (const k in drop) dropPct[k] = { บท: drop[k], "%": pct(drop[k], ok.length) };
  const ต้องกู้ = ok.reduce((a, x) => a + x.KPI.ต้องกู้คืน, 0);
  const กู้ได้ = ok.reduce((a, x) => a + x.KPI.กู้คืนสำเร็จ, 0);
  const acc = ok.filter((x) => x.คะแนน);
  return {
    บททั้งหมด: ok.length,
    OrderCompletionRate: [กลุ่ม("พัสดุ", g((x) => x.KPI.เส้นทาง === "พัสดุ")), กลุ่ม("Grab", g((x) => x.KPI.เส้นทาง === "Grab"))],
    OrderAccuracy: acc.length ? Math.round(acc.reduce((a, x) => a + (x.คะแนน.order_accuracy || 0), 0) / acc.length) : null,
    MissingInfoRate: { บทที่ถามซ้ำ: g((x) => x.KPI.ถามซ้ำ > 0).length, "%": pct(g((x) => x.KPI.ถามซ้ำ > 0).length, ok.length) },
    ConversationLoop: { บทที่วน: g((x) => x.KPI.ถามซ้ำ >= 2).length, "%": pct(g((x) => x.KPI.ถามซ้ำ >= 2).length, ok.length) },
    HumanHandoffRate: { บท: g((x) => x.KPI.ส่งต่อคน).length, "%": pct(g((x) => x.KPI.ส่งต่อคน).length, ok.length) },
    WrongTurnRate: { บท: g((x) => x.KPI.พาออกนอกFlow).length, "%": pct(g((x) => x.KPI.พาออกนอกFlow).length, ok.length),
      ตัวอย่าง: g((x) => x.KPI.พาออกนอกFlow).slice(0, 3).map((x) => ({ เคส: x.เคส, ที่พลาด: x.KPI.เหตุพาออกนอกFlow })) },
    RecoveryRate: { ครั้งที่ต้องกู้: ต้องกู้, กู้สำเร็จ: กู้ได้, "%": pct(กู้ได้, ต้องกู้) },
    FirstPassCompletion: (() => {
      const วัดได้ = ok.filter((x) => x.KPI.เข้าใจตั้งแต่ข้อความแรก !== null);
      const ผ่าน = วัดได้.filter((x) => x.KPI.เข้าใจตั้งแต่ข้อความแรก === true);
      return { บทที่วัดได้: วัดได้.length, ผ่าน: ผ่าน.length, "%": pct(ผ่าน.length, วัดได้.length),
        ตัวอย่างที่ไม่ผ่าน: วัดได้.filter((x) => x.KPI.เข้าใจตั้งแต่ข้อความแรก === false).slice(0, 3)
          .map((x) => ({ เคส: x.เคส, ถามย้อน: x.KPI.เหตุที่ไม่เข้าใจ })),
        หมายเหตุ: "นับเฉพาะบทที่ข้อความแรกของลูกค้าให้ข้อมูลครบพอ (รุ่น + กลิ่นหรือจำนวน)" };
    })(),
    IntentSwitch: (() => {
      const ต้อง = ok.reduce((a, x) => a + x.KPI.เปลี่ยนวิธีส่งกี่ครั้ง, 0);
      const ได้ = ok.reduce((a, x) => a + x.KPI.เปลี่ยนแล้วไปต่อได้, 0);
      return { ครั้งที่ลูกค้าเปลี่ยนวิธีส่ง: ต้อง, เปลี่ยนแล้วไปต่อได้: ได้, "%": pct(ได้, ต้อง),
        หมายเหตุ: "ผ่าน = ไม่ถามรุ่น/กลิ่น/จำนวนซ้ำ และไม่โยนให้คน หลังลูกค้าเปลี่ยนวิธีส่ง" };
    })(),
    DropOffPoint: dropPct,
    หมายเหตุ: "ปิดได้L1 = ออกการ์ดยอด+เลขบัญชี+เก็บออเดอร์รอโอน · สนามซ้อมส่งรูปสลิปไม่ได้ จึงวัด L2 (สลิปผ่าน+ปิดออเดอร์จริง) ไม่ได้",
  };
}
  ...
  try {
      ...
      if (_mv) {
        ...
        if (!_qOnly) {
          ...
          if (mtype === "text" && /#?เปิดบอท|#bot/i.test(ev.message.text)) {
            ...
            await lineReply(TOKEN, replyToken, "แอดมินกลับมาดูแลต่อแล้วค่ะ ✨ สอบถามได้เลยนะคะ 💕", userId);
          }
    ...
    if (mtype === "text" && env.CONV) {
      ...
      if (_askTotal) {
        try {
          ...
          if (_ov) {
            const _msg = totalMsg(JSON.parse(_ov));
            if (_msg) {
    ...
    if (mtype === "sticker") {
      ...
      if (/thank|thanks|thank you|grateful/.test(kw)) {
        msg = "ยินดีค่ะ 💕 มีอะไรให้แอดมินช่วยอีกไหมคะ";
      } else if (/ok|okay|yes|agree|nod|got it|understand/.test(kw)) {
        msg = "รับทราบค่ะ 💕";
      } else if (/hello|hi|greeting|welcome|wave/.test(kw)) {
        msg = "สวัสดีค่ะ 💕 สนใจสินค้าตัวไหน บอกแอดมินได้เลยนะคะ";
      } else if (/sad|cry|angry|sorry|upset/.test(kw)) {
        msg = "มีอะไรให้แอดมินช่วยไหมคะ 🙏🏻 พิมพ์บอกได้เลยค่ะ";
      } else {
        // สติกเกอร์ทั่วไป — ตอบสั้นๆ ให้รู้ว่าเห็นแล้ว แล้วชวนคุยต่อ
        msg = "รับทราบค่ะ 💕 สนใจสินค้าตัวไหน หรืออยากดูเมนู บอกแอดมินได้เลยนะคะ";
      }
    ...
    if (mtype === "text") {
      ...
      if ((_looseConfirm || /^ยืนยัน(รายการ)?(เดิม)?[\s!.]*$/.test(t) || /ขอเลขบัญชี|เลขบัญชี|โอนเข้า(บัญชี)?ไหน|บัญชีอะไร|โอนไปไหน|โอนบัญชีไหน/.test(t)) && env.CONV) {
        try {
          ...
          if (ok) {
            ...
            if (_stale) {
              // 💰 k168: เดิมกรองบรรทัดด้วย /x\d|รวมยอด/ → ถ้า ord ไม่มีบรรทัดยอดรวม
              //   ลูกค้าจะเห็นแค่ "MARBO 9K เยลลี่ x1 = 350" แล้วโอน 350 ทั้งที่ต้องโอน 509
              //   ตอนนี้ใช้ totalMsg() ตัวเดียวกับที่อื่น — มียอดรวมเสมอ หรือบอกตรงๆ ว่ายังไม่สรุป
              const _tm = totalMsg(JSON.parse(ok));
              const _b2 = _tm || (JSON.parse(ok).block || "").split("\n").filter(l => /x\d|รวมยอด|ยอดรวม/.test(l)).slice(0, 6).join("\n");
              await lineReply(TOKEN, replyToken,
      ...
      if ((/แอดมิน/.test(t) && /ติดต่อ|คุย|ขอ|เรียก|หา|อยู่ไหม|อยู่ไหน|อยู่มั้ย|อยู่ป่าว|หน่อย|ช่วย/.test(t)) || /คุยกับคน|คนจริง|เจ้าหน้าที่|พนักงาน|ขอสายด่วน/.test(t)) {
        await muteNow("ขอคุยแอดมิน", t); // ส่งต่อให้คน — แอดมินเงียบแชทนี้
        await lineReply(TOKEN, replyToken, "รับเรื่องแล้วค่ะ เดี๋ยวทีมงานเข้ามาดูแลนะคะ รอสักครู่ค่ะ 🙏🏻💕", userId);
        return;
      }
      // 📦 k24: "ของเข้าวันไหน" → ตอบตายตัว ⛔ ห้ามให้ AI ตอบ เพราะมันจะกุวันเอง
      // เคสจริง 30/7: แอดมินตอบ "รอของเข้าประมาณ 3-5 วันทำการ" ทั้งที่ร้านไม่เคยบอก = สัญญาที่รักษาไม่ได้
      ...
      if (/ของเข้า(วันไหน|เมื่อไหร่|เมื่อไร|ตอนไหน|กี่โมง|กี่วัน|หรือยัง|รึยัง|ยัง)|เมื่อไหร่มีของ|เมื่อไรมีของ|of เข้า|ของจะเข้า|รอของเข้า|กี่วันของเข้า|มีของเมื่อไหร่|เข้าเมื่อไหร่|(?<!แอดมิน)เข้าตอนไหน|(?<!แอดมิน)เข้ากี่โมง|(เมื่อไหร่|เมื่อไร)(จะ)?เติมของ|เติมของ(เมื่อไหร่|เมื่อไร|ตอนไหน|กี่โมง|วันไหน)|สินค้า(จะ)?เข้า(วันไหน|เมื่อไหร่|ตอนไหน)|restock/i.test(t)) {
        await lineReply(TOKEN, replyToken,
          "ต้องขออภัยด้วยนะคะ 🙏🏻 ทางร้านยังระบุวันที่ของจะเข้าแน่นอนไม่ได้ค่ะ\n\nรบกวนทักเข้ามาเช็คใหม่ได้เรื่อยๆ นะคะ แอดมินเช็คสต็อกให้ได้ตลอด 24 ชม.เลยค่ะ 💕\n\nหรือถ้าไม่อยากรอ บอกแนวกลิ่นที่ชอบมาได้ค่ะ (เช่น หวานผลไม้ / เย็นมิ้นต์ / โคล่า) เดี๋ยวแอดมินหากลิ่นที่มีของตอนนี้ให้แทนนะคะ 😊", userId);
        return;
      }
      ...
      //   เหตุที่เคยทำเป็นทางลัด: แอดมินเคยตอบ "20.45 น. จะได้รับวันนี้แน่นอนค่ะ" (รับปาก + ไม่บอกว่าเฉพาะ กทม.)
      //   ตอนนี้กันด้วยกฎ + ตัวกรองขาออกแทน
      // ⏰ k134 (เคสจริง 3/8 17.56): ลูกค้าปักหมุดแล้ว แอดมินแจ้งค่าส่ง 123 แล้ว การ์ด 823 ออกแล้ว
      //   ลูกค้าถาม "ได้ของกี่โมง" → ระบบดันตอบ "รบกวนแชร์โลเคชั่น เดี๋ยวทีมงานเช็คค่าส่งด่วน" = ย้อนกลับไป 3 ก้าว
      //   ต้นเหตุ: "ได้ของกี่โมง" ไม่เข้าทางลัดรอบส่ง เลยตกไปทางตอบเรื่องส่งด่วนทั่วไปที่ขอหมุดเสมอ
      //   กฎ: รู้ค่าส่งด่วน/มีหมุดแล้ว → ห้ามขอหมุดซ้ำเด็ดขาด ตอบตารางรอบส่งไปเลย
      {
        const askWhen = /ได้ของ(กี่โมง|ตอนไหน|เมื่อไหร่|เมื่อไร|วันไหน)|ของถึงกี่โมง|ถึงกี่โมง|กี่โมงถึง|กี่โมงได้|ได้กี่โมง|(เมื่อไหร่|เมื่อไร|ตอนไหน)(จะ)?ได้ของ|ส่งถึงกี่โมง|รอนานไหม|ใช้เวลานานไหม/.test(t);
        let _exW = null;
        try { if (env.CONV) { const v = await env.CONV.get("exp:" + shopId + ":" + userId); if (v) _exW = JSON.parse(v); } } catch (e) {}
        const _efW = EXPFEE.get(shopId + ":" + userId);
        const knowExp = (_efW && Date.now() - _efW.t < 7200000) || (_exW && (typeof _exW.fee === "number" || _exW.lat));
        if (askWhen && knowExp && !(_exW && _exW.outzone)) {
          console.log("K134_WHEN_EXPRESS ตอบตารางรอบ ไม่ขอหมุดซ้ำ");
      ...
      // 🚚🚚 k226 — "ส่งกี่วันถึง" คือคำถามที่ลูกค้าถามบ่อยที่สุดที่ระบบไม่มีทางตอบ
      //   หลักฐาน: ขุดแชทจริง 21,301 คู่ (ลูกค้าถาม → แอดมินคนตอบ)
      //     กลุ่ม "ถามจัดส่ง/เวลา" = **976 ครั้งที่ไม่มีกลไกไหนจับได้เลย** ครอบคลุมแค่ 12%
      //     มากที่สุดในทุกกลุ่ม และเป็นคำถามก่อนตัดสินใจซื้อ ตอบช้า/ตอบมั่ว = เสียออเดอร์
      //   ⚠️ ที่เจอตอนขุด: ข้อความ ETA_MSG มีอยู่ในโค้ดแล้วตั้งแต่แรก **แต่ไม่เคยถูกเรียกใช้เลยสักที่**
      //     คำตอบที่ถูกต้องนอนอยู่เฉย ๆ ไม่มีใครต่อสาย — ต่อสายให้มันแค่นั้น ไม่ได้แต่งข้อมูลใหม่
      //   ⛔ ต้องไม่แย่งงานของด่านอื่น
      //     · "ของถึงไหนแล้ว / ส่งหรือยัง" = ตามพัสดุ → ให้ afterSaleIntent จัดการ
      //     · ถามเวลาส่งด่วนตอนรู้พิกัดแล้ว → ให้บล็อก askWhen ด้านบนจัดการ
      if (/(?:ส่ง|จัดส่ง|ของ|พัสดุ|ได้ของ|ถึง)[^\n]{0,10}(?:กี่วัน|กี่ชม|กี่ชั่วโมง|นานไหม|นานมั้ย|นานแค่ไหน)|(?:กี่วัน|กี่ชม|กี่ชั่วโมง)[^\n]{0,10}(?:ถึง|ได้|ส่ง)|ใช้เวลา(?:กี่วัน|นาน|ส่ง|จัดส่ง)|ระยะเวลา(?:จัดส่ง|ส่ง|ขนส่ง)|ส่งของ(?:กี่วัน|เมื่อไหร่|วันไหน)|สั่งวันนี้[^\n]{0,12}(?:ได้|ถึง|ส่ง)/.test(t)
        && !afterSaleIntent(t)                                   // ตามพัสดุ = คนละเรื่อง
        && !/เลขพัสดุ|เลขติดตาม|ถึงไหน|ส่งหรือยัง|ส่งยัง|ได้ของยัง/.test(t)
        // ⛔ ลูกค้าบอกพื้นที่มาด้วย ("ผมอยู่หลักสี่ สั่งวันนี้จะได้วันไหน")
        //    = ถามรอบส่งด่วนของพื้นที่นั้น ไม่ใช่ถามระยะเวลาทั่วไป → ปล่อยให้เส้นส่งด่วนตอบ
        && !BKK_ZONE.test(t) && !PERI_ZONE.test(t) && !(() => { try { return !!matchUpcountry(t); } catch (e) { return false; } })()) {
        console.log("K226_ETA_ANSWER");
        await lineReply(TOKEN, replyToken, ETA_MSG, userId);
        return;
      }
      // 🎁 k35: ถามโปรโมชั่น → ตอบตายตัว ⛔ ห้าม AI ตอบ (เคสจริง 31/7: ถาม "มีโปรอะไรบ้าง" แล้วแอดมินตอบ "รอทีมงานหลังการขาย")
      ...
      if ((/โปร(โมชั่น|โมชัน)?\s*(อะไร|ไหน|มี|บ้าง|ตอนนี้)|มีโปร|ส่วนลด|ลดราคา|ซื้อ.{0,6}แถม|โปรอะไร/.test(t)
        ...
        // 🎯 k106 (เจ้าของร้านสั่ง 2/8): ถ้ากำลังคุยรุ่นไหนอยู่ → บอกโปรของรุ่นนั้นก่อน ปิดการขายง่ายกว่ายิงลิสต์ยาว
        //   เคสจริง: คุย "ไส้บุหรี่ IQOS JP" อยู่ แล้วถาม "มีโปรโมชั่นไหม" → ควรตอบ "ครบ 2 ชิ้น ส่งฟรี" ทันที
        let _pm = PROMO_MSG;
        try {
          let _hist = [];
          ...
          if (_mdl) {
            // k106c (เจ้าของร้านแก้ 2/8): "หัว M SWITCH = บิ๊กพอต ครบ 4 หัวส่งฟรี" ไม่ใช่หัวเล็ก 10 หัว
            //   → ให้ระบบจัดหมวดตามตารางสินค้าจริง (catOf) อย่างเดียว ห้ามเดาจากคำว่า "หัว"
            let _cat = catOf(_mdl);
            ...
            const _rule = _cat === "iqos" ? { n: "2 ชิ้น", t: "ไส้บุหรี่ IQOS" }
                        : _cat === "smallpod" ? { n: "10 หัว", t: "หัวพอตเล็ก" }
                        : _cat === "bigpod" ? { n: "4 หัว/ชิ้น", t: "บิ๊กพอต · หัวน้ำยาใหญ่ · ชุด KIT" }
                        : _cat === "disp" ? { n: "4 แท่ง", t: "พอตใช้แล้วทิ้ง" } : null;
            if (_no) {
              _pm = "สำหรับ " + _mdl + " ขออนุญาตแจ้งตามจริงนะคะ 🙏🏻\n⚠️ รุ่นนี้ไม่ร่วมโปรส่งฟรีค่ะ (เครื่อง IQOS · น้ำยาขวด · นิโคตินพอช)\n\n🚚 ค่าส่งพัสดุปกติ 40 บาท ทั่วประเทศค่ะ\n\nสนใจรับกี่ชิ้นดีคะ 💕";
            } else if (_rule) {
              _pm = "โปรของ " + _mdl + " ค่ะ 🎁\n\n📦 ซื้อครบ " + _rule.n + " = ส่งฟรีเลยค่ะ (" + _rule.t + ")\n(ยังไม่ครบ = ค่าส่งพัสดุปกติ 40 บาทค่ะ)\n\n⚠️ โปรส่งฟรีใช้กับพัสดุปกตินะคะ ส่งด่วนคิดค่าส่งตามระยะทางค่ะ\n\nสนใจรับกี่ชิ้นดีคะ 💕";
            }
          }
        } catch (e) {}
        await lineReply(TOKEN, replyToken, _pm, userId);
        return;
      }
      ...
      try {
        if (env.CONV) {
          ...
          // 🐛 k200 (เคสจริง 5/8 18.58): ลูกค้าพิมพ์ "ขอเปลี่ยนสินค้าค่ะ" **ตอนยังไม่เคยสั่งอะไรเลย**
          //   → ระบบตีเป็นเคสหลังการขาย ส่งเข้าคน แล้วเงียบใส่ลูกค้าอีก 6 ข้อความติด (เสียการขายจริง)
          //   ความจริง: "เปลี่ยนสินค้า / เคลมได้ไหม" ตอนยังไม่มีออเดอร์ = ลูกค้ากำลังเลือกของอยู่
          //   กฎใหม่: หมวด "เคลม/เปลี่ยน/คืน" และ "แก้ข้อมูลจัดส่ง" ต้องมี **ออเดอร์อยู่จริง** ถึงนับเป็นหลังการขาย
          //   ⛔ หมวด "ปัญหาสินค้า" กับ "ตามพัสดุ" ไม่ต้องมีออเดอร์ก็ส่งต่อได้ (ลูกค้าเดือดร้อนจริง ห้ามให้รอ)
          if (!_asOrd && /เคลม|แก้ข้อมูลจัดส่ง/.test(_asIntent)) {
            console.log("K200_AS_SKIP_NO_ORDER " + _asIntent);
            _asIntent = "";
          }
      ...
      //    แล้วแอดมินปิดท้ายว่า "อยากได้กลิ่นไหน" + กุวิธี "กด *1*" ที่ร้านไม่มีจริง)
      // ✅ ยกเว้นการ "ถามเงื่อนไข" (เคลมยังไง/กี่วัน) → ปล่อยให้ตอบข้อมูลตามปกติ ไม่ต้องส่งต่อ
      {
        // 🙋 k137 (เคสจริง 3/8 18.29 — หนักสุดของวัน): ลูกค้าขอคุยกับคนจริง 3 รอบ
        //   "เรียกมาเลยค่ะ" → "ส่งแต่อันเดิมๆคุยไม่รู้เรื่อง จะมาหรือยังคะ" → "มายังคะ"
        //   บอทตอบ "ทีมงานกำลังเดินทางมาแล้วค่ะ" ทั้งที่ **ไม่เคยส่งเข้าคิวแอดมินเลยสักครั้ง**
        //   = โกหกลูกค้าเต็มๆ ลูกค้ารอเก้อ แล้วไม่มีใครรู้ว่ามีคนรออยู่
        //   กฎเหล็ก: ลูกค้าขอคุยกับคน = ส่งเข้าคิวทันที ห้ามรับปากลอยๆ เด็ดขาด
        const wantHuman = /เรียก(มา|ให้|คน|แอดมิน)|ขอคุยกับคน|คุยกับคน|คนจริง|ขอแอดมิน|มีแอดมิน|ต่อแอดมิน|ขอคุยกับแอดมิน|เรียกแอดมิน|ตอบเหมือน\s*(เอไอ|ai|บอท)|เป็นบอท|นี่บอท|คุยกับบอท|ไม่ใช่คน|จะมาหรือยัง|มายัง|เมื่อไหร่จะมา|คุยไม่รู้เรื่อง|ส่งแต่อันเดิม/i.test(t);
        if (wantHuman) {
          await muteNow("🙋 ลูกค้าขอคุยกับคนจริง — ต้องเข้าไปคุยด่วน", t);
          await lineReply(TOKEN, replyToken,
            "รับเรื่องแล้วนะคะ 🙏🏻 แอดมินส่งต่อให้ทีมงานคนจริงเข้ามาดูแลแล้วค่ะ\nรบกวนรอสักครู่นะคะ 💕", userId);
          return;
        }
        // 💸 k127 (เคสจริง 3/8 15.05): ลูกค้าพิมพ์ "โอนตังค์คืนได้ไหมไม่อยากได้แล้ว"
        //   ระบบตอบ "ไม่เข้าใจคำถาม" + ส่งลิงก์เมนู → ลูกค้าถามซ้ำ → ตอบแบบเดิม → ลูกค้าโกรธ "ไม่ให้อภัยตอบมา"
        //   2 ต้นเหตุ: (1) คำว่า "โอนตังค์คืน" ไม่อยู่ในลิสต์ (มีแต่ "คืนเงิน")
        //              (2) มีคำว่า "ได้ไหม" → ระบบตีเป็น "ถามเงื่อนไขเฉยๆ" เลยไม่ส่งต่อคน
        //   ⛔ เรื่องเงินคืน = ต้องให้คนตัดสินเสมอ ไม่ว่าจะถามด้วยคำว่า "ได้ไหม" หรือไม่
        // 💸💸 k163 (เคสจริง 4/8 12.29 — ลูกค้า ออยออย):
        //   ลูกค้าถามหาของ 3 รุ่นติดแล้วเจอ "หมด" ทุกรุ่น เลยพิมพ์ "ถ้าหมดอีกไม่เอาแล้วค่ะ"
        //   → ระบบจับคำว่า "ไม่เอาแล้ว" เป็น **ขอเงินคืน** → ตอบ "เรื่องคืนเงิน/ยกเลิกรายการ
        //     ส่งต่อทีมงาน" + เด้งเคสด่วนเข้าหลังบ้าน — ทั้งที่ลูกค้าไม่เคยสั่ง ไม่เคยจ่ายเงินสักบาท
        //   ผล: ลูกค้างง · แอดมินได้เคสด่วนปลอม · เสียโอกาสขาย (ควรเสนอทางเลือกแทน)
        //   กฎใหม่: คำอ้อมๆ ("ไม่เอาแล้ว") = ขอเงินคืน **ก็ต่อเมื่อมีเงินอยู่จริง** (มีออเดอร์ค้าง)
        //   ⛔ คำที่พูดถึงเงินตรงๆ (คืนเงิน/โอนคืน/refund) ส่งต่อคนเสมอ ไม่ว่ามีออเดอร์หรือไม่
        //      — พลาดเคสเงินคืนจริงเสียหายกว่าเคสด่วนปลอมมาก
        // k165: รวมกฎเป็นตัวเดียว (แทน RX_MONEYBACK + เงื่อนไข k163 ที่กระจายอยู่)
        let _hasOrder = false, _paid = false;
        try {
          if (env.CONV) {
            const _ov = await env.CONV.get("ord:" + shopId + ":" + userId);
            if (_ov) { const _oj = JSON.parse(_ov); _hasOrder = !!(_oj && _oj.status); _paid = ordPaidActive(_oj); }
          }
        } catch (e) {}
        const moneyBack = refundIntent(t, _paid);   // k165: เกณฑ์คือ "จ่ายเงินแล้วหรือยัง"
        if (moneyBack) console.log("K165_REFUND_INTENT paid=" + _paid + " hasOrder=" + _hasOrder + " msg=" + t.slice(0, 40));
        // 😠 ลูกค้าเริ่มไม่พอใจ → ห้ามยิงข้อความสำเร็จรูปซ้ำอีก ต้องให้คนเข้ามาคุย
        const angry = RX_ANGRY.test(t);           // k147b: เดิม /ตอบมา\b/ ใช้ไม่ได้กับภาษาไทย (\b ไม่มีผลหลังสระ)
        const askInfo = /ยังไง|อย่างไร|เงื่อนไข|กี่วัน|ต้องเตรียม|ต้องทำ|ขั้นตอน|ได้มั้ย|ได้ไหม|รับเคลม/.test(t) && !moneyBack && !angry;
        if (moneyBack || angry) {
          // k165: ถ้าลูกค้าจ่ายเงินแล้วจริง ต้องขึ้นหน้าคิวให้ชัดว่ามีเงินค้างอยู่
          await muteNow(moneyBack
            ? (_paid ? "💸💸 ลูกค้าจ่ายเงินแล้วขอคืนยอด — ต้องให้คนตัดสิน ด่วนที่สุด" : "💸 ลูกค้าขอเงินคืน/ยกเลิก — ต้องให้คนตัดสิน ด่วน")
            : "😠 ลูกค้าไม่พอใจ — ต้องให้คนเข้าไปคุย ด่วน", t);
          await lineReply(TOKEN, replyToken,
            moneyBack
              ? (_paid
                  ? "รับเรื่องแล้วนะคะ 🙏🏻 ระบบตรวจพบว่าคุณลูกค้าชำระเงินเข้ามาแล้ว\nเรื่องคืนยอด/ยกเลิกรายการ ทีมงานจะเข้ามาดูแลและติดต่อกลับโดยตรงค่ะ รบกวนรอสักครู่นะคะ 💕"
                  : "รับเรื่องแล้วนะคะ 🙏🏻 เรื่องคืนเงิน/ยกเลิกรายการ แอดมินส่งต่อให้ทีมงานตรวจสอบให้ทันทีค่ะ\nรบกวนรอสักครู่ เดี๋ยวทีมงานเข้ามาดูแลต่อโดยตรงนะคะ 💕")
              : "ต้องขออภัยอย่างสูงเลยนะคะ 🙏🏻 ที่ตอบไม่ตรงกับที่คุณลูกค้าถาม\nแอดมินส่งเรื่องให้ทีมงานเข้ามาดูแลโดยตรงแล้วค่ะ รบกวนรอสักครู่นะคะ 💕", userId);
          return;
        }
        const complain = /ของ(ยัง)?ไม่(ถึง|มา|ได้)|ยังไม่ได้(รับ)?ของ|ของยังไม่มา|ของหาย|พัสดุหาย|พัสดุยังไม่|ส่งช้า|ล่าช้า|ยังไม่ส่ง|ของไม่ครบ|ได้ไม่ครบ|ของเสีย|ของพัง|ใช้ไม่ได้|สูบไม่ขึ้น|หัวตัน|น้ำยาซึม|น้ำยารั่ว|รั่ว|ซึม|เครื่องไม่ติด|ไฟไม่เข้า|ชาร์จไม่เข้า|จอไม่ติด|ดูดไม่ออก|ไม่มีควัน|ของปลอม|ไม่เหมือนรูป|ตีกลับ|คืนเงิน|ส่งผิด|ได้ผิด/.test(t)
          // 🐛 k200 (เคสจริง 5/8 18.58): "ขอเปลี่ยน..." เคยอยู่ในลิสต์นี้แบบไม่มีเงื่อนไข
          //   ลูกค้าที่ยังไม่เคยสั่งพิมพ์ "ขอเปลี่ยนสินค้าค่ะ" (หมายถึงเปลี่ยนใจเลือกของ) → โดนส่งเข้าคิวคน
          //   แล้วเงียบใส่อีก 6 ข้อความติดทั้งที่ลูกค้าพยายามสั่งของอยู่ = เสียการขายจริง
          //   กฎใหม่: "ขอเปลี่ยน" นับเป็นเคสร้องเรียนก็ต่อเมื่อ **มีออเดอร์อยู่จริง**
          || (/ขอเปลี่ยน|เปลี่ยนสินค้า/.test(t) && _hasOrder);
        if (complain && !askInfo) {
          await muteNow("เคสหลังการขาย ⚠️", t);
          await lineReply(TOKEN, replyToken,
            "ขออภัยในความไม่สะดวกอย่างมากค่ะ 🙏🏻\nแอดมินรับเรื่องไว้แล้วนะคะ กำลังส่งต่อให้ทีมงานหลังการขายตรวจสอบให้ทันทีค่ะ\n\nระหว่างรอ รบกวนแจ้งข้อมูลนี้ไว้ก่อนได้เลยค่ะ จะได้เช็คให้ไวขึ้น 💕\n• เลขออเดอร์ หรือ ชื่อ-ที่อยู่ที่สั่ง\n• สินค้าที่มีปัญหา (รุ่น/กลิ่น)\n• ถ้าเป็นของเสีย/ไม่ครบ รบกวนแนบคลิปตอนแกะกล่องด้วยนะคะ\n\nรอสักครู่นะคะ ทีมงานหลังการขายจะเข้ามาดูแลต่อค่ะ 🙏🏻", userId);
          return;
        }
      }
      // 🔎 k36: ถามวิธีเช็คของ/เลขพัสดุ (ไม่ใช่การร้องเรียน — เคสร้องเรียนถูกดักไปด้านบนแล้ว)
      // เคสจริง 31/7: "เช็คของที่ไหนครับ" → แอดมินตอบคลุมเครือจนเหมือนรับปากว่าจะมีเลขพัสดุให้
      if (/เลขพัสดุ|เลขติดตาม|tracking|เช็คของ|เช็คพัสดุ|เช็คสถานะ|สถานะออเดอร์|สถานะจัดส่ง|ของถึงไหน|ของอยู่ไหน|ติดตามพัสดุ|ติดตามยังไง|ดูสถานะ|ส่งของยัง|ส่งออกยัง|ได้เลขยัง|พัสดุถึงไหน/i.test(t)) {
        // 📦 k124: ถ้าแอดมินกรอกเลขพัสดุไว้แล้ว → ตอบเลขจริงเลย ไม่ต้องรบกวนคน
        //   (คำถามอันดับ 1 ที่ระบบเคยตอบไม่ได้ — เจอ 8,620 ครั้งในแชทร้านหลัก)
        try {
          let _tr = TRACK.get(shopId + ":" + userId);
          if (!_tr && env.CONV) { const v = await env.CONV.get("trk:" + shopId + ":" + userId); if (v) _tr = JSON.parse(v); }
          if (_tr && _tr.no) {
            const _link = (TRACK_URL[_tr.c] || TRACK_URL.flash)(_tr.no);
            await lineReply(TOKEN, replyToken,
              "เลขพัสดุของคุณลูกค้าคือ 📦\n" + _tr.no + "\n\nเช็คสถานะได้ที่ลิงก์นี้เลยค่ะ 👉\n" + _link +
              "\n\nถ้าสถานะยังไม่ขึ้น รบกวนรออีกสัก 6-12 ชม. นะคะ ระบบขนส่งอัปเดตช้ากว่าของจริงค่ะ 🙏🏻", userId);
            return;
          }
        } catch (e) {}
      ...
      if (/(?:https?:\/\/)?[^\s]*(?:maps\.app\.goo\.gl|goo\.gl\/maps|google\.[a-z.]+\/maps|maps\.google)/i.test(t)) {
        ...
        if (ll) {
          const { km } = riderFee(ll.lat, ll.lng);   // k75: กม.ไว้ให้ทีมงานกะระยะ ไม่เอาไปคิดเงิน
          ...
          await lineReply(TOKEN, replyToken, "ขออภัยค่ะ อ่านพิกัดจากลิงก์นี้ไม่ได้ 🙏🏻 รบกวนกดปุ่ม + ในแชท → เลือก 'ตำแหน่งที่ตั้ง' → ปักหมุดจุดจัดส่ง → กดส่ง มาอีกครั้งนะคะ เดี๋ยวทีมงานเช็คราคาให้ค่ะ 🛵", userId);
        }
        return;
      }
      // 🔧 วิธีใช้ / ปัญหาการใช้งาน — โค้ดตอบเอง (โมเดลมักปฏิเสธการสอนใช้ผลิตภัณฑ์ยาสูบ → แอดมินเงียบ)
      // 🚫 k136 (เคสจริง 3/8 12.55): "สินค้าที่เปลี่ยนหัวน้ำยามีแบรนด์ไหนบ้างคะ" = ถามรายชื่อแบรนด์
      //   แต่คำว่า "เปลี่ยนหัว" ไปตรงกับทางลัด "วิธีใช้งาน" → ระบบส่งคู่มือการใช้งานไปแทน คนละเรื่องเลย
      //   กฎ: ถ้าประโยคถามหา "รายชื่อ/แบรนด์/รุ่นไหนบ้าง" → ไม่ใช่คำถามวิธีใช้ ปล่อยให้ตอบเป็นรายการสินค้า
      if (/^(ใช้ยังไง|ใช้ไง|ใช้งานยังไง)|วิธีใช้|ใช้ยังไงคะ|ใช้ยังไงครับ|สูบยังไง|ดูดยังไง|ชาร์จ|เปลี่ยนหัว|ใส่หัว|ถอดหัว|เปิดเครื่องยังไง|ไฟกระพริบ|ไม่มีควัน|ดูดไม่ติด|ดูดไม่ออก|เครื่องไม่ทำงาน|ใช้ไม่ได้/i.test(t)
        && !/แบรนด์ไหน|ยี่ห้อไหน|รุ่นไหนบ้าง|มีอะไรบ้าง|มีรุ่นไหน|มีแบรนด์|มียี่ห้อ|มีตัวไหน|ตัวไหนบ้าง|อะไรบ้างคะ|อะไรบ้างครับ|แนะนำ/.test(t)) {
        await lineReply(TOKEN, replyToken,
          ...
          "ถ้ายังใช้ไม่ได้ แจ้งรุ่นที่ใช้มาได้เลยค่ะ เดี๋ยวทีมงานดูแลให้นะคะ 🙏🏻", userId);
        return;
      }
      ...
      // 🚬 คำถามเรื่องนิโคติน/สุขภาพ/สารในตัวสินค้า — โค้ดตอบเอง (โมเดลมักปฏิเสธ → แอดมินเงียบ)
      ...
      // เคสจริงจาก log: แอดมินแต่งชื่อแอดมินว่า "แอน" แล้วยืนยันว่า "แอนเป็นแอดมินจริงของร้าน" = โกหกลูกค้า
      ...
      //   ให้โค้ดตอบเอง + เด้งคิวให้ทีมงานยืนยัน (เรื่องเทคนิคคนรู้จริงตอบดีกว่า)
      if (/(ใส่|ใช้|เสียบ|สลับ|ต่อ)(กับ|ด้วย|ร่วม)?.{0,12}(เครื่อง|บอดี้|ตัวเครื่อง|หัว|พอต).{0,20}(ได้|เข้า)?(ไหม|มั้ย|ป่าว|เปล่า|รึ)/.test(t)
        ...
        await lineReply(TOKEN, replyToken,
          "เรื่องอุปกรณ์ใส่ด้วยกันได้ไหม ขออนุญาตให้ทีมงานยืนยันให้ชัดนะคะ 🙏🏻\n(แอดมินไม่อยากตอบผิดแล้วคุณลูกค้าซื้อไปใช้ไม่ได้ค่ะ)\n\nรบกวนแจ้ง \"รุ่นเครื่อง\" + \"รุ่นหัว\" ที่จะใช้คู่กัน เดี๋ยวทีมงานเช็คให้ทันทีค่ะ 💕", userId);
        try { await muteNow("ถามความเข้ากันได้ของอุปกรณ์ 🔌 — ทีมงานยืนยัน", String(t || "")); } catch (e) {}
        return;
      }
      if (/แอดมินชื่อ|ชื่ออะไรครับ|ชื่ออะไรคะ|ชื่อไรครับ|ชื่อไรคะ|คุณชื่ออะไร|เรียกว่าอะไร/.test(t)
        ...
        await lineReply(TOKEN, replyToken,
          "แอดมินเป็นผู้ช่วย AI ของร้าน ABC ค่ะ 🤖 ตอบได้ตลอด 24 ชม.เลยนะคะ\n" +
          "ถ้าอยากคุยกับทีมงานคนจริง บอกได้เลยค่ะ เดี๋ยวเรียกให้ทันที 💕", userId);
        return;
      }
      ...
      if (_cancel) {
        // k108 (เคสจริง 2/8 20.47 น.): ยกเลิกออเดอร์ที่ "ยังไม่โอน" แล้วแอดมินเงียบไปเลย
        ...
        try {
          if (env.CONV) {
            ...
            if (o) { try { const oj = JSON.parse(o); if (ordPaidActive(oj)) paidOrder = true; } catch (e2) { paidOrder = true; } }
        ...
        if (paidOrder) {
          ...
          await lineReply(TOKEN, replyToken, "รับทราบค่ะ 🙏🏻 เดี๋ยวทีมงานเช็คออเดอร์แล้วดำเนินการยกเลิกให้นะคะ 💕", userId);
          try { await muteNow("ขอยกเลิกออเดอร์ (โอนแล้ว) ⚠️", t); } catch (e) {}
        } else {
          // 🔁 k139 (เคสจริง 3/8 18.47): ลูกค้าพิมพ์ "ไม่รับแล้วค่ะ" แล้วตามด้วย "ยกเลิก" ติดกัน
          //   ระบบตอบข้อความยกเลิกเหมือนกันเป๊ะ 2 รอบ = ดูเหมือนบอทค้าง ลูกค้าที่กำลังไม่พอใจอยู่ยิ่งหงุดหงิด
          //   แก้: ถ้าเพิ่งยกเลิกไปภายใน 90 วิ ไม่ต้องพูดซ้ำ ตอบสั้นๆ ครั้งเดียวพอ
          let _justCancelled = false;
          try {
            const _ck = "cxl:" + shopId + ":" + userId;
            const _prev = CANCELLED.get(_ck);
            if (_prev && Date.now() - _prev < 90000) _justCancelled = true;
            CANCELLED.set(_ck, Date.now());
          } catch (e) {}
          if (_justCancelled) {
            console.log("K139_DUP_CANCEL ตอบสั้น ไม่พูดซ้ำ");
            await lineReply(TOKEN, replyToken, "รับทราบค่ะ 💕", userId);
          } else {
            await lineReply(TOKEN, replyToken, "รับทราบค่ะ 🙏🏻 ยกเลิกรายการให้เรียบร้อยแล้วนะคะ\nถ้าอยากสั่งใหม่หรือให้แอดมินแนะนำรุ่นอื่น บอกได้เลยค่ะ 💕", userId);
          }
        }
        return;
      }
      // ⏸ k66: "เดี๋ยวสั่งใหม่ / ไว้ก่อน / ขอคิดดูก่อน" → ตอบสั้นๆ พอ
      // เคสจริง 1/8: ลูกค้าพิมพ์ "เดี๋ยวสั่งใหม่" → แอดมินยัดลิสต์กลิ่น MARBO 9K มา 10 บรรทัด
      if (/^(เดี๋ยว|ไว้)(ค่อย)?(สั่ง|ซื้อ|เอา)(ใหม่|ทีหลัง|ก่อน)?|ไว้ก่อน|ขอคิดดูก่อน|ขอดูก่อน|เดี๋ยวมาใหม่|แป๊บนึง|ขอเวลาคิด/i.test(t)) {
        await lineReply(TOKEN, replyToken, "ได้เลยค่ะ 💕 ทักมาได้ตลอดเลยนะคะ แอดมินอยู่ 24 ชม.ค่ะ 🙏🏻", userId);
        return;
      }
      // 🛵 k113 (เคสจริง 2/8 21.26 น.): ส่งด่วน+สลิปผ่าน ระบบขอ "ชื่อ+เบอร์" แล้ว
      //   แต่ไม่มีโค้ดรับช่วงต่อ → AI กลับไปไล่ขอที่อยู่พัสดุทีละช่อง (เลขที่/เขต/ถนน/ไปรษณีย์) วนไม่จบ
      //   แก้: ลูกค้าส่งข้อความมีเบอร์โทร + ออเดอร์ ✅ โหมดส่งด่วน → ปิดออเดอร์ด้วยหมุดทันที
      if (/0\d{1,2}[- ]?\d{3}[- ]?\d{3,4}/.test(t)) {
        try {
          if (env.CONV) {
            const _ov = await env.CONV.get("ord:" + shopId + ":" + userId);
            ...
            if (_ov && _ex) {
              ...
              if (_oj && /✅/.test(_oj.status || "") && !/พร้อมจัดส่ง/.test(_oj.status || "") && _ej && _ej.lat) {
                ...
                await lineReply(TOKEN, replyToken, "รับข้อมูลผู้รับเรียบร้อยค่ะ 🙏🏻\n🛵 ออเดอร์พร้อมจัดส่งตามหมุดที่แชร์ไว้แล้วนะคะ เดี๋ยวไรเดอร์ติดต่อตามเบอร์ที่ให้ไว้เลยค่ะ ขอบคุณที่อุดหนุนนะคะ 💕", userId);
                console.log("K113_EXPRESS_CLOSED " + userId.slice(0, 8));
                return;
              }
            }
          }
      ...
      // 💸 k81 เคสจริง 2/8 (เจ้านายเทสโอนจริง 320): ลูกค้าพิมพ์ "โอนไปแล้ว" → AI ตอบเอง "เดี๋ยวทีมงานตรวจสอบสลิปให้"
      ...
      if (/(?:โอน|จ่าย|ชำระ)(?:เงิน)?(?:ไป|ให้|มา)?(?:เรียบร้อย)?(?:แล้ว|เเล้ว|เสร็จ)/.test(t)
        ...
        try {
          if (env.CONV) {
            const _ov = await env.CONV.get("ord:" + shopId + ":" + userId);
            if (_ov) { const _oj = JSON.parse(_ov); if (ordPaidActive(_oj)) _ordPaid = true; }
          }
        } catch (e) {}
        if (_ordPaid) {
        ...
        if (_claims >= 2) {
          await lineReply(TOKEN, replyToken, "ขออภัยที่ล่าช้านะคะ 🙏🏻 เดี๋ยวทีมงานเข้ามาตรวจสอบยอดโอนให้ทันทีเลยค่ะ 💕", userId);
      ...
      // 💸💸 k164 (เคสจริง 4/8 — ลูกค้า PHUNG): แอดมินแจ้งค่าส่งด่วน 280 บาท
      //   ลูกค้าตอบ "ไม่เอาแพง" → บอทตอบ "รบกวนพิมพ์ชื่อรุ่นหรือส่งรูปสินค้ามาให้หน่อยนะคะ"
      //   = ไม่ตอบเรื่องที่ลูกค้าบ่นเลยสักคำ · ต้นเหตุ: ข้อความไม่มีชื่อรุ่น → unknownAskHint
      //   สั่งให้ถามชื่อรุ่น ทั้งที่บริบทคือ "ลูกค้ากำลังจะหนีเพราะค่าส่งแพง"
      //   จังหวะนี้คือจังหวะปิดการขาย — ต้องเสนอทางเลือกพัสดุ 40 บาททันที ไม่ใช่ถามชื่อรุ่น
      if (/แพง|ลดหน่อย|ลดได้ไหม|ลดได้มั้ย|ถูกกว่านี้|ไม่ไหว|เยอะจัง|โหดจัง/.test(t) && !_MODEL_IN(t)) {
        let _fee = null;
        try {
          const _ef = EXPFEE.get(shopId + ":" + userId);
          if (_ef && Date.now() - _ef.t < 7200000 && typeof _ef.fee === "number") _fee = _ef.fee;
          if (_fee === null && env.CONV) {
            const _x = await env.CONV.get("exp:" + shopId + ":" + userId);
            if (_x) { const _j = JSON.parse(_x); if (_j && typeof _j.fee === "number") _fee = _j.fee; }
          }
        } catch (e) {}
        if (_fee !== null) {
          console.log("K164_EXPFEE_TOO_PRICEY fee=" + _fee);
          await lineReply(TOKEN, replyToken,
            "เข้าใจเลยค่ะ 🙏🏻 ค่าส่งด่วนคิดตามระยะทางจริง ทางร้านปรับให้ไม่ได้นะคะ\n\n" +
            "📦 ถ้าไม่รีบ เปลี่ยนเป็น **พัสดุปกติ** ได้เลยค่ะ ค่าส่งแค่ 40 บาท (ซื้อครบตามโปรส่งฟรีด้วยนะคะ)\n" +
            "จัดส่งออกภายใน 1-2 วัน ได้รับภายใน 2-3 วันค่ะ\n\n" +
            "🛵 ส่งด่วนตามหมุดคือ " + _fee + " บาท ได้ของไวกว่ามากค่ะ\n\nรับแบบไหนดีคะ 💕", userId);
          return;
        }
      }
      ...
      if (/ฟรี(?![ซสzs])|โปร|ไม่คิดค่าส่ง|ไม่เสียค่าส่ง/.test(t)
        ...
        await lineReply(TOKEN, replyToken,
          // 🧮 k138 (เจ้าของร้านชี้ 3/8): เดิมเขียนว่า "เข้าโปรแล้วเลือกส่งด่วน = ประหยัดค่าส่งพัสดุ 40 บาทไป"
          //   ผิด! เลือกส่งด่วนก็ไม่ได้จ่าย 40 อยู่แล้ว (คนละวิธีส่ง) → ไม่มีอะไรให้ประหยัด
          //   ลูกค้าอ่านแล้วนึกว่าจะได้ลด 40 บาทจากค่าส่งด่วน พอเห็นยอดจริงจะรู้สึกโดนหลอก
          "ขออนุญาตแจ้งให้ชัดนะคะ 🙏🏻\n\n🎁 โปรส่งฟรี ใช้กับ \"พัสดุปกติ\" เท่านั้นค่ะ (เช่น สูบทิ้งครบ 4 แท่ง = ส่งฟรี)\n🛵 ส่งด่วน (แกร็บ) คิดค่าส่งตามระยะทางจริงเสมอ โปรส่งฟรีใช้ไม่ได้ค่ะ\n\nเลือกได้แบบเดียวนะคะ:\n• พัสดุปกติ = ส่งฟรี (เข้าโปรแล้ว) · ได้รับใน 2-3 วัน\n• ส่งด่วน = ได้ไววันนี้ · จ่ายค่าส่งตามระยะทาง\n\nสนใจรับแบบไหนดีคะ 💕", userId);
        return;
      }
      // 🩹 k182 (GR-D · เคสจริงจาก Final Audit): ด่านกันส่งด่วนต่างจังหวัด (k64) อยู่ "ข้างใน" บล็อกนี้
      //   แต่ประตูเข้าบล็อกรับคำว่า "ส่งด่วน" เฉพาะตอนที่มันเป็นทั้งข้อความเท่านั้น (^...$)
      //   → "อยู่ขอนแก่น ขอส่งด่วนได้ไหม" · "ส่งด่วนได้ไหม อยู่ภูเก็ต" เข้าไม่ถึงด่าน k64 เลย
      //   → หลุดไปให้ AI ตอบ "ได้ค่ะ ปักหมุดมา" = รับปากในสิ่งที่ร้านทำไม่ได้
      //   (คำว่า "แกรป" อยู่ในลิสต์แบบเจอที่ไหนก็ได้ จึงรอด — เลยดูเหมือนด่านทำงาน ทั้งที่รั่วครึ่งเดียว)
      //   แก้ที่กฎเดิม: ให้ "ส่งด่วน" อยู่ในลิสต์แบบเจอที่ไหนก็ได้ เหมือน "แกรป"
      //   ⚠️ ไม่ต้องเพิ่มด่านใหม่ — ด่านกันออเดอร์หาย (looksLikeOrderText) อยู่หลัง k64 อยู่แล้ว
      if (/แกร?[็ๆ]?[บป]|grab|ไรเดอร์|rider|มอเตอร์ไซค์|วินมอไซ|เมสเซนเจอร์|messenger|ลาลามูฟ|lalamove|ตามหมุด|ที่หมุด|ตามพิกัด|ส่งด่วน|ส่งดวน/i.test(t)
        ...
        // เคสจริง 1/8: ลูกค้าบอก "เชียงใหม่ ส่งแกร็บได้หรอครับ" → แอดมินตอบ "ได้เลยค่ะ 🛵 แชร์โลเคชั่นมา"
        ...
        // 🧾🧾 k160: ข้อความนี้ "เป็นการสั่งของ" ด้วย → ห้ามจบที่ทางลัด ให้ไหลไปทางรับออเดอร์ปกติ (k102)
        //   เดิม: "เอามาโบ องุ่น 1 แท่ง ส่งแกรป" → ทางลัดจับคำว่า "แกรป" แล้ว return = **ออเดอร์หายทั้งดุ้น**
        //   → แอดมินกรอกค่าส่งใน /expfee ก็หาออเดอร์ไม่เจอ (ต้องมี status "รอโอน 💰") = ไม่มีการ์ด ลูกค้าต้องสั่งใหม่
        //   ⚠️⚠️ ต้องอยู่ **หลัง** ด่านต่างจังหวัด (k64) เสมอ — วางไว้ก่อนหน้าแล้วลูกค้าเชียงใหม่
        //      ที่สั่งของพร้อมคำว่า "แกรป" จะหลุดด่าน แล้วโดนขอหมุดทั้งที่ร้านส่งด่วนไปไม่ถึง
        //   ⚠️ ต้องอยู่ **ก่อน** ก้อนขอหมุด/แจ้งค่าส่ง เพราะก้อนนั้น return ทุกทาง
        if (looksLikeOrderText(t)) {
          console.log("K160_ORDER_IN_EXPRESS_MSG pass-through");
        } else {
        let exp = null;
        ...
        if (exp && exp.outzone) {
          ...
          // ⏰ k153: เดิมก้อนนี้ไม่เช็คเวลาเลย — เคสจริง 3/8 23.14 ลูกค้าแชร์หมุดแล้ว เลยไม่เคยเข้าก้อน else
          //   ที่มี _afterLast ของ k83 → ลูกค้ารอทั้งคืนโดยไม่มีใครบอกว่าเลยรอบไปแล้ว
          await lineReply(TOKEN, replyToken, "ได้รับหมุดแล้วนะคะ 📍 ทีมงานกำลังเช็คค่าส่งด่วนให้อยู่ค่ะ เดี๋ยวแจ้งราคาทันทีนะคะ 🛵💕" + lateNote(), userId);
        } else if (exp && typeof exp.fee === "number") {
          await lineReply(TOKEN, replyToken, "จากหมุดที่ส่งมา ค่าส่งด่วนของคุณลูกค้าคือ " + exp.fee + " บาทค่ะ (ระยะทาง ~" + exp.km + " กม.) 🛵\nรับสินค้ารุ่นไหน กลิ่นอะไร กี่ชิ้นดีคะ 💕" + lateNote(), userId);
        } else {
          ...
          const _afterLast = thTime().afterLast;   // k153: ใช้ตัวกลางตัวเดียว (เดิมคำนวณเองตรงนี้จุดเดียว)
          if (_afterLast) {
            await lineReply(TOKEN, replyToken,
              "มีบริการส่งด่วนค่ะ 🛵 (เฉพาะ กทม. และปริมณฑล)\nแต่ตอนนี้เลยรอบส่งด่วนรอบสุดท้ายของวันแล้วนะคะ 🙏🏻\nรอบถัดไปจัดส่งออก 10.30 น. ค่ะ\n\nสั่งซื้อ + ชำระเงินตอนนี้ได้เลยนะคะ เดี๋ยวจัดคิวส่งด่วนรอบแรกให้เลยค่ะ\nรบกวนแชร์โลเคชั่น (ปักหมุด) มาไว้ได้เลย เดี๋ยวทีมงานเช็คราคาแจ้งให้ทันทีค่ะ", userId);
          } else {
            await lineReply(TOKEN, replyToken, "ได้เลยค่ะ 🛵 รบกวนแชร์โลเคชั่น (ปักหมุด) จุดจัดส่งมาให้หน่อยนะคะ\n(กดปุ่ม + ในแชท → 'ตำแหน่งที่ตั้ง' → ปักหมุด → ส่ง หรือส่งลิงก์ Google Maps มาก็ได้ค่ะ)\nเดี๋ยวทีมงานเช็คราคาส่งด่วนให้ทันทีค่ะ", userId);
          }
        }
        // 💰💰 k160 (เจอตอนไล่โฟลว์ 4 ส.ค.): ลูกค้าสั่งแบบพัสดุจนได้การ์ดยอด 390 แล้ว
        //   ต่อมาพิมพ์ "ขอส่งแกรปแทน" → ord: ยังค้าง "รวมยอดชำระ 390" (ค่าส่งพัสดุ 40)
        //   พอลูกค้าพิมพ์ "ยืนยัน" → ระบบส่งการ์ดเลขบัญชี **ยอด 390** ทั้งที่เลือกส่งด่วน
        //   → ลูกค้าโอน 390 แต่ยอดจริงคือ 350 + ค่าส่งด่วน = สลิปไม่ตรงยอดทุกครั้ง
        //   (ตรงข้ามกับ k78 ที่ทำไว้แล้วสำหรับทิศ "ด่วน → พัสดุ" · ทิศนี้ยังไม่มีใครดู)
        //   แก้: ล้างยอดพัสดุออกจากออเดอร์ทันที เหลือ "ค่าส่งด่วน (รอทีมงานเช็ค)"
        //        → "ยืนยัน" จะไม่ส่งการ์ดเลขบัญชี เพราะไม่มี "รวมยอดชำระ" ให้จับ
        try {
          if (env.CONV) {
            const _ov = await env.CONV.get("ord:" + shopId + ":" + userId);
            if (_ov) {
              const _oj = JSON.parse(_ov);
              if (_oj && _oj.status === "รอโอน 💰" && /รวมยอดชำระ/.test(String(_oj.block || "")) && !/ค่าส่งด่วน/.test(String(_oj.block || ""))) {
                _oj.block = String(_oj.block)
                  .replace(/\nค่าส่ง \d+/, "")
                  .replace(/\nรวมยอดชำระ [\d,]+/, "\nค่าส่งด่วน (รอทีมงานเช็ค)");
                _oj.t = Date.now();
                await env.CONV.put("ord:" + shopId + ":" + userId, JSON.stringify(_oj), { expirationTtl: 259200 });
                console.log("K160_SWITCH_TO_EXPRESS cleared parcel total");
              }
            }
          }
        } catch (e) {}
        return;
        }   // ปิด else ของ k160 (ข้อความที่เป็นการสั่งของ จะไม่เข้ามาถึงตรงนี้)
      }
      ...
      if (/รูปแบบการจัดส่ง|วิธีการจัดส่ง|การจัดส่ง|ค่าส่งเท่าไหร่|ค่าจัดส่ง|ส่งยังไง|จัดส่งยังไง|ส่งแบบไหน/.test(t)) {
        ...
        try {
          if (env.CONV) {
            ...
            if (_feeS !== null) {
              await lineReply(TOKEN, replyToken, "ค่าส่งด่วนสำหรับหมุดของคุณลูกค้าคือ " + _feeS + " บาทค่ะ 🛵\n(หรือพัสดุปกติ 40 บาท ได้รับใน 2-3 วันค่ะ)\nรับแบบไหนดีคะ 💕", userId);
              return;
            }
            if (_pendS) {
              await lineReply(TOKEN, replyToken, "ทีมงานกำลังเช็คค่าส่งด่วนให้อยู่ค่ะ เดี๋ยวแจ้งราคาทันทีนะคะ 🛵\n(หรือรับแบบพัสดุปกติ ค่าส่ง 40 บาท ก็ได้ค่ะ) 💕", userId);
              return;
            }
          }
        } catch (e) {}
    ...
    try {
      if (env.CONV) {
        ...
        if (d && d !== LANG) {
          // k145: เปลี่ยน "ไทย → อังกฤษ" ต้องมีหลักฐานหนัก (คำอังกฤษจริง ≥2 คำหลังตัดชื่อสินค้า)
          //   ส่วนเปลี่ยนกลับเป็นไทย ทำได้ทันทีเสมอ (ปลอดภัยกว่า)
          const _txt = String((ev.message && ev.message.text) || "");
          const _strong = d === "th" || _txt.replace(PRODUCT_TOKENS, " ").split(/\s+/).filter(w => /[A-Za-z]{3,}/.test(w)).length >= 2;
          if (_strong) { LANG = d; await env.CONV.put("lang:" + shopId + ":" + userId, d, { expirationTtl: 604800 }); }
        }
      }
    } catch (e) {}
    ...
    // 🔒 k15: ห้ามส่งข้อมูลโอนเข้าสมอง AI เด็ดขาด (เคสจริง 28/7: แอดมินพิมพ์ "ข้อมูลโอน: ชื่อ <เจ้าของบัญชี>" ก่อนลูกค้ากดยืนยัน)
    ...
    // ⛔ รายชื่อรุ่นที่หมดสต็อกทั้งรุ่น → ห้ามแอดมินเอาไปแนะนำ
    SOLD_OUT_MODELS = [];
    let outNote = "";
    try {
      if (env.CONV) {
        ...
        //     (ข) ที่อื่นทั้งระบบ ใช้ findStockForItem ซึ่งจับคู่ข้ามความแรง/ชื่อเพี้ยนได้
        //   ผล: (ก) บอก "หมดทั้งรุ่น" แต่ (ข) หากลิ่นที่มีของเจอ → ข้อความขัดกันเองในบับเบิลเดียว
        //   แก้: ใช้ findStockForItem เป็นตัวตัดสินเดียวทั้งระบบ (ตรงกับตอนออกการ์ดเป๊ะ)
        const outs = [];
        for (const mdl in FLAVORS) {
          const fl = (FLAVORS[mdl] && FLAVORS[mdl].f) || [];
          if (!fl.length) continue;
          let okCount = 0;
          for (const f of fl) {
            const q = findStockForItem(smAll, mdl, f);
            if (q === null || q > _buf || stockOtherStrength(smAll, mdl, f) > _buf) { okCount++; break; }
          }
          if (okCount === 0) outs.push(mdl);
        }
    ...
    // ⏰⏰ k153 (เคสจริง 3/8 23.14 — ลูกค้า B🦋A · ยืนยันด้วยแคปหน้าจอ LINE ว่าส่งถึงลูกค้าจริง):
    //   ลูกค้า 23.13: "หมายถึงส่งตอนนี้ได้ใช่ไหม" → บอท: "ใช่ค่ะ ถ้าชำระก่อน 20.45 น. รอบส่งด่วนจะออกภายในวันนี้"
    //   ลูกค้า 23.14: "แล้ว 5ทุ่มยังส่งของอยู่ไหม" → บอท: "หลัง 20.45 รอบส่งออก 10.30 วันถัดไปค่ะ
    //     **ตอนนี้ยังพอมีเวลาเหลืออยู่**" ← ขัดกันเองในข้อความเดียว ตอนนั้น 23.14 เลยรอบมา 2.5 ชม.แล้ว
    //   ลูกค้า: "อ้าวนึกว่าจะได้คืนนี้" → บอทยังยืนยันซ้ำอีกว่า "ก่อน 20.45 ยังทันรอบส่งด่วนวันนี้ค่ะ"
    //   ต้นเหตุ (ยืนยันกับโค้ดแล้ว): บล็อก k56 บอก AI แค่ "กฎ" ว่ารอบสุดท้าย 20.45
    //     แต่ **ไม่เคยบอกว่าตอนนี้กี่โมง** → AI ท่องกฎออกมาราวกับยังเป็นกลางวัน
    //     (โค้ดที่รู้เวลามีอยู่แล้วคือ _afterLast ของ k83 แต่อยู่แค่ในทางลัด "ยังไม่เคยแชร์หมุด")
    //   ตระกูลเดียวกับ k137: รับปากลูกค้าในสิ่งที่ไม่จริง — รอบนี้ลูกค้าอาจโอนเงินแล้วรอของทั้งคืน
    //   ⚠️ ต้องคำนวณตอน "รับ request" เท่านั้น ห้ามใส่ใน SYSTEM_PROMPT ที่เป็น const
    //      (Cloudflare isolate อยู่ข้ามชั่วโมงได้ → เวลาจะค้างเป็นตอนที่ worker บูต)
    const _t153 = thTime();
    const timeNote = "\n\n# ⏰⏰ เวลาจริงตอนนี้ (ข้อเท็จจริง เชื่อข้อมูลนี้ก่อนทุกอย่าง ห้ามคำนวณเอง)\n" +
      "ขณะนี้เวลาประเทศไทย **" + _t153.hhmm + " น.**\n" +
      (_t153.afterLast
        ? "🌙 **เลยรอบส่งด่วนรอบสุดท้ายของวัน (20.45 น.) มาแล้ว**\n" +
          "⛔⛔ ห้ามพูดว่า 'ยังทันรอบวันนี้' / 'ได้รับภายในวันนี้' / 'ได้รับคืนนี้' / 'ตอนนี้ยังพอมีเวลา' / 'ยังทันอยู่' เด็ดขาด — ไม่จริงแล้ว\n" +
          "⛔ ห้ามพูดประโยคเงื่อนไข 'ถ้าชำระก่อน 20.45 น. จะ...' เพราะเวลานั้นผ่านไปแล้ว ลูกค้าอ่านแล้วเข้าใจว่ายังทัน\n" +
          "✅ ต้องบอกตรงๆ ว่ารอบส่งด่วนถัดไปคือ **10.30 น. ของวันถัดไป** และสั่ง+ชำระตอนนี้ได้เลย เดี๋ยวจัดคิวรอบแรกให้"
    ...
    showLoading();   // k47: อ่านรูปใช้เวลา ต้องโชว์จุดไข่ปลา
      ...
      // 🖼🖼 k156 (เคสจริง 3/8 23.58 — ลูกค้า JW · ยืนยันด้วยแคป LINE + หลังบ้าน):
      //   ลูกค้าสั่ง BOOST POD กล้วย 4 อันไว้ (ออเดอร์ค้างสถานะ "รอโอน") แล้วส่ง **รูปเมนู BIG POD
      //   ที่วงกลมกลิ่นไว้ 2 กลิ่น** มาถามต่อ → ระบบตอบ "ได้รับสลิปแล้วนะคะ ระบบตรวจอัตโนมัติไม่สำเร็จ"
      //   + เด้งเคสด่วนเข้าหลังบ้าน "สลิปมีปัญหา (code 1007): รูปภาพไม่มี QR Code"
      //   → ลูกค้าถามกลิ่น แต่ได้คำตอบเรื่องสลิป = งง · แอดมินเสียเวลากับเคสด่วนปลอม
      //   → และลูกค้า "สั่งของเพิ่มระหว่างรอโอน" ไม่ได้เลย ทุกรูปกลายเป็นสลิปหมด
      //   ต้นเหตุ (ยืนยันกับโค้ดแล้ว): k80 บังคับ _slipForced = ข้ามการดูรูปทั้งหมด (dataUri = null)
      //     แล้วยัด reply = "[SLIP]" ตรงๆ → vision ไม่เคยได้เห็นรูปเลยสักครั้ง
      //   ⚠️ k80 มีเหตุผลที่ดี (เคสจริง 2/8: สลิป K+ จริงแต่ vision ไม่ตอบแท็ก → ลูกค้าโอนแล้วโดนเมิน)
      //     ห้ามถอด k80 ทิ้ง — แต่เปลี่ยนจาก "ตัดสินแทน" เป็น "ตาข่ายรอง"
      //   กฎใหม่: ให้ **vision ดูรูปก่อนเสมอ** · "รอโอน" ใช้ตอน vision ดูไม่ออก/ตอบกำกวมเท่านั้น
...
"⛔⛔ กฎสูงสุดของการอ่านรูป (สำคัญกว่าทุกขั้นด้านล่าง):\n" +
"1) เครื่องหมายที่ลูกค้าทำไว้ (วงกลม · กรอบ · ขีดเส้น · ลูกศร · ไฮไลต์ · ติ๊กถูก) = **คำสั่งของลูกค้า**\n" +
"   ถ้ารูปมีเครื่องหมาย ให้อ่าน **เฉพาะสิ่งที่อยู่ในบริเวณที่ทำเครื่องหมาย** เท่านั้น\n" +
"   ส่วนอื่นของรูปเป็นแค่บริบท **ไม่ใช่ตัวเลือกให้เสนอ** ห้ามลิสต์รุ่นอื่นในภาพที่ไม่ได้ถูกวง\n" +
"2) ถ้าสิ่งที่เห็นในภาพขัดกับสิ่งที่ถูกวง → **ยึดสิ่งที่ถูกวงเสมอ**\n" +
"3) ⛔⛔ ห้ามเอ่ยชื่อรุ่นที่ **ไม่ปรากฏอยู่ในรูปนี้** เด็ดขาด\n" +
"   ห้ามหยิบรุ่นจากบทสนทนาก่อนหน้า จากรายการสินค้า หรือจากความจำมาเติมลงในคำตอบ\n" +
"   อ่านบริเวณที่วงไม่ออก/รูปเบลอ → ถามกลับตรงๆ ว่าวงตรงไหน ห้ามเดา ห้ามเดารุ่นยอดฮิต\n\n" +
"ขั้นที่ 2 — อ่านบริเวณที่ลูกค้าทำเครื่องหมาย (ทำก่อนเสมอ):\n" +
...
"6) ⛔ ห้ามบอกว่ามีของ/หมด เอง — แค่ระบุว่าเห็นรุ่นอะไร ระบบจะเช็คสต็อกให้เอง" },
      ...
      // k156: โหลดรูปไม่ได้ + มีออเดอร์รอโอน → ยังถือเป็นสลิปเหมือนเดิม (คงตาข่าย k80)
      reply = !dataUri ? "[SLIP]" : await askAI(env.OPENROUTER_KEY, [{ role: "system", content: sysFull }, ...histForAI(history, 8), visionMsg], VISION_MODELS);
      ...
      // k156: ตาข่ายรองของ k80 — มีออเดอร์รอโอนอยู่ และ vision "ดูไม่ออกว่าเป็นอะไร"
      //   (ตอบว่าง / สั้นจนไม่มีเนื้อหา / ไม่ได้พูดถึงรุ่น-กลิ่น-เมนู-สินค้าเลย) → ถือเป็นสลิปไว้ก่อน
      //   ⛔ ห้ามเหมาเป็นสลิปถ้า vision อ่านออกแล้วว่าเป็นเมนู/สินค้า (นั่นคือบั๊กที่กำลังแก้อยู่)
      ...
      if (reply.indexOf("[SLIP]") !== -1) {
        ...
        // ✅ สลิปผ่าน  → แอดมินขอที่อยู่จัดส่งต่อ (ไม่มิ้วต์) แล้วค่อยสรุปออเดอร์
        ...
        // 🛵 k110 (เคสจริง 2/8 20.53 น.): ออเดอร์ส่งด่วน (ปักหมุดแล้ว) แต่สลิปผ่านแล้วแอดมินถาม
        ...
        try {
          ...
          if (sok && sok.noConfig) {
            ...
            if (sok.httpOk && sok.success && d.success) {
              ...
              statusLine = "⛔ สลิปมีปัญหา (code " + code + "): " + m.slice(0, 60) + " — ทีมงานเช็คด่วน";
              if (code === 1012 || /ซ้ำ|duplicate|เคย|ตรวจสอบแล้ว|ตรวจแล้ว|already|used/i.test(m)) {
                ...
                // สลิปซ้ำ = โอนถูกแล้ว แต่ส่งสลิปใบเดิมที่เคยตรวจไปแล้ว → ส่งต่อทีมงานยืนยัน (ไม่โทษลูกค้า)
                statusLine = "⚠️ สลิปนี้เคยตรวจแล้ว (อาจส่งซ้ำ) — ทีมงานยืนยันด่วน";
                customerMsg = "✅ ได้รับสลิปแล้วนะคะ 🙏🏻 ระบบตรวจพบว่าสลิปนี้เคยส่งเข้ามาแล้ว เดี๋ยวทีมงานเช็คและยืนยันการชำระให้อีกครั้งค่ะ 💕";
              } else if (code === 1010 || /บัญชี.*ไม่ตรง|ไม่ตรง.*บัญชี|ผู้รับ|receiver/i.test(m)) {
                customerMsg = "⚠️ ตรวจสอบสลิปแล้ว บัญชีผู้รับไม่ตรงกับบัญชีของร้านนะคะ 🙏🏻\nรบกวนตรวจสอบว่าโอนเข้าบัญชีที่ถูกต้องไหมคะ เดี๋ยวทีมงานเข้ามาดูแลค่ะ";
              } else {
                // อื่นๆ (QR อ่านไม่ได้/สลิปไม่รองรับ/ระบบล่ม) → ส่งต่อแอดมิน ไม่โทษว่าสลิปไม่ชัด
                customerMsg = "🙏🏻 ได้รับสลิปแล้วนะคะ ระบบตรวจอัตโนมัติไม่สำเร็จ เดี๋ยวทีมงานเข้ามาตรวจสอบและยืนยันการชำระให้อีกครั้งค่ะ 💕";
              }
            }
          }
        } catch (e) {}

        // อัพสถานะออเดอร์ให้ทีมงานเห็นผลตรวจ
        try {
          if (ordObj) {
        ...
        if (slipPassed) {
          // ชำระผ่าน → แอดมินขอที่อยู่ต่อ (ไม่มิ้วต์) + จดประวัติว่าชำระแล้ว เพื่อให้แอดมินรู้ว่าต้องขอที่อยู่แล้วสรุปออเดอร์
          try {
            if (env.CONV) {
              const next = stampHist([...history, { role: "user", content: "[ลูกค้าส่งสลิปโอนเงิน — ตรวจสอบแล้วชำระเงินถูกต้อง]" }, { role: "assistant", content: _expOrd ? "ยืนยันการชำระเงินเรียบร้อยค่ะ รอบนี้จัดส่งด่วนตามหมุดที่ลูกค้าแชร์ไว้แล้ว กำลังขอแค่ชื่อผู้รับ+เบอร์โทรสำหรับไรเดอร์ (มีที่อยู่จากหมุดแล้ว ห้ามขอที่อยู่/เขต/รหัสไปรษณีย์เด็ดขาด)" : "ยืนยันการชำระเงินเรียบร้อยค่ะ กำลังขอที่อยู่จัดส่งจากลูกค้า" }].slice(-20));
      ...
      // แอดมินจะไม่รู้ว่ารูปนั้นมีอะไร แล้วหยิบรุ่นมั่วมาตอบ (เคสจริง 29/7: รูป JOIWAY+VOSOON → ตอบ ESKO BAR SWITCH)
      let seen = [];
      try {
      ...
      // เคสจริง 31/7: ออเดอร์ MARBO 9K อยู่ ลูกค้าขอเปลี่ยนเป็นบลูไอซ์ (มีของจริง) แต่แอดมินตอบว่า "หมดสต็อก"
        ...
        if (fl) {
          ...
          if (mdl && FLAVORS[mdl]) {
            ...
            if (real) {
              ...
              // 🔄 k170 (เจอตอนไล่โฟลว์ 5 ส.ค. — ยืนยันด้วยการรันโค้ดจริง):
              //   ลูกค้า "เปลี่ยนเป็นเยลลี่แทน" ตอนยังไม่ได้โอนเลย
              //   → บอทตอบ "รอทีมงานยืนยันการแก้ไขรายการ" + **เงียบแชทรอคน** · ออเดอร์ไม่เปลี่ยนตาม
              //   = ลูกค้านั่งรอคนที่อาจไม่มา ทั้งที่ระบบออกการ์ดใหม่ให้เองได้ (เหมือนตอน "เพิ่มเป็น 4 แท่ง")
              //   เกณฑ์ที่ถูก = เกณฑ์เดียวกับ k165: **จ่ายเงินแล้วหรือยัง**
              //     ยังไม่จ่าย → เปลี่ยนได้เลย ปล่อยไหลไปทางออกการ์ดปกติ (ไม่ตอบตรงนี้ ไม่เงียบแชท)
              //     จ่ายแล้ว  → เงินอยู่กับร้านแล้ว ต้องให้คนตัดสิน (คงพฤติกรรมเดิมไว้)
              let _swapPaid = false;
              try {
                if (env.CONV) {
                  const _sv = await env.CONV.get("ord:" + shopId + ":" + userId);
                  if (_sv) _swapPaid = /✅/.test(String((JSON.parse(_sv) || {}).status || ""));
                }
              } catch (e) {}
              if (isSwap && have && !_swapPaid) {
                console.log("K170_SWAP_BEFORE_PAY pass-through");
              } else {
              let msg;
              if (have) {
                msg = "กลิ่น" + real + " ของ " + mdl + " มีพร้อมส่งค่ะ 💕";
                if (isSwap) { msg += "\n\nรบกวนรอทีมงานยืนยันการแก้ไขรายการสักครู่นะคะ 🙏🏻"; await muteNow("ขอเปลี่ยนกลิ่น/รายการ (จ่ายเงินแล้ว)", text); }
                else msg += "\n\nสนใจรับกี่ชิ้นดีคะ 😊";
              } else {
              ...
              await lineReply(TOKEN, replyToken, msg, userId);
              return;
              }   // ปิด else ของ k170 (เปลี่ยนกลิ่นก่อนจ่าย จะไม่เข้ามาถึงตรงนี้)
            }
          }
        }
      }
      ...
      if (/มีกลิ่น(อะไร|ไหน|ใด)|กลิ่น(อะไร|ไหน)บ้าง|กลิ่นอะไรมั่ง|เหลือกลิ่น(อะไร|ไหน)|เหลืออะไรบ้าง|มีอะไรเหลือ|มีสีอะไรบ้าง|สีอะไรบ้าง|ครบทุกกลิ่น(มั้ย|ไหม)?|กลิ่นครบ(มั้ย|ไหม)|กลิ่น(อะไร|ไหน)หมด|หมดกลิ่น(อะไร|ไหน)|หมดอะไรบ้าง/i.test(text)) {
        ...
        if (!mdl && !_wantRec) {
          ...
          await lineReply(TOKEN, replyToken, "ได้เลยค่ะ 💕 รบกวนบอกรุ่นที่สนใจหน่อยนะคะ เดี๋ยวแอดมินเช็คกลิ่นที่มีของจริงให้ทันทีค่ะ\n(เช่น MARBO 9K · INFY · RELX · ESKO BAR · KS QUIK — หรือบอกแนวที่ชอบก็ได้ค่ะ เช่น สายเย็น สายผลไม้ สายหวาน)", userId);
          return;
        }
        if (mdl) {
      ...
      const hint = _explainQ
        ...
        : (function () {
            ...
            let _prevCat = "";
            try { const _pc = slotCurrent(pslot); if (_pc && _pc.model) _prevCat = topicOfModel(_pc.model); } catch (e) {}
            ...
            _AMBIG_CTX = _ambigHard ? { why: _ambig, t: Date.now() } : null;
            ...
            let h = aliasHint(tForHint) + flavorHint(tForHint, smForHint, bufForHint) + brandHint(tForHint, smForHint, bufForHint) + legoHint(tForHint, smForHint, bufForHint) + locHint(textH) + flavorSearchHint(tForHint, smForHint, bufForHint) + styleHint(tForHint, smForHint, bufForHint) + unknownAskHint(textH, smForHint, bufForHint) + typoHint(textH, smForHint, bufForHint) + _catList + _ambigBlock;
    ...
    try {
      if (reply.indexOf("ทีมงานหลังการขาย") !== -1) {
        ...
        const _real = _mbk || _agr || /ของ(ยัง)?ไม่(ถึง|มา|ได้)|ยังไม่ได้(รับ)?ของ|ของยังไม่มา|ของหาย|พัสดุหาย|พัสดุยังไม่|ส่งช้า|ล่าช้า|ยังไม่ส่ง|ของไม่ครบ|ได้ไม่ครบ|ของเสีย|ของพัง|ใช้ไม่ได้|สูบไม่ขึ้น|หัวตัน|น้ำยาซึม|เครื่องไม่ติด|ชาร์จไม่เข้า|ตีกลับ|คืนเงิน|เคลม|ส่งผิด|ได้ผิด|ไม่ตรงปก|แก้ออเดอร์|เปลี่ยนที่อยู่|ยกเลิกออเดอร์|ร้องเรียน|โกง|แย่มาก|เลขพัสดุ|เช็คสถานะ|คุยกับทีมงาน|ขอทีมงาน|ขอคุยคน/.test(_um);
        if (!_real) {
          _skipMute = true;
    ...
    // 🔒 k35: กันข้อมูลภายในหลุด — เคสจริง 31/7: แอดมินพิมพ์ "(จำนวนภายใน 190 — ห้ามบอกลูกค้า)" ให้ลูกค้าเห็น
    try {
      ...
      if (/(เหลือ|มี|สต็อก)?[^\n]{0,10}(ค่อนข้าง)?จำกัด|เหลือน้อย|ใกล้หมด|มีไม่มาก|ไม่ค่อยเหลือ|เหลือไม่เยอะ|ร่อยหรอ|รีบ ?ๆ? ?สั่ง/.test(reply)) {
        ...
        if (reply.replace(/[\s\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}️]/gu, "").length < 8)
          reply = "บอกกลิ่นที่สนใจมาได้เลยค่ะ 💕 เดี๋ยวแอดมินเช็คให้ว่ามีของไหมนะคะ";
      ...
      // 🆕 k133 (เคสจริง 3/8 17.41): ลูกค้าถาม MARBO 9K แท้ → ระบบตอบ 3 รอบ ขัดกันเองทุกรอบ
      //   "ของหมด/รอของเข้าอยู่ค่ะ" → "หมดทุกรสชาติค่ะ" → "หมดชั่วคราว" **แล้วลิสต์ 7 กลิ่นที่มีของต่อทันที**
      //   ลูกค้าถามย้ำ 2 รอบว่า "ทำไมสินค้าหมดไวจังคะ" = ไม่เชื่อแล้ว และเกือบเสียลูกค้าประจำไปเลย
      //   ต้นเหตุ: ด่านเดิมจับแค่คำว่า "หมดทุกกลิ่น" แต่ AI เขียนว่า "หมดทุกรสชาติ" → ด่านไม่ทำงาน
      //   → ขยายคำที่จับ + เพิ่มด่านจับ "ขัดกันเองในข้อความเดียว" (บอกหมดทั้งรุ่น แต่ลิสต์กลิ่นที่มีของ)
      if (/หมดทุกกลิ่น|หมดทุกรสชาติ|หมดทุกรส|หมดทุกตัว|หมดทุกแบบ|หมดทั้งรุ่น|หมดยกรุ่น|หมดหมดทุก|ไม่เหลือกลิ่นไหนเลย|ไม่มีของเลย|ของหมด\s*\/\s*รอของเข้า/.test(reply)) {
        try {
          ...
          for (const m in FLAVORS) {
            ...
            if (left.length) {
              reply = reply.replace(/หมดทุกกลิ่น|หมดทุกรสชาติ|หมดทุกรส|หมดทุกตัว|หมดทุกแบบ|หมดทั้งรุ่น|หมดยกรุ่น|ไม่เหลือกลิ่นไหนเลย|ไม่มีของเลย|ของหมด\s*\/\s*รอของเข้า/g, "หมดบางกลิ่น")
      ...
      // 🤥 k137: ห้าม "รับปากว่าคนกำลังมา" โดยที่ไม่ได้เรียกคนจริง
      //   เคสจริง 3/8 18.31: "ทีมงานกำลังเดินทางมาแล้วค่ะ" — ไม่มีใครถูกเรียก ไม่มีอะไรเข้าคิว
      //   ลูกค้ารอเก้อ 3 รอบ แล้วร้านไม่รู้ตัวด้วยซ้ำว่ามีคนรออยู่
      //   กฎ: ถ้าคำตอบพูดว่าทีมงาน/คนจริงกำลังมา → ระบบต้องส่งเข้าคิวจริงตรงนั้นเลย
      try {
        if (/ทีมงาน[^\n]{0,20}(กำลัง|เดินทาง|จะเข้า|เข้ามา|ดูแลให้|ติดต่อกลับ)|คนจริง[^\n]{0,20}(กำลัง|จะเข้า|เข้ามา)|เรียก[^\n]{0,10}ให้ทันที|เร่งทีมงาน/.test(reply)) {
          const _mk = "mute:" + shopId + ":" + userId;
          const _already = MUTED.has(_mk) || (env.CONV && await env.CONV.get(_mk));
          if (!_already) {
            console.log("K137_PROMISE_WITHOUT_QUEUE → บังคับเข้าคิว");
            await muteNow("🙋 บอทรับปากว่าคนจะเข้ามาคุย — ต้องเข้าไปจริง", String(msgText || "").slice(0, 80));
          }
        }
      } catch (e) {}
      // 📍 k134: รู้หมุด/ค่าส่งด่วนแล้ว → ห้ามขอหมุดซ้ำเด็ดขาด (ไม่ว่า AI จะเขียนมายังไง)
      //   เคสจริง 3/8 17.56: การ์ด 823 (รวมค่าส่งด่วน 123) ออกไปแล้ว ลูกค้าถาม "ได้ของกี่โมง"
      //   → ระบบตอบ "รบกวนแชร์โลเคชั่น เดี๋ยวทีมงานเช็คค่าส่งด่วน" ลูกค้างงว่าส่งไปแล้วนี่
      try {
        let _exG = null;
        try { if (env.CONV) { const v = await env.CONV.get("exp:" + shopId + ":" + userId); if (v) _exG = JSON.parse(v); } } catch (e) {}
        const _efG = EXPFEE.get(shopId + ":" + userId);
        const _known = (_efG && Date.now() - _efG.t < 7200000) || (_exG && (typeof _exG.fee === "number" || _exG.lat));
        if (_known && !(_exG && _exG.outzone) && /แชร์โลเคชั่น|แชร์โลเคชัน|ปักหมุด|ส่งลิงก์\s*Google\s*Maps|ตำแหน่งที่ตั้ง/i.test(reply)) {
          const _fee = (_efG && _efG.fee) != null ? _efG.fee : (_exG && _exG.fee);
          console.log("K134_PIN_REASK_BLOCKED fee=" + _fee);
      ...
      //   เคสจริง 3/8 17.41: "MARBO 9K แท้ ... หมดชั่วคราวค่ะ" แล้วบรรทัดถัดมาลิสต์ 7 กลิ่นที่มีของ
      //   ลูกค้าอ่านแล้วงงว่าตกลงมีหรือไม่มี → ถามย้ำ 2 รอบ "ทำไมสินค้าหมดไวจังคะ"
      //   กฎ: ถ้าในข้อความเดียวมีทั้ง "บอกว่ารุ่นนี้หมด" และ "ลิสต์กลิ่นของรุ่นนั้นที่มีของ ≥2 กลิ่น"
      //        → ประโยคที่บอกหมดคือของปลอม ตัดทิ้ง เหลือแต่ลิสต์จริง
      try {
        for (const m in FLAVORS) {
          if (m.length < 5 || reply.indexOf(m) === -1) continue;
          const fl = FLAVORS[m].f || [];
          // นับกลิ่นของรุ่นนี้ที่ถูก "ลิสต์" ในคำตอบ (ขึ้นต้นด้วย - หรือ •)
          let listed = 0;
          for (const f of fl) {
            const bare = String(f).replace(/\s*\d+(\.\d+)?%\s*$/, "").trim();
            if (bare.length >= 3 && new RegExp("[-•]\\s*" + bare.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).test(reply)) listed++;
          }
          if (listed < 2) continue;
          // มีลิสต์กลิ่นจริง → ประโยคที่บอกว่า "ทั้งรุ่นหมด" ต้องหายไป
          const before = reply;
          reply = reply.split("\n").filter(ln => {
            if (ln.indexOf(m) === -1) return true;
            return !/หมด|ไม่มีของ|รอของเข้า|of\s*stock/i.test(ln);
          }).join("\n").replace(/\n{3,}/g, "\n\n").trim();
          if (reply !== before) { console.log("K133_CONTRADICTION_FIXED " + m + " listed=" + listed); }
          break;
        }
      } catch (e) {}
      ...
      if (SOLD_OUT_MODELS.length) {
        ...
        if (named.length) {
          // 🩹 k181 (QA · เคสจริงลูกค้า Hunter): ขอโทษเฉพาะรุ่นที่ "ลูกค้าถามถึงเอง" เท่านั้น
          //   เดิมเอารุ่นที่หมดทุกตัวที่เจอในคำตอบมาขึ้นต้นเป็นคำขอโทษ
          //   เคสจริง: ลูกค้าถาม "แล้วแต่สูบทิ้งมีไหมครับ" (ไม่ได้เอ่ยรุ่นไหนเลย)
          //     → ตอบ "ขออภัย RELX POD CLEAR 18K · ABC 8K · INFY 12K หมดชั่วคราวค่ะ"
          //     → ลูกค้าอ่านแล้วเหมือนร้านไม่มีอะไรขาย ทั้งที่มีของ = เสียการขายฟรีๆ
          //   ตัวตรวจจับถูกอยู่แล้ว (ห้ามเสนอของหมด) — ที่ผิดคือ "วิธีเขียนคำตอบใหม่"
          //   กฎ: รุ่นที่ลูกค้าไม่ได้ถาม ตัดออกเงียบๆ · ขอโทษเฉพาะรุ่นที่ลูกค้าเอ่ยถึงจริง
          //   "ลูกค้าถามถึงรุ่นนี้ไหม" ต้องดู 3 ทาง ไม่ใช่เทียบชื่อเต็มอย่างเดียว
          //     1) ตัวแปลชื่อสินค้าชี้ตรงรุ่นนั้น ("หัวดำ" → RELX BOOST POD)
          //     2) ลูกค้าพิมพ์ชื่อรุ่นมาตรงๆ (เต็มหรือบางส่วน เช่น "ABC LEGO" ของ "ABC LEGO 20K")
          //     3) ลูกค้าถามระดับแบรนด์ ("แบรนด์ abc มีของมั้ย") = ถามถึงทุกรุ่นในแบรนด์นั้น
          const _askRaw = String(msgText || "");
          const _askTxt = normTH(_askRaw);
          let _askedM = "", _askedBrands = [];
          try { _askedM = _MODEL_IN(_askRaw) || ""; } catch (e) {}
          try { _askedBrands = BRAND_TH.filter(([re]) => re.test(_askRaw)).map(([, b]) => b); } catch (e) {}
          const _brandOfModel = (k) => { try { const h = BRAND_TH.find(([re]) => re.test(k)); return h ? h[1] : ""; } catch (e) { return ""; } };
          const _askedGone = named.filter(m => {
            const nm = normTH(m);
            if (_askedM && normTH(_askedM) === nm) return true;
            if (_askTxt && (_askTxt.indexOf(nm) !== -1 || nm.indexOf(_askTxt) !== -1)) return true;
            if (_askedBrands.length && _askedBrands.indexOf(_brandOfModel(m)) !== -1) return true;
            return false;
          });
          const gone = (_askedGone.length ? _askedGone : named).join(" · ");
          const _quiet = !_askedGone.length;      // ลูกค้าไม่ได้ถามถึงรุ่นที่หมดเลย → ไม่ต้องขึ้นคำขอโทษ
          ...
          // 🩹 k181 (QA): เดิมตัด "ทั้งบรรทัด" ที่มีชื่อรุ่นหมด
          //   แต่ AI มักตอบหลายรุ่นในบรรทัดเดียว เช่น "แนะนำ ABC 8K · POD CLEAR · MARBO 9K ค่ะ"
          //   → รุ่นที่ "มีของ" ในบรรทัดเดียวกันถูกตัดทิ้งไปด้วย = ลูกค้าไม่เห็นของที่ซื้อได้เลย
          //   กฎ: ตัดที่ระดับ "รุ่น" ไม่ใช่ระดับ "บรรทัด" — เหลือรุ่นที่มีของไว้เสมอ
          const _ตัดรุ่นหมด = (l) => {
            if (!named.some(m => l.indexOf(m) !== -1)) return l;
            const parts = l.split(/\s*[·•]\s*|\s*,\s*|\s+\/\s+/);
            if (parts.length < 2) return "";                       // มีรุ่นเดียวในบรรทัด → ตัดทั้งบรรทัดเหมือนเดิม
            const keep = parts.filter(seg => !named.some(m => seg.indexOf(m) !== -1));
            if (!keep.length) return "";
            const txt = keep.join(" · ").replace(/\s{2,}/g, " ").trim();
          ...
          reply = reply.split("\n")
            ...
            .join("\n")
            .replace(/\n{3,}/g, "\n\n").trim();
          const _เหลือ = reply.replace(/[\s\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}️]/gu, "").length >= 8;
          //   เงียบได้เฉพาะตอน "ยังมีของอย่างอื่นให้เสนอ" เท่านั้น
          //   ถ้าตัดแล้วไม่เหลืออะไรเลย ต้องบอกว่าหมด — ไม่งั้นลูกค้าได้ข้อความลอยๆ ที่ไม่มีข้อมูลอะไรเลย
          //   ⚠️ "ยังมีเนื้อหาเหลือ" ไม่พอ — ต้องเหลือ **ชื่อรุ่นที่เสนอขายได้จริง** ด้วย
          //     ไม่งั้นประโยคปิดท้ายลอยๆ อย่าง "สนใจตัวไหนดีคะ" จะถูกนับว่ามีของเสนอ
          let _มีรุ่นเสนอ = false;
          try { for (const nm of MODEL_NAMES_LONGFIRST) { if (nm.length >= 4 && reply.indexOf(nm) !== -1) { _มีรุ่นเสนอ = true; break; } } } catch (e) {}
          reply = (_quiet && _เหลือ && _มีรุ่นเสนอ)
            ? reply
            : "ขออภัยด้วยนะคะ 🙏🏻 ตอนนี้ " + gone + " หมดชั่วคราวค่ะ\n\n"
              + (_เหลือ ? reply + "\n\n" : "")
              + "บอกแนวกลิ่นหรืองบที่ต้องการมาได้เลยค่ะ เดี๋ยวแอดมินหารุ่นที่มีของจริงให้แทนนะคะ 💕";
          console.log("K181_SOLDOUT_STRIPPED " + named.join(",") + " ลูกค้าถามถึง=" + (_askedGone.join(",") || "ไม่มี"));
        }
      }
      ...
      if (reply.replace(/[\s\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}️]/gu, "").length < 8) {
        // k127: เดิมตอบตายตัวว่า "กลิ่นนี้มีพร้อมส่ง" ทุกกรณี — ผิดหนักถ้าไม่มีบริบทกลิ่นเลย
        //   เคสจริง 3/8: ลูกค้าพิมพ์แค่ "?" → ระบบตอบ "กลิ่นนี้มีพร้อมส่งค่ะ สนใจรับกี่ชิ้นดีคะ" = ไม่รู้เรื่อง
        //   และวงเล็บ "ขอสงวนการแจ้งจำนวนสต็อก" ทำให้ดูบ่ายเบี่ยง → ตัดออก
        let _hasCtx = false;
        try { _hasCtx = !!_MODEL_IN(String(msgText || "")); } catch (e) {}
        reply = _hasCtx
          ? "กลิ่นนี้มีพร้อมส่งค่ะ 💕 สนใจรับกี่ชิ้นดีคะ"
          : "ขออภัยค่ะ 🙏🏻 แอดมินยังไม่แน่ใจว่าคุณลูกค้าหมายถึงรุ่นไหนนะคะ\nรบกวนบอกชื่อรุ่นหรือแนวกลิ่นที่ชอบมาได้เลยค่ะ เดี๋ยวแอดมินเช็คของให้ทันทีค่ะ 💕";
        console.log("EMPTY_AFTER_FILTER fixed ctx=" + _hasCtx);
      }
      // 💸 k41: ห้ามแอดมินลดราคา/แถมเอง — กฎเจ้าของร้าน (เดิมมีแค่ในคำสั่ง ไม่มีตัวกรองจริง)
      ...
      if (/(?<!ถ้า|หาก)สลิปถูกต้อง|ตรวจ(?:สอบ)?สลิป(?:เรียบร้อย)?(?:แล้ว|ผ่าน)|เงินเข้าบัญชี(?:ร้าน)?(?:แล้ว|เรียบร้อย)|ยืนยันการชำระ(?:เงิน)?(?:เรียบร้อย)?แล้ว|ได้รับยอด(?:เงิน|โอน)/.test(reply)) {
        ...
        try {
          if (env.CONV) {
            const _ov = await env.CONV.get("ord:" + shopId + ":" + userId);
            if (_ov) { const _oj = JSON.parse(_ov); if (ordPaidActive(_oj)) _paidOk = true; }
          }
        } catch (e) {}
        if (!_paidOk) {
      ...
      // 🤥 k55: ห้ามรับปากว่า "เดี๋ยวจะกลับมาแจ้ง" — แอดมินไม่มีทางกลับมาทักเองได้ = โกหกลูกค้า
      // เคสจริง 1/8: "ระบบสต็อกกำลังอัปเดตอยู่ เดี๋ยวอีกสักครู่นะคะ จะแจ้งกลิ่นที่มีของจริงให้ทราบทันทีค่ะ"
      //   → ลูกค้ารอเก้อ ไม่มีใครทักกลับ (และ "ระบบกำลังอัปเดต" ก็เป็นเรื่องที่แอดมินแต่งขึ้นเอง)
      // ⚠️ k58: เดิมเขียนกว้างไป ไปจับ "เดี๋ยวแอดมินเช็คให้ทันทีค่ะ" ซึ่งเป็นคำพูดปกติ
      //   ทำให้ลูกค้าถาม "ทำไมต้องรอบสุดท้าย" แล้วได้คำตอบว่า "ตอนนี้เช็ครายการให้ไม่ได้ค่ะ" = ตอบไม่ตรงคำถาม
      //   ตอนนี้จับเฉพาะ "รับปากว่าจะกลับมาทักเอง" จริงๆ เท่านั้น (แอดมินทักเองไม่ได้ = โกหก)
      if (/จะ(กลับมา)?แจ้ง(ให้ทราบ|ให้อีกครั้ง|ให้อีกที|กลับ)|กลับมาแจ้ง|แจ้งกลับ(ให้|ไป)|(รอ|อีก)สักครู่[^\n]{0,25}(จะ|แล้ว)[^\n]{0,10}แจ้ง|ระบบ(สต็อก)?กำลัง(อัปเดต|อัพเดต|ประมวลผล)|(แจ้ง|บอก)(ให้ทราบ|ให้)[^\n]{0,10}(ในภายหลัง|เร็วๆ ?นี้|ทีหลัง)/.test(reply)) {
        reply = "ขออภัยด้วยนะคะ 🙏🏻 ตอนนี้แอดมินยังเช็ครายการให้ไม่ได้ค่ะ\n\nรบกวนบอกรุ่นที่สนใจมาได้เลยนะคะ เดี๋ยวแอดมินเช็คกลิ่นที่มีของให้ทันทีค่ะ 💕\nหรือดูรายการทั้งหมดที่มีของจริงได้จากเมนูนี้เลยค่ะ\nhttps://cutt.ly/menu4";
    ...
    // 🛡 กันแอดมินแจกของแถม/เลขบัญชีเอง (ระบบเป็นคนออกการ์ดของแถม + การ์ดชำระเงินเท่านั้น)
    try {
      ...
      // 2.5) 💸 ห้ามราคาเป็นช่วง — MARBO แท้/โคลน คนละตัว (เคสจริง: แอดมินเขียน "MARBO 9K (290-350 บาท)")
      ...
      // เคสจริง 31/7: ลูกค้าถาม "ชอบแนวเย็นๆหวานๆ" → แอดมินมโนชื่อกลิ่นที่ MARBO 9K ไม่มี
      ...
      try {
        ...
        // เคสจริง 31/7: ลูกค้าถาม "ไอคอส คืออะไร" (ถามความหมาย) → แอดมินเอาลิสต์กลิ่น TEREA มาแปะหัวข้อความ
        ...
        if (_intro && _bullets === 0 && _hintModels.length && _sm) {
          ...
          if (_have.length) {
            ...
            // k164: เครื่องมี "สี" ไม่ใช่ "กลิ่น" — เดิมพูดว่า "กลิ่นแนวที่คุณลูกค้าชอบ" กับเครื่อง = ลูกค้างง
            const _u = unitWord(_mdl);
            reply = "ต้องขออภัยด้วยนะคะ 🙏🏻 ตอนนี้ " + _mdl + " หมดชั่วคราวทุก" + _u + "ค่ะ\n\nรบกวนบอก" + _u + "หรือรุ่นอื่นที่สนใจมาได้เลยค่ะ เดี๋ยวแอดมินเช็คให้ทันที หรือดูรุ่นที่มีของทั้งหมดได้ที่เมนูนี้เลยค่ะ 💕\nhttps://cutt.ly/menu4";
        ...
        if (!_hintModels.length && _bullets >= 3 && /กลิ่น|หวาน|เย็น|แนะนำ/.test(reply)
          ...
          reply += "\n\n⚠️ กลิ่นที่มีจริงขึ้นกับแต่ละรุ่นนะคะ บอกรุ่นที่สนใจมาได้เลยค่ะ เดี๋ยวแอดมินเช็คให้ว่ารุ่นนั้นมีกลิ่นไหนพร้อมส่งบ้าง 💕";
    ...
    try {
      const _askedM = (Array.isArray(_hintModels) && _hintModels.length === 1) ? _hintModels[0] : "";
      if (_askedM && env.CONV) {
      ...
      if (_mg) {
        ...
        if (!_hit) {
          ...
          reply = _cands.length
            ...
            : "ขอเช็คให้ตรงนิดนึงนะคะ 🙏🏻 รบกวนพิมพ์ชื่อกลิ่นที่ต้องการเต็มๆ อีกครั้งค่ะ (หรือบอกแนวที่ชอบก็ได้ค่ะ เช่น มิ้นต์เย็น ผลไม้ องุ่น) เดี๋ยวแอดมินเช็คกลิ่นที่มีของให้ทันทีค่ะ 💕";
        }
      }
    } catch (e) {}
    ...
    try {
      ...
      try {
        ...
        if (!reply) reply = "รับทราบค่ะ 💕 มีอะไรให้แอดมินช่วยอีกไหมคะ";
      } catch (e) {}
    ...
    if (reply.indexOf("ทวนคำสั่งซื้อ") !== -1) {
      ...
      if (notReady) {
        ...
        if (calc.rows.some(r => r.unknown && !r.free) || calc.goods <= 0) {
          ...
          if (_again >= 2) {
            await lineReply(TOKEN, replyToken, "ขออภัยที่ถามซ้ำนะคะ 🙏🏻 แอดมินยังจับชื่อรุ่นไม่ได้ค่ะ\n\nเลือกจากเมนูนี้แล้วส่งชื่อรุ่นมาได้เลยค่ะ 👇\nhttps://cutt.ly/menu4\n(หรือส่งรูปสินค้ามาก็ได้นะคะ เดี๋ยวแอดมินเช็คให้ทันทีค่ะ 💕)", userId);
        ...
        try {
          if (env.CONV) {
            ...
            try { if (await sendPayInfo(env, TOKEN, "", userId, shopId, { block })) console.log("K188_PAY_AFTER_TOTAL"); } catch (e) {}
          }
        } catch (e) {}
      } else {
...
async function lineReply(token, replyToken, text, userId, quick, sysAck, force) {
  ...
  try {
    if (userId && !sysAck) {
      ...
      const _prev = LASTOUT.get(userId);
      let _sameAsk = false;
      if (_prev && _prev.sig === _sig && Date.now() - _prev.t < 1800000) {
        _sameAsk = !!_askNow && _askNow === _prev.ask;
        _dupN = _sameAsk ? 0 : (_prev.n || 1) + 1;
        console.log("K180_DUP_REPLY n=" + _dupN + " ลูกค้าถามเรื่องเดิม=" + _sameAsk + " msg=" + msg.slice(0, 40));
      }
      _repeatSameAsk = _sameAsk;
      LASTOUT.set(userId, { sig: _sig, ask: _askNow, t: Date.now(), n: _dupN || 1 });
  ...
  const one = { type: "text", text: _repeatSameAsk
    ? "ยืนยันอีกครั้งนะคะ 💕\n\n" + msg
    : _dupN >= 3
