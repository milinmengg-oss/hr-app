...
const BUILD = "2026-08-03-k139-nodupcancel";
  ...
  try {
    ...
    if (mtype === "text") {
      ...
      // ✅ ยกเว้นการ "ถามเงื่อนไข" (เคลมยังไง/กี่วัน) → ปล่อยให้ตอบข้อมูลตามปกติ ไม่ต้องส่งต่อ
      {
        ...
        const moneyBack = /(ขอ|เอา|โอน|จะเอา|อยากได้)?\s*(ตัง|ตังค์|สตางค์|เงิน|ค่าของ|ยอด)\s*(คืน)|คืน\s*(ตัง|ตังค์|เงิน)|คืนเงิน|refund|ไม่อยากได้แล้ว|ไม่เอาของแล้ว|เปลี่ยนใจ.{0,10}(ไม่เอา|คืน)/i.test(t);
        // 😠 ลูกค้าเริ่มไม่พอใจ → ห้ามยิงข้อความสำเร็จรูปซ้ำอีก ต้องให้คนเข้ามาคุย
        const angry = /ไม่ให้อภัย|ตอบมา\b|ทำไมไม่เข้าใจ|ตอบไม่ตรง|ถามอยู่|ห่วย|แย่มาก|แจ้งตำรวจ|สคบ|โกง|หลอก|จะร้องเรียน|เอาเรื่อง|ไม่พอใจ|งี่เง่า|โง่/.test(t);
