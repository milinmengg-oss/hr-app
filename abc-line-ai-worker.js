...
const BUILD = "2026-08-01-k55-carrymodel";
...
async function handleEvent(ev, env, TOKEN, shopId) {
  try {
    ...
    if (mtype === "text") {
      ...
      // 💧 k46: "Freebase กับ Saltnic คืออะไร / ต่างกันยังไง" → อธิบายตายตัว ⛔ ห้าม AI เอาสต็อกมาตอบ
      if (/freebase|ฟรีเบส|saltnic|salt ?nic|ซอลนิค|ซอลท์นิค/i.test(t)
        && /คือ ?อะไร|คืออะไร|ต่างกัน|ต่างยังไง|แปลว่า|หมายถึง|เลือกยังไง|อันไหนดี|แบบไหนดี|เหมาะกับ|มือใหม่|อธิบาย|difference|what is/i.test(t)) {
...
"7) ถ้ารูปไม่มีการวง/ทำเครื่องหมายเลย → ถามว่า \"สนใจรุ่นไหนในภาพคะ 💕 บอกชื่อรุ่นกับกลิ่นมาได้เลยนะคะ\"" },
      ...
      const hint = _explainQ
        ...
        : (function () {
            ...
            let h = aliasHint(tForHint) + flavorHint(tForHint, smForHint, bufForHint) + brandHint(tForHint, smForHint, bufForHint);
    ...
    try {
      ...
      if (/(จะ|เดี๋ยว)[^\n]{0,25}(แจ้ง|บอก|ส่ง|อัปเดต|อัพเดต)[^\n]{0,25}(ให้ทราบ|ให้อีกที|ให้อีกครั้ง|กลับมา|ทันที|ในภายหลัง|เร็วๆ ?นี้|สักครู่)|รอสักครู่[^\n]{0,30}(แจ้ง|บอก)|ระบบ(สต็อก)?กำลัง(อัปเดต|อัพเดต|ประมวลผล)/.test(reply)) {
