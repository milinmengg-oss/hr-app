...
const BUILD = "2026-07-31-k35-noleak-promo";
...
async function handleEvent(ev, env, TOKEN, shopId) {
  try {
    ...
    if (mtype === "text") {
      ...
      if (/โปร(โมชั่น|โมชัน)?\s*(อะไร|ไหน|มี|บ้าง|ตอนนี้)|มีโปร|ส่วนลด|ลดราคา|ซื้อ.{0,6}แถม|โปรอะไร/.test(t) && !/แจ้งปัญหา|ของเสีย|เคลม/.test(t)) {
