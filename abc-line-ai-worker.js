...
const BUILD = "2026-08-02-k82-confirmfix";
...
function unknownAskHint(text, sm, buf) {
  ...
  if (!/มี.{0,15}(มั้ย|ไหม|ป่าว|เปล่า|บ้าง)|เอา|ขอ(?!ง)|สั่ง|ราคา|เท่าไห?ร่|กี่บาท|ตัวไหน|รุ่นไหน|ขาย/.test(s)) return "";
...
✅ รูปแบบที่ถูก: "เรื่องนี้อัญญาไม่มีข้อมูลค่ะ 🙏🏻 เดี๋ยวแอดมินเช็คให้นะคะ" แล้วชวนกลับเข้าเรื่องสินค้า
  ...
  try {
    ...
    if (mtype === "text") {
      ...
      if (/แอดมินชื่อ|ชื่ออะไรครับ|ชื่ออะไรคะ|ชื่อไรครับ|ชื่อไรคะ|คุณชื่ออะไร|เรียกว่าอะไร/.test(t)
        ...
        await lineReply(TOKEN, replyToken,
          "อัญญาเป็นผู้ช่วย AI ของร้าน ABC ค่ะ 🤖 ตอบได้ตลอด 24 ชม.เลยนะคะ\n" +
      ...
      if (/^(เดี๋ยว|ไว้)(ค่อย)?(สั่ง|ซื้อ|เอา)(ใหม่|ทีหลัง|ก่อน)?|ไว้ก่อน|ขอคิดดูก่อน|ขอดูก่อน|เดี๋ยวมาใหม่|แป๊บนึง|ขอเวลาคิด/i.test(t)) {
        await lineReply(TOKEN, replyToken, "ได้เลยค่ะ 💕 ทักมาได้ตลอดเลยนะคะ อัญญาอยู่ 24 ชม.ค่ะ 🙏🏻", userId);
        return;
      }
