...
const BUILD = "2026-08-05-k182-expupcountry";
...
async function handleEventCore(ev, env, TOKEN, shopId) {
  try {
    ...
    if (mtype === "text" && env.CONV) {
      ...
      const _askTotal = /ต้องโอน(ยอด)?(ไหน|เท่า ?ไหร?ร่|กี่บาท)|โอนยอดไหน|ยอด(รวม)?(เท่า ?ไหร?ร่|กี่บาท)|กี่บาท(คะ|ครับ)?$|ทั้งหมดเท่า ?ไหร?ร่|สรุปยอด|ยอดสุทธิ/.test(_tq)
        || /\d{2,5}\s*(หรือ|รึ|หรอ)\s*\d{2,5}/.test(_tq);   // "350 หรือ 509 คะ" = กำลังสับสนเรื่องยอด
      if (_askTotal) {
        try {
...
"   ⛔ ห้ามตัดกลิ่นใดกลิ่นหนึ่งทิ้งเงียบๆ เด็ดขาด ถ้ากลิ่นไหนหมด ต้องบอกลูกค้าตรงๆ ว่ากลิ่นนั้นหมด ห้ามหายไปเฉยๆ" },
      ...
      let _preOrder = "";
      try {
        const _askTotal = /รวมยอด|สรุปยอด|คิดยอด|คิดเงิน|คิดตัง|ทั้งหมดเท่าไห?ร่|รวมเท่าไห?ร่|ยอดเท่าไห?ร่|เอาตามนี้|ตามนี้เลย|ก็แจ้งแล้ว|แจ้งไปแล้ว/.test(t);
        if (_askTotal) {
