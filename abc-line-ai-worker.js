...
const BUILD = "2026-08-04-k164-answerfix";
  ...
  try {
    ...
    if (mtype === "text") {
      ...
      // ✅ ยกเว้นการ "ถามเงื่อนไข" (เคลมยังไง/กี่วัน) → ปล่อยให้ตอบข้อมูลตามปกติ ไม่ต้องส่งต่อ
      {
        ...
        const _softQuit = /ไม่อยากได้แล้ว|ไม่อยากได้ละ|ไม่เอาของแล้ว|ไม่เอาแล้ว/.test(t)
          && !/(ตัง|ตังค์|สตางค์|เงิน|ค่าของ|ยอด)\s*คืน|คืนเงิน|โอนคืน|โอนกลับ|refund/i.test(t);
        let _hasOrder = false;
        try {
          if (env.CONV) {
            const _ov = await env.CONV.get("ord:" + shopId + ":" + userId);
            if (_ov) { const _oj = JSON.parse(_ov); _hasOrder = !!(_oj && _oj.status); }
          }
        } catch (e) {}
        if (_softQuit && !_hasOrder) console.log("K163_SOFT_QUIT_NOT_REFUND (ไม่มีออเดอร์ค้าง)");
        const moneyBack = RX_MONEYBACK.test(t) && !(_softQuit && !_hasOrder);   // k147b + k163
        ...
        if (moneyBack || angry) {
          await muteNow(moneyBack ? "💸 ลูกค้าขอเงินคืน — ต้องให้คนตัดสิน ด่วน" : "😠 ลูกค้าไม่พอใจ — ต้องให้คนเข้าไปคุย ด่วน", t);
          await lineReply(TOKEN, replyToken,
            moneyBack
