...
- ส่งด่วน (เฉพาะ กทม.+ปริมณฑล) = คิดตามระยะทาง ให้ขอลูกค้า "แชร์โลเคชั่น (ปักหมุด) หรือส่งลิงก์ Google Maps" มา แล้ว **ทีมงานจะเช็คราคาจากแอปขนส่งจริงให้** (⛔ แอดมินห้ามกุตัวเลขค่าส่งด่วนเองเด็ดขาด ทุกกรณี — ราคาต้องมาจากแอดมินเท่านั้น)
...
- ค่าส่งด่วนคิดตามระยะทาง — ถ้าลูกค้าเลือกส่งด่วน ให้ขอโลเคชั่น(ปักหมุด)มาก่อน พอลูกค้าปักหมุด ทีมงานจะเช็คราคาจากแอปจริงแล้วแจ้งเอง (⛔ แอดมินห้ามกุตัวเลขค่าส่งด่วนเองทุกกรณี, ห้ามระบุชื่อบริษัทขนส่ง)
...
- ส่งด่วน: มีบริการในบางพื้นที่ (กทม.+ปริมณฑล) ตอบว่า "มีบริการส่งด่วนค่ะ 🛵 รบกวนคุณลูกค้าแชร์โลเคชั่น (ปักหมุด) หรือส่งลิงก์ Google Maps มาให้หน่อยนะคะ เดี๋ยวทีมงานเช็คราคาจากแอปให้ทันทีค่ะ" — ⛔ ห้ามกุค่าส่งด่วนเอง ⛔ ห้ามพูดว่า "กำลังประสานทีมขนส่ง / รอ 5-10 นาที" หรือกุเวลารอใดๆ เด็ดขาด
    ...
    if (mtype === "text") {
      ...
      if (/(?:https?:\/\/)?[^\s]*(?:maps\.app\.goo\.gl|goo\.gl\/maps|google\.[a-z.]+\/maps|maps\.google)/i.test(t)) {
        ...
        if (ll) {
          ...
          await lineReply(TOKEN, replyToken, "ขออภัยค่ะ อ่านพิกัดจากลิงก์นี้ไม่ได้ 🙏🏻 รบกวนกดปุ่ม + ในแชท → เลือก 'ตำแหน่งที่ตั้ง' → ปักหมุดจุดจัดส่ง → กดส่ง มาอีกครั้งนะคะ เดี๋ยวทีมงานเช็คราคาจากแอปให้ค่ะ 🛵", userId);
        }
        return;
      }
      ...
      if (/แกร?[็ๆ]?[บป]|grab|ไรเดอร์|rider|มอเตอร์ไซค์|วินมอไซ|เมสเซนเจอร์|messenger|ลาลามูฟ|lalamove|ตามหมุด|ที่หมุด|ตามพิกัด/i.test(t)
        ...
        if (exp && exp.outzone) {
          ...
          await lineReply(TOKEN, replyToken, "ได้รับหมุดแล้วนะคะ 📍 ทีมงานกำลังเช็คค่าส่งด่วนจากแอปให้อยู่ค่ะ เดี๋ยวแจ้งราคาทันทีนะคะ 🛵💕", userId);
      ...
      if (/รูปแบบการจัดส่ง|วิธีการจัดส่ง|การจัดส่ง|ค่าส่งเท่าไหร่|ค่าจัดส่ง|ส่งยังไง|จัดส่งยังไง|ส่งแบบไหน/.test(t)) {
        ...
        try {
          if (env.CONV) {
            ...
            if (_pendS) {
              await lineReply(TOKEN, replyToken, "ทีมงานกำลังเช็คค่าส่งด่วนจากแอปให้อยู่ค่ะ เดี๋ยวแจ้งราคาทันทีนะคะ 🛵\n(หรือรับแบบพัสดุปกติ ค่าส่ง 40 บาท ก็ได้ค่ะ) 💕", userId);
              return;
            }
          }
        } catch (e) {}
      ...
      if (notReady) {
        ...
        if ((_wantExpress || _expPending) && expFee === null) {
          ...
          await lineReply(TOKEN, replyToken,
            ...
            (_expPending
              ? "🛵 ทีมงานกำลังเช็คค่าส่งด่วนจากแอปอยู่ค่ะ ทราบราคาแล้วระบบจะสรุปยอดรวม + ส่งการ์ดยืนยันให้ทันทีนะคะ 💕"
              : "🛵 รับแบบส่งด่วน รบกวนแชร์โลเคชั่น (ปักหมุด) จุดจัดส่งมาให้หน่อยนะคะ\nเดี๋ยวทีมงานเช็คค่าส่งจากแอป แล้วระบบจะสรุปยอดรวม + ส่งการ์ดยืนยันให้ทันทีค่ะ 💕"), userId);
          return;
        }
