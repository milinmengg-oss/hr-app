...
const BUILD = "2026-08-04-k165-refundfix";
...
const RX_MONEYBACK = /(ขอ|เอา|โอน|จะเอา|อยากได้)?\s*(ตัง|ตังค์|สตางค์|เงิน|ค่าของ|ยอด)\s*(คืน)|คืน\s*(ตัง|ตังค์|เงิน)|คืนเงิน|โอนคืน|โอนกลับ|ขอเงินคืน|refund|ไม่อยากได้แล้ว|ไม่อยากได้ละ|ไม่เอาของแล้ว|ไม่เอาแล้ว|เปลี่ยนใจ.{0,10}(ไม่เอา|คืน)/i;
...
"   ⛔ ห้ามตัดกลิ่นใดกลิ่นหนึ่งทิ้งเงียบๆ เด็ดขาด ถ้ากลิ่นไหนหมด ต้องบอกลูกค้าตรงๆ ว่ากลิ่นนั้นหมด ห้ามหายไปเฉยๆ" },
    ...
    try {
      if (reply.indexOf("ทีมงานหลังการขาย") !== -1) {
        ...
        const _mbk = RX_MONEYBACK.test(_um), _agr = RX_ANGRY.test(_um);
