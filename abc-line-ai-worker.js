...
const NO_GUESS_RULE = "\n\n# ⛔⛔ ห้ามเดา/แต่งรายละเอียดสินค้าเด็ดขาด\n" +
"- ห้ามแต่งชื่อกลิ่นเอง ใช้ได้เฉพาะชื่อกลิ่นที่ระบบส่งมาให้ในข้อความนี้เท่านั้น ถ้าไม่มีข้อมูล ให้ตอบว่า 'เดี๋ยวแอดมินเช็คกลิ่นที่มีให้อีกครั้งนะคะ 🙏🏻'\n" +
"- ห้ามบอกว่ากลิ่นไหน 'ขายดี' หรือ 'ยอดนิยม' ถ้าระบบไม่ได้บอกมา — บอกได้แค่ชื่อรุ่นกับราคา\n" +
"- ไส้บุหรี่ IQOS / TEREA ⛔ ห้ามจับคู่ชื่อรุ่นกับสี (เช่น 'สีฟ้า = FUSION MENTHOL') เพราะร้านไม่มีข้อมูลสี ให้เรียกชื่อรุ่นภาษาอังกฤษตรงๆ เท่านั้น\n" +
"- เรื่องข้อกฎหมาย/ความถูกผิดของสินค้า ⛔ ห้ามให้ความเห็นหรือรับรองใดๆ ให้ตอบว่า 'เรื่องข้อกฎหมายแอดมินไม่สามารถให้คำแนะนำได้ค่ะ 🙏🏻' แล้วส่งต่อแอดมิน\n" +
"- ⛔⛔ ห้ามเขียนราคาเป็นช่วงเด็ดขาด (เช่น '290-350 บาท') 1 รุ่น = 1 ราคาเท่านั้น ถ้ามีหลายแบบให้แยกบรรทัด\n" +
"- ⛔⛔ MARBO 9K แท้ (350 บาท) กับ MARBO 9K โคลน/เทียบแท้ (290 บาท) เป็น **คนละสินค้ากัน** ห้ามรวมราคา ห้ามเขียนเป็นช่วง\n" +
"  ถ้าลูกค้าพูดถึง MARBO 9K เฉยๆ ให้แจ้งทั้งสองแบบแยกบรรทัด แล้วถามว่ารับแบบไหน เช่น:\n" +
"  • MARBO 9K แท้ = 350 บาท/แท่ง\n" +
"  • MARBO 9K โคลน (เทียบแท้) = 290 บาท/แท่ง";
...
"7) ถ้ารูปไม่มีการวง/ทำเครื่องหมายเลย → ถามว่า \"สนใจรุ่นไหนในภาพคะ 💕 บอกชื่อรุ่นกับกลิ่นมาได้เลยนะคะ\"" },
      ...
      // 🧵 คำถามสั้น/กำกวม ("เหลือกลิ่นเดียวหรอ" "แล้วอันนี้ล่ะ" "เท่าไหร่") → บอกจีทูว่ากำลังคุยรุ่นไหนอยู่
      // เคสจริง: ลูกค้าถามต่อเรื่อง RELX BOOST POD แต่จีทูลอกคำตอบเก่าเรื่องแบรนด์ ABC มาตอบซ้ำ
      let ctxNote = "";
      try {
        // ถ้าลูกค้าเอ่ยชื่อรุ่นเอง (ไทยหรืออังกฤษ) = เปลี่ยนเรื่องแล้ว ห้ามใบ้รุ่นเก่า
        const namedModel = FLAVOR_KEYS.some(k => normTH(text).indexOf(normTH(k)) !== -1)
          || TH_MODEL.some(([re]) => re.test(text));
        const short = text.length <= 30 && !namedModel;
        if (short && history.length) {
          const recent = history.slice(-4).map(h => typeof h.content === "string" ? h.content : "").join(" ");
          let last = null;
          for (const k of FLAVOR_KEYS) { if (recent.indexOf(k) !== -1) { last = k; break; } }
          if (!last) for (const [re, key] of TH_MODEL) if (re.test(recent)) { last = key; break; }
          if (last) ctxNote = "\n\n[ระบบใบ้ให้ — ห้ามพูดถึงข้อความนี้] ลูกค้ากำลังถามต่อเนื่องเรื่อง **" + last + "** " +
            "→ ตอบเฉพาะเรื่องรุ่นนี้ให้ตรงคำถาม ⛔ ห้ามลอกคำตอบเดิมจากข้อความก่อนหน้ามาตอบซ้ำทั้งดุ้น ⛔ ห้ามเปลี่ยนไปพูดเรื่องรุ่น/แบรนด์อื่นเอง";
        }
      } catch (e) {}
      const hint = aliasHint(text) + flavorHint(text, smForHint, bufForHint) + brandHint(text, smForHint, bufForHint) + ctxNote;
    ...
    if (reply.indexOf("ทวนคำสั่งซื้อ") !== -1) {
      ...
      if (notReady) {
        ...
        try {
          // ✅ นับทั้งที่ "ลูกค้าพิมพ์" และที่ "จีทูเคยเสนอไปในเทิร์นก่อนๆ"
          // (เคสจริง: ลูกค้าส่งรูปวงกลมกลิ่น → จีทูอ่านรูปแล้วทวนชื่อกลิ่น → ลูกค้าตอบแค่ "กลิ่นละ 2 ชิ้น"
          //  ถ้านับเฉพาะที่ลูกค้าพิมพ์ จะบล็อกการ์ดแล้ววนถามซ้ำไม่จบ)
          // ⛔ ไม่นับข้อความที่จีทูกำลังจะตอบรอบนี้ — กันการ์ดผีที่จีทูกุกลิ่นขึ้นมาเองในเทิร์นเดียว
          const custSaid = [String(msgText || "")].concat(
            history.slice(-8).map(h => typeof h.content === "string" ? h.content : "")
          ).join(" ");
          ...
          if (!fromImage && !okConfirm) {
            ...
            if (ghost.length) {
              ...
              let gn = 0;
              try { gn = parseInt((await env.CONV.get("ghost:" + shopId + ":" + userId)) || "0", 10) || 0; } catch (e) {}
              if (gn >= 1) {
                try { await env.CONV.delete("ghost:" + shopId + ":" + userId); } catch (e) {}
                await muteNow("จีทูสรุปรายการไม่ได้ (ถามวน) 🔁", String(msgText || "").slice(0, 80));
                await lineReply(TOKEN, replyToken, "ขออนุญาตให้แอดมินสรุปรายการให้นะคะ 🙏🏻 รอสักครู่ค่ะ เดี๋ยวแอดมินเข้ามาดูแลต่อทันทีค่ะ 💕", userId);
                return;
              }
              try { await env.CONV.put("ghost:" + shopId + ":" + userId, String(gn + 1), { expirationTtl: 1800 }); } catch (e) {}
