...
## ขั้น 1 — ลูกค้าเลือกรุ่น+กลิ่น/สี+จำนวนครบแล้ว → สรุปยอด + ให้โอน (⛔ ห้ามขอที่อยู่ตอนนี้)
ออกบล็อกนี้เป๊ะ:
💰 ยอดที่ต้องชำระ
...
แล้วแจ้งเลขบัญชีร้าน (จาก "ข้อมูลชำระเงินของร้าน" ถ้ามี) และปิดท้ายว่า "โอนแล้วส่งสลิปกลับมาในแชทนี้ได้เลยนะคะ ระบบจะตรวจสลิปให้อัตโนมัติค่ะ 💕"
...
แล้วปิดท้ายว่า "รับออเดอร์เรียบร้อยค่ะ 🎉 แอดมินจะจัดส่งให้เลยนะคะ ขอบคุณที่อุดหนุนค่ะ 💕"
...
async function handleEvent(ev, env, TOKEN, shopId) {
  try {
          ...
          if (sok) {
            ...
            if (sok.httpOk && sok.success && d.success) {
              ...
              if (!expected) {
                ...
                customerMsg = "✅ ตรวจสอบการชำระเงินเรียบร้อยค่ะ\nยอดโอน " + slipAmt + " บาท เข้าบัญชีร้านถูกต้อง 🎉 ขอบคุณที่อุดหนุนนะคะ 💕" + ADDR_FORM;
              } else if (Math.abs(slipAmt - expected) <= 1) {
                slipPassed = true;
                statusLine = "ชำระแล้ว ✅ ยอด " + slipAmt + " ตรงออเดอร์ → " + recv + " (รอที่อยู่จัดส่ง)";
                customerMsg = "✅ ตรวจสอบการชำระเงินเรียบร้อยค่ะ\nยอด " + slipAmt + " บาท ตรงกับออเดอร์ เข้าบัญชีร้านถูกต้อง 🎉 ขอบคุณที่อุดหนุนนะคะ 💕" + ADDR_FORM;
              } else {
    ...
    const isPayBlock = reply.indexOf("💰 ยอดที่ต้องชำระ") !== -1;   // ขั้น 1: แจ้งยอดให้โอน (ยังไม่ชำระ)
    const isOrderBlock = reply.indexOf("📦 สรุปออเดอร์") !== -1;      // ขั้น 3: สรุปออเดอร์หลังชำระ+ที่อยู่
    try {
      if (env.CONV && (isPayBlock || isOrderBlock) && hasItem) {
        const ordKey = "ord:" + shopId + ":" + userId;
        const name = await lineProfileName(TOKEN, userId);
