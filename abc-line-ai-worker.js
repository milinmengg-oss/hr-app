    ...
    if (mtype === "text") {
      ...
      // ✅ ยกเว้นการ "ถามเงื่อนไข" (เคลมยังไง/กี่วัน) → ปล่อยให้ตอบข้อมูลตามปกติ ไม่ต้องส่งต่อ
      {
        ...
        try {
          if (env.CONV) {
            const _ov = await env.CONV.get("ord:" + shopId + ":" + userId);
            if (_ov) { const _oj = JSON.parse(_ov); _hasOrder = !!(_oj && _oj.status); _paid = /✅/.test(String((_oj && _oj.status) || "")); }
          }
        } catch (e) {}
...
async function lineReply(token, replyToken, text, userId, quick, sysAck, force) {
  ...
  if (!force && userId && MUTED_UIDS.has(userId)) {
    ...
    if (_q && _q.left > 0) { _q.left--; }                 // ข้อความแจ้งรับเรื่อง 1 ครั้ง = ผ่านได้
    else { console.log("K201_BLOCK_AFTER_MUTE uid=" + String(userId).slice(0, 8)); return; }
  }
