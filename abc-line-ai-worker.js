    ...
    if (env.CONV) {
      ...
      if (_mv) {
        ...
        if (!_qOnly) {
          ...
          // 🩹 k185 (GR-G): เดิมพอส่งต่อคนแล้ว ข้อความถัดๆ มาของลูกค้าจะเงียบสนิท 12 ชม.
          //   เคสจริงจากรอบตรวจ 100 เคส: ส่งสลิป → ตอบ "รอทีมงานตรวจสอบ" → ส่งสลิปซ้ำ → **เงียบ**
          //   กระทบทุกเส้นที่ส่งต่อคน (เงินคืน · ลูกค้าโกรธ · สลิปมีปัญหา · ยอดไม่ตรง)
          //   = บทสนทนาที่เดิมพันสูงสุด แต่ลูกค้ารู้สึกเหมือนโดนเท
          //   ⚠️ ยังต้องเงียบเป็นหลัก (ให้คนรับช่วง) — แค่บอกครั้งเดียวว่ารับเรื่องแล้ว
          //   ⛔ ห้ามใส่ราคา/ยอด/เลขบัญชี/ชื่อรุ่น — จะขัดกับสิ่งที่ทีมงานกำลังคุย
          try {
            const _ak = wakeKey(shopId, userId);
            const _last = MUTEACK.get(_ak) || 0;
            if (Date.now() - _last > 1800000) {          // ตอบรับได้ครั้งเดียวต่อ 30 นาที
              MUTEACK.set(_ak, Date.now());
              if (MUTEACK.size > 500) { const k0 = MUTEACK.keys().next().value; MUTEACK.delete(k0); }
              await lineReply(TOKEN, replyToken, "รับเรื่องไว้แล้วนะคะ 🙏🏻 ทีมงานกำลังดูแลอยู่ค่ะ รบกวนรอสักครู่นะคะ 💕", userId, null, true);
              console.log("K185_MUTE_ACK uid=" + userId);
            }
          } catch (e) {}
...
async function lineReply(token, replyToken, text, userId, quick, sysAck) {
